...
| PR-004 | SPIFFE Workload API JWT-SVID unimplemented (X.509 complete) | Low | PARTIALLY FIXED — tracker [#3617](https://github.com/ferrum-edge/ferrum-edge/issues/3617). `FERRUM_MESH_CA_BACKEND=internal` implements mint / validate / JWT bundle streaming behind `FERRUM_MESH_WORKLOAD_API_ENABLED`, with operator-configured ES256 signing material (restart + multi-replica continuity) and a provable rotation overlap. `spire` serves bundles/validate from the agent's own `FetchJWTBundles` stream; **delegated `FetchJWTSVID` stays fail-closed `UNIMPLEMENTED` there as a terminal boundary** — SPIRE authorizes that RPC by the calling process's identity, so proxying it would substitute Ferrum's own SPIFFE ID. `FetchJWTBundles` never streams an empty map in either posture |
...
| SPIFFE Workload API JWT-SVID delegated mint (SPIRE backend) | #3617 | Mint/validate/bundles implemented for the `internal` CA backend; `spire` serves bundles+validate. Delegated mint under `spire` requires SPIRE's delegated-identity/admin API and stays fail-closed — documented terminal boundary, not a deferral |
