...
| PR-004 | SPIFFE Workload API JWT-SVID unimplemented (X.509 complete) | Low | PARTIALLY FIXED — tracker [#3617](https://github.com/ferrum-edge/ferrum-edge/issues/3617). `FERRUM_MESH_CA_BACKEND=internal` implements mint / validate / JWT bundle streaming behind `FERRUM_MESH_WORKLOAD_API_ENABLED`, with operator-configured ES256 signing material (restart + multi-replica continuity), **externally-rotated-only signing keys** (an in-process replacement for configured material is refused, since it would diverge per replica and vanish on restart), and a provable rotation overlap for the dev-only ephemeral key. **Serving a Workload API on `FERRUM_MESH_CA_BACKEND=spire` is refused at startup** — a SPIRE agent issues only the calling process's own identity, so neither X.509 nor JWT mint could serve an attested downstream workload, and proxying its mint RPC would substitute Ferrum's SPIFFE ID. Ferrum still consumes SPIRE X.509/JWT trust material for peer verification under that backend. `FetchJWTBundles` never streams an empty map |
...
| SPIFFE Workload API delegated issuance (SPIRE backend) | #3617 | Mint/validate/bundles implemented for the `internal` CA backend. Serving a Workload API on `spire` is refused at startup and needs SPIRE's delegated-identity/admin API — documented terminal boundary, not a deferral. Trust-material consumption under `spire` is unaffected |
