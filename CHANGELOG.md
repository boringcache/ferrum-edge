...
- `ai_semantic_cache` no longer discards Redis quarantine-`DEL` failures for
  ...
  + 30s TTL, hard-capped) prevents immediate re-download/parse/delete
  amplification of the same poisoned remote value while still reconsidering
  repaired replacements within that bound. Invalid entries remain unserved;
