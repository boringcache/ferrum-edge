...
- Gateway API `GRPCRoute` predicates are now translated instead of dropped.
  ...
  parentRef the operator wrote. Cross-kind routes never collapse, including
  when separate listeners admit them: because the route table is port-agnostic,
  merging would make each backend reachable through the other kind's listener.
  An identical `(hosts, listen path)` therefore fails config validation closed.
