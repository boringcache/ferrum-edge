...
3. **eBPF Builder Stage**: Only the explicit `runtime-ebpf` / `runtime-ebpf-tools` targets compile `ebpf/ferrum-ebpf` to a BPF ELF with LLVM 19 development tooling, an explicit `LLVM_PREFIX` for `bpf-linker`, nightly Rust, `rust-src`, and `-Z build-std=core`
