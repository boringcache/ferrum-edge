...
|-------------|-------------|
| `dns_override` | Static IP address override for this proxy's backend. Bypasses all DNS resolution for every hostname the proxy dials (including load-balanced target hosts that differ from `backend_host`). |
