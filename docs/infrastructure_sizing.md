...
At this scale, use the [Control Plane / Data Plane](cp_dp_mode.md) deployment mode with multiple Data Plane instances for horizontal scaling. For multi-region deployments, see [multi_region_ha.md](multi_region_ha.md): configure `FERRUM_DP_CP_GRPC_URLS` with CPs that **serve the DP's namespace** (same-region replicas or hot-standby CPs), not CPs in other regions that run a different `FERRUM_NAMESPACE`.
...
| `FERRUM_ACCEPT_THREADS` | `0` (auto-detect = CPU cores) | Parallel accept() loops per proxy listener port on duplicated fds of one exclusive listen socket (never SO_REUSEPORT); non-Unix falls back to one loop |
...
**`FERRUM_ACCEPT_THREADS`** — The number of parallel accept() loops per proxy listener port. Ferrum binds **one exclusive listen socket** and duplicates that fd for extra workers, so tokio can poll `accept()` on several tasks without setting `SO_REUSEPORT`. A second unrelated process therefore cannot bind the same TCP proxy address/port and split traffic. This is **orthogonal** to `FERRUM_WORKER_THREADS` — worker threads control the tokio runtime thread pool for all async work (request handling, plugin execution, backend calls), while accept threads specifically parallelize **connection intake**. The default of `0` auto-detects available CPU cores. Set to `1` to disable multi-accept. Only effective on Unix; non-Unix platforms warn and run one accept loop.
...
| `net.ipv4.ip_local_port_range` | `1024 65535` | Maximize available ephemeral ports. The gateway applies `IP_BIND_ADDRESS_NO_PORT` on outbound sockets to optimize port reuse. Monitor `port_exhaustion_events` in authenticated `GET /overload` detail; unauthenticated callers receive only the coarse `level` |
