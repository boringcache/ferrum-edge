...
| `FERRUM_ACCEPT_THREADS` | `0` (auto-detect = CPU cores) | Parallel accept() loops per proxy listener port via SO_REUSEPORT; non-Unix falls back to one loop |
