...
*A source tuple and a socket mark are both forgeable (issues #3956, #3957).* Under this datapath's threat model — a same-node workload holding `CAP_NET_ADMIN` in the HOST network namespace — every attribute a packet carries is attacker-chosen. `SO_MARK` sets `NODE_WAYPOINT_INBOUND_AUTH_MARK`; binding a node-local address sets a trusted node source; `IP_TRANSPARENT` plus a bind sets a published Service ClusterIP and port. So neither `node source + mark` nor `reply tuple + mark` is an authorization, and neither is their disjunction: because the reply-source map is listener-wide, a replay of one published tuple reaches **any** enrolled destination on the node, not just the Service that published it. Swapping one lane for the other moves the bypass and black-holes half the datapath — the backend dial needs the node-source lane and the source-pinned reply needs the reply-source lane.
...
Because the helper is a `SCHED_CLS` helper gated on the kernel's `CONFIG_SOCK_CGROUP_DATA`, a kernel without it fails the classifier at **load**, not at admission — a visible enrollment failure rather than a silent open lane. That configuration is already implied: `CONFIG_CGROUP_BPF`, which this node-agent's `connect4`/`connect6` hooks require, selects it.
