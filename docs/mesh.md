...
| `FERRUM_MESH_SVID_ROTATION_DRAIN_SECONDS` | `0` | Seconds to wait after a backend client SVID rotation before force-draining old-generation backend pool entries. `0` keeps existing connections until normal idle/health cleanup. SVID rotations only — a committed gateway **trust** withdrawal retires the transports the removed root authenticated synchronously and does not consult this window (see `docs/cp_dp_mode.md`) |
