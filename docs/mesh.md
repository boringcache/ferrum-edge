...
- **Watermarks**: `desired` / `sent` / `acknowledged` each carry `version`, `at`, and `age_seconds` (clamped ≥ 0 and periodically refreshed even while a row is stable). Versions are exact opaque values: non-empty, no surrounding whitespace or control characters, and at most 256 UTF-8 bytes. `desired` advances only when that DP's projected `MeshSlice` content changes; version-only/no-op reloads and unrelated namespaces do not manufacture drift. Retained disconnected rows keep a bounded subscription selector and content digest so the same projection rule remains accurate while partitioned.
