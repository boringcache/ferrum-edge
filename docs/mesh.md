...
- **End-to-end status (#1808, #2013): both topologies now have a UDP source-capture
  ...
  **Scoped-policy
  ...
  idempotence, and pod-deletion cleanup. The separate enrolled-destination two-pod
  fixture remains the residual described in [12]. eBPF UDP capture stays a non-goal
  (#1803 per-pod TC not resurrected).
