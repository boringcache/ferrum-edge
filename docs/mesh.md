...
- `rewrite.authority` → the `Host` / `:authority` forwarded to the backend. The plugin writes the new authority into the request `Host` header and flips `preserve_host_header` on the effective proxy so every backend dispatch path (reqwest / H2 / gRPC / H3 / HBONE) forwards it verbatim.
