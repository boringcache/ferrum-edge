...
*Steering lifecycle.* Desired destinations are derived during mesh materialization and carried as candidate metadata; they are **not** published from preparation. The serving `StreamListenerManager` publishes the intersection of that desired set with listeners that are actually bound on the accepted generation, retracts a destination before its listener stops, and retracts on bind failure, deferred DTLS, task failure, and shutdown — so a rejected candidate cannot change the live datapath and a destination is never marked without a serving socket. Destination and interface updates are one serialized latest-state machine: each event mutates only its component while holding the cold-path mutex, and the backend reconcile always derives from the latest combined state inside that critical section, so a stale cloned plan cannot install after a newer event. Listener task exit marks that exact spawn generation non-serving before any retraction; a failed predecessor cannot retract or republish a successfully bound replacement on the same key. Shutdown fences bind-success watchers under the listener-map lock, publishes the empty plan while they cannot re-enter, and only then closes sockets. Serving ownership is the generated listener identity (`NamespacedResourceId` / reserved `__mesh-nw-udp-` proxy id prefix), never a substring of the namespace-qualified runtime key. The interface set handed to the reconcile is the source-attribution index's **published** generation (`NodeWaypointUdpSourceIndex::publish` returns it), not the planner's pre-publication list: a contested ingress interface (refused for every claimant), a malformed or UID-mismatched binding, a duplicate, and an over-bound generation therefore contribute nothing to steering, so no interface is diverted to a listener that could only deny it. Ferrum-owned objects outlive the process that installed them, so the FIRST reconcile of every process runs the exact-name teardown even when it steers nothing — reaping a crashed predecessor's rules — while later unchanged polls run no command. A generation change EMPTIES the `mangle` mark chain before the `raw` notrack chain is repopulated, so nothing is ever marked under a destination generation whose notrack and local-delivery prerequisites do not match it; the resulting window is unsteered (fail-closed at the pod-veth guard), never cross-generation or wrong-destination admission. Every PUBLISHED address family must install completely: a node missing `ip6tables` while an IPv6 destination is published fails the whole apply under `set -e`, the datapath is torn down by exact name, and the whole plan is retried on the next reconcile — there is no best-effort per-family arm, because a published-but-uninstalled family is a silent black hole that would never be retried. Failed-script diagnostics are bounded and name only Ferrum-owned objects.
...
`FERRUM_UDP_REPLY_SOURCES` / `FERRUM_UDP_REPLY_SOURCES6` hold `(source address, source port)` pairs. In the **tc UDP arm only**, a datagram to an enrolled pod passes when it carries `NODE_WAYPOINT_INBOUND_AUTH_MARK` **and** its source is either an already-trusted configured node source (the relay's backend dial, unchanged) or an exact entry in these maps (the source-pinned reply). Both halves are still required: the mark alone admits nothing, the source alone admits nothing, and TCP does not read these maps at all — its semantics are unchanged. Exactness is the boundary: entries are single addresses with a single port, never a CIDR, a Service range, or a port-blind form, and the guard's own `enrolled_destination_authorized` mark check is the one place the mark is compared, so the two arms cannot drift. IPv4 and IPv6 are required together, because a dual-stack waypoint that authorized one family would black-hole the other.

Authorization is bound to the SERVING lifecycle, not to configuration. The publisher is driven by the same `NodeWaypointUdpSteering` instance that owns the steering rules, which is only ever handed destinations whose listeners are actually bound on the accepted generation — so a candidate address never becomes authorized by being parsed. Within one reconcile the previous generation is torn down first, the new reply sources are authorized **before** the rules that will steer datagrams at them, and on teardown the order reverses (rules first, then the authorization) so nothing is ever steered at an address whose authorization has already gone. Withdrawal, listener failure, steering failure, a refused plan, shutdown, and `Drop` all take the same retraction path. Every failure is fail-closed: a publication that cannot be written refuses the whole generation and tears the datapath down (leaving the Service path unsteered, i.e. its pre-existing dropped-at-the-guard posture), and a withdrawal that cannot be completed is never recorded as done, so the next reconcile retries it instead of settling on `Unchanged`.

Ownership follows the existing split rather than widening it: the node-agent remains the **sole writer of every BPF map**, and the mesh proxy's bpffs mount stays read-only (a writable one would also let the proxy rewrite the pinned original-destination identity records). The proxy therefore publishes on the channel it already owns — the node-agent pod registry directory it writes `.ready` markers into — under `<registry dir>/.udp-reply-src/`, which holds exactly two well-known files. `desired` is written only by the proxy: one bounded, strictly parsed manifest carrying the WHOLE authorized set plus the generation naming it, staged into a temporary sibling and `rename`d into place, so a reader on the 250 ms poll observes the entire previous generation or the entire new one and never a partial set. Each line is `<family>-<hex address>-<port>`, generated from a parsed `IpAddr` and never from operator text, and the lines are strictly ascending, so one set has exactly one rendering: a textual address, uppercase hex, a wrong address width, an extra separator, a non-canonical or zero port, a duplicate, a reordering, a count that disagrees with the body, trailing bytes, or an oversized file refuses the WHOLE generation rather than narrowing it. Hex rather than the textual form makes the set a true set — `::1` and its expanded spelling are one line, so one address cannot hold two authorizations of which a withdrawal removes only one. `applied` is written only by the node-agent, and **only after the complete IPv4+IPv6 set of that exact generation is live in both maps**. A generation is `(owner, sequence)`: the owner is a secure-random per-process token (16 lowercase hex digits; unavailable randomness fails publication closed rather than falling back to a collision-prone value) and the sequence is monotonic within it, bumped whenever the desired set changes. It is not a secret and is never treated as one — it exists so an acknowledgement left by a crashed predecessor, or written for an earlier set, can never be read as proof about the generation a live proxy is asking for. The node-agent reconciles on the same 250 ms cadence as UDP readiness (the bound on how long a revoked ClusterIP can stay authorized), as a **whole-set replacement** that removes stale entries before inserting new ones, so a proxy crash, restart, or missed retraction converges and a partial failure can only ever narrow what is authorized. It retracts the acknowledgement BEFORE it touches a map, re-reads `desired` after both map families complete, and rewrites `applied` only when that exact manifest is still current, so a crash or successor publication mid-apply leaves no proof behind. The proxy also binds the acknowledgement back to its own exact canonical manifest (not only the owner/sequence text), so corrupt same-generation content and a predecessor's late acknowledgement satisfy nothing. It refuses the whole generation — with no acknowledgement — when the set exceeds the map bound (rather than truncating), when either required map is absent from the loaded program **even for an empty withdrawal**, on any scan/remove/insert error or partial family failure, when the manifest is unreadable or malformed, and when any entry names an **enrolled pod address** (the relay may authorize a Service address, never a workload's own; the remainder is NOT authorized in its place). An absent channel is "nothing authorized" rather than a retained previous set. `validate_required` rejects NodeWaypoint capture readiness when either map is missing from the ELF, so a node that cannot authorize its relay's replies never reports ready. Nothing on this channel carries a secret or a peer-supplied value; diagnostics are a closed `&'static str` reason set and are rate-limited to one line per transition.
...
*DTLS material for those listeners.* A `dtls` listener terminates with the DTLS-specific `FERRUM_DTLS_CERT_PATH` / `FERRUM_DTLS_KEY_PATH` (plus optional `FERRUM_DTLS_CLIENT_CA_CERT_PATH` for frontend client-certificate verification), which mesh mode loads at startup — the same sources the database/file/dp serving paths use, and the same sources the mesh PeerAuthentication live reload rebuilds active DTLS configs from, so a listener that binds at startup and one that binds or is swapped after a PeerAuth change always handshake with the same certificate. `FERRUM_FRONTEND_TLS_CERT_PATH` / `_KEY_PATH` are deliberately NOT reused: on a mesh proxy that pair is the operator override for the INBOUND TCP listener's server identity, so sharing it would make configuring a DTLS listener silently replace a SPIRE-issued inbound identity. Expiry is validated at startup with the same gate the other serving modes use; when no DTLS material is configured a `dtls` listener stays visibly deferred (`FrontendDtlsDeferred`) instead of binding. In Helm, mount the material with `ambient.extraVolumes` / `ambient.extraVolumeMounts`.
