...
*How the relay cgroup set is established.* The manifest carries a third field: the publishing proxy's own Kubernetes pod UID (`FERRUM_MESH_NODE_WAYPOINT_RELAY_POD_UID`, chart-rendered from the downward API `metadata.uid` and refused as an operator-supplied value). That is a **name, not an authorization**. The node-agent resolves it against this node's own cgroup hierarchy (`resolve_pod_cgroup_path` + the bounded `collect_cgroup_tree_inodes` walk, the same machinery that enrolls per-pod workload identity) and writes the resulting pod-slice-plus-container-leaf ids; `bpf_skb_cgroup_id` reports the container LEAF, which is why the whole subtree is enrolled rather than the pod inode alone. A publisher cannot widen the sender proof by asserting an id, and the node-agent refuses the whole generation when the named pod is one of THIS node's enrolled workloads, when a resolved id belongs to an enrolled pod's subtree, when the identity resolves to no live cgroup, or when the set exceeds `UDP_RELAY_CGROUP_MAX_ENTRIES` (256, refused whole rather than truncated). The relay may answer for a Service address; it may never answer as one of the workloads this guard protects.
