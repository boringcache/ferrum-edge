...
- **Fail-closed contract (F3 §3.3 Stage 6) — every relay decision drops/rejects on
  ...
  | **Transport selection** — exactly one of `mesh.hbone` / `mesh.mtls` | egress session | end session if neither tag | `egress_transport_branch_selects_by_tag` |
  | **Authenticated peer** (dest) — `peer_spiffe_id.is_some()` | dest handler, before any socket opens | 403 | `authenticated_hbone_*_without_peer_is_rejected` |
  ...
  | **Marker discipline** — `udp`/`hbone` predicates disjoint; an unknown / future marker → neither → 404 | dispatch | 404 (both skew directions) | `connect_marker_classification_is_exhaustive_and_fail_closed`, `udp_and_hbone_predicates_are_disjoint` |
