...
**UDP/DTLS per-pod scoping** (issue #3286): UDP and DTLS stream sessions through a node-waypoint proxy ARE scoped per source pod. The socket-cookie channel TCP uses is unavailable — node-waypoint TCP identity is keyed by the per-connection socket cookie the eBPF `connect4`/`connect6` cgroup hooks stamp, there are no UDP capture hooks, and a UDP stream proxy demultiplexes every client off one shared frontend socket carrying a single cookie — so UDP/DTLS resolves its source through a separate attribution index instead.
