...
| orig-dst → proxy identity bridge (node-waypoint) | **Experimental** | `src/ebpf/orig_dst_bridge.rs` mirrors socket-cookie records into the resolver; the **accept-side cookie bridge (GAP-2M) is now implemented** in the kernel `sock_ops` program (re-keys orig-dst by connection 4-tuple, re-stamps under the accept-side cookie), so resolution can succeed with no proxy-side change. CI compile/load-tests it; end-to-end resolution is **unverified on a live datapath** and fails closed (never misattributes) on a tuple/byte-order mismatch. |
