...
Because the helper is a `SCHED_CLS` helper gated on the kernel's `CONFIG_SOCK_CGROUP_DATA`, a kernel without it fails the classifier at **load**, not at admission — a visible enrollment failure rather than a silent open lane. That configuration is already implied: `CONFIG_CGROUP_BPF`, which this node-agent's `connect4`/`connect6` hooks require, selects it. There is a second, quieter kernel floor (issue #4021): before Linux **5.15** (commit `8520e224f547`) mounting the cgroup-v1 `net_cls` or `net_prio` controller set `cgroup_sk_alloc_disabled`, so `sock_cgroup_ptr()` fell back to the root cgroup and `bpf_skb_cgroup_id()` reported the ROOT id for every socket on the node. That fails CLOSED — the root id is never inside the resolved relay pod slice — but with a symptom indistinguishable from a misresolved relay pod UID, so the effective requirement is **kernel >= 5.15, OR no cgroup-v1 `net_cls`/`net_prio` mounted**. See [`docs/node_agent.md`](node_agent.md) for the operator-facing statement.
