...
Ownership follows the existing split rather than widening it: the node-agent remains the **sole writer of every BPF map**, and the mesh proxy's bpffs mount stays read-only (a writable one would also let the proxy rewrite the pinned original-destination identity records). The proxy therefore publishes CLAIMS on the channel it already owns — the node-agent pod registry directory it writes `.ready` markers into — as one file per authorized source under `<registry dir>/.udp-reply-src/`, named `<family>-<hex address>-<port>`. The name is generated from a parsed `IpAddr`, never operator text, and the node-agent parses it strictly: a textual address, uppercase hex, a wrong address width, an extra separator, a non-canonical or zero port, or anything else authorizes nothing. Hex rather than the textual form makes the claim set a true set — `::1` and its expanded spelling are one name, so one address cannot hold two authorizations of which a withdrawal removes only one. The node-agent reconciles the directory into the maps on the same 250 ms cadence as UDP readiness (the bound on how long a revoked ClusterIP can stay authorized), as a **whole-set replacement** that removes stale entries before inserting new ones, so a proxy crash, restart, or missed retraction converges and a partial failure can only ever narrow what is authorized. It refuses the whole set when it exceeds the map bound rather than truncating, treats an unreadable claim directory as "nothing authorized" rather than retaining the previous set, and refuses outright any claim naming an **enrolled pod address** — the relay may authorize a Service address, never a workload's own. `validate_required` rejects NodeWaypoint capture readiness when either map is missing from the ELF, so a node that cannot authorize its relay's replies never reports ready. A claim carries no secret and no peer-supplied value; diagnostics are a closed `&'static str` reason set and are rate-limited to one line per transition.
