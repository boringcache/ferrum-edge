...
- **`connectionPool.http.http1MaxPendingRequests`** — **applied** (top-level / `portLevelSettings` only — set inside a subset it is deferred-and-warned, see below) as a per-`(host, port)` **pending-request gate** on the **reqwest/HTTP-1.1** backend-dispatch path. It bounds how many requests can be simultaneously *waiting on connection capacity* for a destination; when the queue is full a new H1 request is **shed with a 503** ("upstream overflow" in Envoy terms, classified `dispatch_policy_rejected` so the shed itself is not treated as a connect failure and re-amplified) rather than queued unboundedly. The slot is held only for the connection-pending phase (released the moment dispatch obtains capacity / the response begins), matching Envoy's pending-request gauge. The connection-failure **retry** path re-enters the same gate per attempt (the initial attempt's slot is already released before the retry loop runs), so `retry_on_connect_failure` retries are bounded too. **HTTP/1.1-scoped by design** (the knob is named `http1*`): the multiplexed transports (direct H2, gRPC, HTTP/3, HBONE, mesh-mTLS) do **not** consult it — their request concurrency is governed by `http2MaxRequests` / `h2_max_concurrent_streams`, not a connection-pending queue. **Accepted limitation — buffered bodies only:** the gate governs buffered-body H1 dispatch; requests with a **streamed request body** are excluded from the cap. With a streamed body the reqwest `send()` does not merely wait for connection capacity — it then drives the (possibly slow / large) client upload while holding the slot, and reqwest exposes no post-connect / pre-upload hook to scope the slot to the connection-wait, so capping streamed uploads would shed unrelated requests based on upload duration rather than connection-wait. Enforced via an RAII guard on `ProxyState.backend_pending_limit` (see `src/backend_pending_limit.rs`), keyed per resolved `(host, port)` endpoint. Zero is rejected at translate time, and the native/file mesh-config path applies the same positive-value validation (`Some(0)` would shed every H1 request to the destination).
...
| `trafficPolicy.connectionPool.http.http1MaxPendingRequests` | Supported (buffered-body HTTP/1.1 dispatch only) | Lands on `Upstream.port_overrides[port].http1_max_pending_requests` and projects onto `Proxy.pool_http1_max_pending_requests` via the per-target effective proxy. Enforced on the **reqwest/HTTP-1.1** backend-dispatch path (`proxy_to_backend` in `src/proxy/mod.rs`) as a per-`(host, port)` **pending-request gate** (`src/backend_pending_limit.rs`, a sharded `CachePadded` atomic CAS-bumped like `backend_conn_limit`): a request that cannot acquire a pending slot is **shed with a 503** ("upstream overflow"), classified `dispatch_policy_rejected` so the shed itself is not treated as a connect failure (it is not re-amplified, and it does not trip backend circuit breakers). The slot is acquired in the *pending-acquire phase* (immediately before dispatch onto the shared reqwest client) and released by an RAII guard the moment dispatch returns (capacity obtained / response begun / dial failed) — so a request counts as "pending" exactly while it waits for connection capacity, matching Envoy's pending-request gauge, NOT for the response-stream lifetime. The gate is consulted only for dispatch **known to be HTTP/1.1 at acquire time** (`reqwest_dispatch_is_http1_only`): a `DO_NOT_UPGRADE` proxy, a **plaintext `http`** backend (reqwest never speaks h2c over cleartext), or an **HTTPS backend with h2 disabled**; an HTTPS backend that may ALPN-negotiate h2 is left **uncapped** (an `http1*` knob must not 503 an h2 backend — that is `http2MaxRequests`'s job). **HTTP/1.1-scoped by design** (the knob is named `http1*`): the direct-H2 / gRPC / HTTP/3 / HBONE / mesh-mTLS branches return before reaching the reqwest path, so the multiplexed transports never consult the gate — their request concurrency is governed by `http2MaxRequests` (`h2_max_concurrent_streams`). **Accepted limitation — buffered bodies only:** requests with a **streamed request body** are excluded from the cap, because with a streamed body `send()` then drives the entire client upload while holding the slot and reqwest exposes no post-connect / pre-upload hook to scope the slot to the connection-wait; capping them would shed unrelated requests based on upload duration. The connection-failure retry path (`proxy_to_backend_retry`) **re-enters the same gate per attempt** under the same H1 predicate (the initial attempt's slot is already released before the retry loop runs), so `retry_on_connect_failure` retries are bounded too and an attempt never double-counts. Top-level fan-out applies to every target port; per-port `portLevelSettings` overrides per-port. Set inside a `subsets[].trafficPolicy` it is NOT applied (deferred + warned), because the subset apply path builds a `SubsetTrafficPolicy` that carries no `connectionPool.http`. Zero is rejected at translate time, and the native/file mesh-config `MeshConfig::validate()` path applies the same positive-value check (`Some(0)` would shed every H1 request to the destination). |
