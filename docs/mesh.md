...
|---|---|---|
| `sourceLabels` | Labels from the exact NodeWaypoint pod scope, or from authenticated-peer-SPIFFE workloads narrowed by authoritative canonical client IP; unresolved replicas are accepted only when every candidate has identical labels | Yes |
