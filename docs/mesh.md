...
- `ferrum_mesh_tcp_connections_opened_total` / `ferrum_mesh_tcp_connections_closed_total` -- TCP connection lifecycle counters. Both halves are keyed on **admission**: the opened counter is emitted once by the accept path after the complete `on_stream_connect` plugin chain accepted the connection, under the final mesh identity, tag-override, and disable metadata — not from inside the `workload_metrics` hook, which several effective instances may each run over intermediate metadata. A connection a stream plugin rejects contributes to neither the opened nor the closed/byte families, so the two counters stay balanced. UDP/DTLS streams are excluded by their `udp` request protocol.
