...
- **Inbound TC ingress redirect implemented (IPv4 + IPv6), opt-in and live-gated** — `ferrum_tc_ingress_redirect` (`ebpf/ferrum-ebpf/src/tc_ingress_redirect.rs`) is a tc **ingress** classifier attached to the node capture interfaces named by `FERRUM_NODE_AGENT_INGRESS_REDIRECT_IFACES` (unset = off, and the whole datapath stays inert). It steers inbound TCP for enrolled workloads into the local NodeWaypoint relay with `bpf_sk_assign()` instead of a node-global `nat PREROUTING -j REDIRECT`. **Scope is per workload and per port**: the destination must be in `FERRUM_POD_IPS`/`FERRUM_POD_IPS6` carrying `POD_CAPTURE_FLAG_INBOUND_REDIRECT`, *and* the exact `(pod address, destination port)` pair must be in `FERRUM_POD_INBOUND_PORTS`/`FERRUM_POD_INBOUND_PORTS6` (derived from the pod's declared `containerPorts`); anything else is returned untouched, so unenrolled traffic is never captured. **Original destination metadata is preserved without NAT** — addresses are never rewritten, so the relay's accepted socket reports the workload's real `podIP:appPort` from `getsockname()` with no conntrack table and no reverse NAT; the relay listener therefore binds `IP_TRANSPARENT`/`IPV6_TRANSPARENT` so its replies may be sourced from the captured pod address. Local delivery of assigned packets rides a Ferrum-owned `ip rule fwmark 0x735 lookup 33134` at priority `101` (below the kernel `main` rule at 32766) plus `ip route add local default dev lo table 33134`, per family; the table is distinct from the UDP TPROXY table `33133` so teardown of one never reaps the other. **Loop prevention** is three independent guards: the relay's own auth mark (`0x734`), the redirect mark itself (`0x735`), and traffic already addressed to the relay port. **Fail closed**: a packet that is in scope but for which no relay socket resolves is dropped, never delivered unredirected. **The node-global iptables fallback is NOT removed** — it remains the documented path for kernels that cannot run eBPF at all (`FERRUM_NODE_AGENT_FALLBACK_MODE=iptables`), which is a separate concern from the covered eBPF path. Verification: the shared decision table (scope, arming, and all three loop-prevention bypasses) is unit-tested in `ferrum-ebpf-common`, and the `ebpf-live` CI job load/verifies the program on a real kernel — which is what checks the `bpf_skc_lookup_tcp`/`bpf_sk_assign`/`bpf_sk_release` reference-tracking rules — then attaches and detaches it on a scratch veth tc ingress hook and round-trips both address families of the scope maps. **End-to-end packet steering is not yet asserted on a live kernel**: no live test drives a real TCP connection through the redirect into a transparent listener and back. That needs a live multi-pod node and belongs in `node-waypoint-ebpf-live`; until it lands, treat the opt-in as unproven end to end — which is why it is default-off and why the iptables fallback is retained.
