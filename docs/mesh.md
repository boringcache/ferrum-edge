...
- `ferrum_mesh_tcp_sent_bytes_total` / `ferrum_mesh_tcp_received_bytes_total` -- TCP byte counters recorded on disconnect. Direction follows Ferrum's gateway-perspective convention, matching `StreamTransactionSummary.bytes_sent` / `bytes_received` and `ferrum_api_bytes_sent_total` / `ferrum_api_bytes_received_total`: **sent** is client-to-backend and **received** is backend-to-client. Istio names the same two counters the other way round (`istio_tcp_sent_bytes_total` is its response direction), so swap the two series when porting an Istio dashboard. The `TCP_SENT_BYTES` / `TCP_RECEIVED_BYTES` Telemetry selectors map to the Ferrum family of the same name, so a disable or tag override follows the Ferrum direction too.
