...
- **Startup is fail-closed**: a missing, unparsable, non-regular, oversized (>64 MiB), unstable, or mesh-invalid document refuses startup, matching file-mode validation semantics. Reads use the shared bounded stable-file primitive (opened-target regular-file check, Unix `O_NONBLOCK`, streaming `limit + 1`, byte-identical dual read). Unknown/extensionless paths sniff JSON vs YAML once from the leading byte so large documents are not parsed twice merely to detect format.
- **Reload**: SIGHUP (Unix; `ferrum-edge reload`). Filesystem and parse work run on `spawn_blocking` so a stalled/large candidate cannot occupy a Tokio core worker. Rapid signals are coalesced (one in-flight generation plus at most one follow-up); an older completed load cannot overwrite a newer accepted generation. A failed reload logs a warning and keeps the last good slice — identical to how the CP consumers retain the last accepted slice. Shutdown aborts any in-flight blocking join handle. Reload is signal-driven only; there is no file watcher or poll timer. Non-Unix platforms load once and require a restart.
