...
| `ferrum.request_transformer.<scope>.enabled` / `ferrum.response_transformer.<scope>.enabled` | request/response transformer plugins with `runtime_overlay_scope: "<scope>"` | When `false`, every header / query / body rule on the plugin instance is short-circuited. Missing key falls back to `default_enabled` (defaults to `true`). Publishing a **response**-side gate map also retires `response_caching` entries stored under the previous map: a cached response is a finalized representation that replays with presentation transforms skipped, so it is refetched from the origin rather than replayed under a policy that did not produce it. A gate cycle that lands back on an identical map keeps its entries. |
