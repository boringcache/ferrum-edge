...
A single-claimant port deliberately keeps its own individual listener with no destination table, which is what preserves the direct-node-address lane documented below. Terminating DTLS is therefore one Service per port; a pre-protocol destination dispatcher that would let plain UDP and DTLS share one bound socket is a documented residual, not a supported configuration.
