...
*The listener that carries it.* A NodeWaypoint materializes UDP/DTLS **service listeners** from the mesh service inventory when `FERRUM_MESH_NODE_WAYPOINT_UDP_LISTENERS_ENABLED=true` (`materialize_node_waypoint_udp_listeners`). Each in-mesh `MeshService` port whose protocol is `udp` or `dtls` becomes ONE datagram listener bound on that port number in the host network namespace, with an upstream whose targets are the service's local-cluster backing pods at their resolved `targetPort` (the same fail-closed named-`targetPort` rule the east-west materializer uses). `udp` relays opaque datagrams; `dtls` terminates frontend DTLS on the listener and forwards plaintext datagrams to the backend — the same L4 transport, so every UDP-family predicate treats the two identically and only the listener's frontend posture differs. Materialization is DP-side, in the same `prepare_normalized_gateway_config_for_mesh` pass as the east-west and egress-gateway stream listeners, so it needs neither a `stream_match` (which `Proxy::validate` rejects on `udp`/`dtls`) nor a CP-side carrier. It is default-OFF because a NodeWaypoint is `hostNetwork: true`: turning it on claims node-wide port numbers for every UDP service in the slice. Refusals are fail-closed and field-specific — a port already claimed by a mesh runtime listener or another stream proxy, a port claimed by two different services (BOTH refused: a datagram carries neither host nor SNI to disambiguate them), a service port with no reachable local-cluster endpoint, and port `0`. A listener that cannot BIND (a `dtls` port with no usable frontend DTLS material, or a scoped listener whose socket cannot enable the ingress cmsg for a family it serves) is recorded as a `StreamBindFailure` in stream-listener health; it is never started as a black hole. An unparseable value for the switch aborts mesh startup rather than silently serving no listeners.
