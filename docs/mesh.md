...
| `FERRUM_MESH_EGRESS_GATEWAY_ADDR` | — | **Source-side** EgressGateway `host:port` for captured EXTERNAL UDP (issue #3263). Set on a `sidecar`/`ambient` data plane. Requires `FERRUM_MESH_EGRESS_GATEWAY_SPIFFE_ID`; one without the other is a startup error. Unset means captured external UDP is dropped, never direct-dialed |
