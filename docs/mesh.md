...
| `trafficPolicy.connectionPool.http.http1MaxPendingRequests` | Supported (HTTP/1.1 dispatch only) | Lands on `Upstream.port_overrides[port].http1_max_pending_requests` and projects onto `Proxy.pool_http1_max_pending_requests` via the per-target effective proxy. Enforced on the **reqwest/HTTP-1.1** backend-dispatch path (`proxy_to_backend` in `src/proxy/mod.rs`) as a per-`(host, port)` **pending-request gate** (`src/backend_pending_limit.rs`, a sharded `CachePadded` atomic CAS-bumped like `backend_conn_limit`): a request that cannot acquire a pending slot is **shed with a 503** ("upstream overflow"), classified `dispatch_policy_rejected` so it is not retried and does not trip backend circuit breakers. The slot is acquired in the *pending-acquire phase* (immediately before dispatch onto the shared reqwest client) and released by an RAII guard the moment dispatch returns (capacity obtained / response begun / dial failed) — so a request counts as "pending" exactly while it waits for connection capacity, matching Envoy's pending-request gauge, NOT for the response-stream lifetime. **HTTP/1.1-scoped by design** (the knob is named `http1*`): the direct-H2 / gRPC / HTTP/3 / HBONE / mesh-mTLS branches return before reaching the reqwest path, so the multiplexed transports never consult the gate — their request concurrency is governed by `http2MaxRequests` (`h2_max_concurrent_streams`). The connection-failure retry path (`proxy_to_backend_retry`) deliberately does NOT acquire a pending slot, so an initial attempt and its retry never both count. Top-level fan-out applies to every target port; per-port `portLevelSettings` overrides per-port. Set inside a `subsets[].trafficPolicy` it is NOT applied (deferred + warned), because the subset apply path builds a `SubsetTrafficPolicy` that carries no `connectionPool.http`. Zero is rejected at translate time. |
