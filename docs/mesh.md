...
- **Policy port** — `UpstreamTarget::dispatch_policy_port()`, i.e. `service_port_policy_key` when mesh service discovery recorded a declared Service port that a Kubernetes `targetPort` remapped, otherwise `UpstreamTarget.port`. This is the key for **every** DestinationRule lookup the upgrade makes: the `connectionPool.tcp.maxConnections` admission slot, and the `resolve_backend_connection_proxy_for_target` projection that supplies `connectTimeout`, `portLevelSettings[].tls` (CA, client cert/key, `verify_server_cert`, SAN allow-list), and the pool knobs. Target selection chooses the policy port; policy always follows the selected Service port, never the workload port and never the transport listener.
