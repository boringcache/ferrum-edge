...
- **`connectionPool.http.h2UpgradePolicy`** — **applied** to the plain-HTTPS backend HTTP/1.1-vs-HTTP/2 dispatch fork. `DO_NOT_UPGRADE` forces the reqwest/H1 path even when the backend capability registry proves H2; `UPGRADE` prefers direct-H2 (and treats an unclassified `Unknown` target as a hint to try H2, staying fail-safe against a proven-`Unsupported` one); `DEFAULT`/absent leaves probe-driven behavior unchanged. Does NOT touch gRPC (always H2) or HBONE/mesh-mTLS transport selection. Unknown enum values are rejected at translate time.
