...
**Mixed valid + inapplicable attachments stay valid.** Native/file/`MeshSubscribe` validation rejects malformed attachments (empty names, over-long strings, cross-namespace Service/Gateway, unsupported GatewayClass names) hard, and still fails closed when *every* attachment is unresolved (missing Service, missing Gateway against a real `waypoint_bindings` inventory, or GatewayClass owned outside the root namespace) so the policy cannot silently broaden. One missing/inapplicable sibling arm does **not** invalidate a policy that still has a valid applicable target — the unmatched arm is retained for OR semantics and never becomes a wildcard. DP slices reconstruct `MeshConfig` without `waypoint_bindings`, so Gateway presence is not proven there; live `waypoint_name` / class stamps keep matching exact and fail-closed.
