...
**Selector policy without proxy labels.** A `WorkloadSelector`-scoped policy whose selector carries labels cannot be evaluated when no proxy labels are resolved, and the cold-path filter would drop it (leaving an empty policy set that evaluates to `Allow`). The plugin distinguishes two cases at construction:

- **Slice-driven injection (mesh mode).** The slice computed authoritative labels that resolved empty — the ambiguous shared-SPIFFE case (the policy rode in as a candidate-any superset and a downstream consumer, per-pod NodeWaypoint or the xDS DP, re-filters against real labels). Construction **tolerates** it with a warning and the cold-path filter drops it from enforcement for this slice; set `FERRUM_MESH_WORKLOAD_LABELS` (or `mesh_slice.labels`) for deterministic scoping.
...
An explicit `portLevelSettings` entry for the selected port still wins: for a port reachable at apply time the per-port `dispatch_port_overrides` map is consulted first and the fallback is the field-level `.or(...)`. The fallback carries the **top-level overlay ONLY** — `dispatch_port_override_fallback_from_upstream` deliberately does **not** fold the upstream's per-port entries into it (folding cross-leaks one port's `connectionPool.http` onto another on a multi-port upstream). **Named-targetPort SD upstreams are an accepted limitation (#1816):** when a Service uses a **named `targetPort`** the per-port entry is keyed by the **declared service port** (named targetPorts resolve to a workload port at runtime and cannot be re-keyed at apply time), while the LB-selected target carries the **resolved workload port** — so the dispatch-time per-port lookup (by resolved port) MISSES and only the **top-level fallback** is consulted; an explicit per-port `connectionPool.http` value authored on a named-targetPort service port is therefore **not applied** to the resolved dial port (the top-level value applies instead). Numeric `targetPort`s are re-keyed P→T at apply time and are unaffected. Resolving per-port-by-service-port for named-targetPort SD upstreams — and immediate route-table rebuild on a DR-only edit to the SD fallback (a `#[serde(skip)]` DR-derived proxy field; `delta_routes_changed` keys on proxy changes, so the edit applies on the next route rebuild — shared with the established per-port `dispatch_port_overrides`) — are tracked in **#1816**.
