...
- `ferrum_xds_streams_rejected_total{node_id}` -- ADS streams the CP rejected for exceeding the per-node concurrent-stream ceiling (`FERRUM_XDS_MAX_STREAMS_PER_NODE`). A growing value flags a misconfigured or hostile DP opening many streams under one node id.
