...
| `{"error":"HBONE backend unavailable: …"}` | An **Ambient / waypoint** dial (HTTP/2 CONNECT over `:15008`). Seeing this from a `sidecar`-topology process means the target was tagged for the Ambient transport, not that the sidecar path failed |
