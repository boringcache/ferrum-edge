...
An explicit `portLevelSettings` entry for the selected port still wins. For a port reachable at apply time the per-port `dispatch_port_overrides` map is consulted first and the fallback is the `.or(...)`. But when a Service uses a **named `targetPort`** the per-port entry is keyed by the **declared service port** (named targetPorts resolve to a workload port at runtime and cannot be re-keyed at apply time), while the LB-selected target carries the **resolved workload port** — so the dispatch-time per-port lookup (by resolved port) would miss and only the fallback would be consulted. To keep "`portLevelSettings` wins" intact in that case, the projection **folds the upstream's explicit per-port `connectionPool.http` fields into the projected fallback** (`dispatch_port_override_fallback_from_upstream` overlays them so the per-port value wins over the top-level value); a mesh outbound HTTP upstream owns exactly one service port, so a single overlaid fallback is correct for every resolved dial port the targets present. **Residual (pre-existing, not introduced here):** only the six `connectionPool.http` fields the fallback carries are folded in this way — other per-port `portLevelSettings` fields (`connectTimeout` / per-port `tls` / `algorithm` / `maxConnections`) authored on a **named-targetPort** service port are still keyed by the declared service port and so are not applied to the runtime-resolved dial port for a pure-service-discovery upstream (numeric `targetPort`s are re-keyed P→T at apply time and are unaffected). Closing that broader per-port-by-service-port resolution for SD upstreams is tracked as a follow-up.
