...
2. **Refuse a plaintext-capable inbound listener.** If the resolved inbound mTLS/HBONE termination listener admits plaintext — PeerAuthentication resolved to `PERMISSIVE` or `DISABLE`, including a per-app-port override, or no usable server identity exists — startup is **refused under `FERRUM_MESH_PRODUCTION_MODE=true`** even when a usable server `ServerConfig` also exists (a production mesh must enforce mTLS on every inbound app port). In dev it is allowed with a loud warning: reaching plaintext in dev is intentional — either an explicit `PeerAuthentication` plaintext-capable mode, or a no-identity posture the config-time `FERRUM_MESH_ALLOW_NO_CA` gate already let through. The **same enforcement runs on PeerAuthentication live reload** (`FERRUM_MESH_PEER_AUTH_LIVE_RELOAD_ENABLED=true`): a later update that would make any inbound app port plaintext-capable is *rejected* under production (the slice is kept-last-good in its entirety — fail-closed by retention, never crashing a running data plane), so a production sidecar cannot start serving plaintext after startup.
