...
**Implementation status — transport vs. materialization.** These are separate layers. The HBONE transport and automatic SVID-mTLS origination are implemented in the proxy datapath and activate whenever a route's upstream target is HBONE-tagged (`mesh.hbone=true`). What Ferrum materializes *from the slice* differs by topology: inbound sidecar routes, egress-gateway routes, and east-west routes are built automatically; **Sidecar/Ambient outbound** routes to in-mesh services are not yet materialized (tracked), so outbound capture currently requires explicitly supplied proxies. An outbound `404` therefore means no route was materialized for that destination — not that mTLS/HBONE is unavailable.
