...
**Stream scoping**: raw **TCP** stream connections through a node-waypoint proxy are scoped per source pod, the same as HTTP/HBONE. The TCP stream accept loop resolves each accepted connection's `SO_COOKIE` through the shared `NodeWaypointIdentityResolver`, looks up that pod's `PolicyScopeCache`, and stamps it onto the connection context so namespace-scoped and selector-scoped `AuthorizationPolicy` DENY/ALLOW rules are enforced for the correct source pod. A connection whose source pod identity cannot be resolved (no node-agent enrollment yet, non-Linux, unknown cookie, slice/identity race) falls back to **mesh-wide-only** policies — the safe default — and the transaction records `mesh_authz.scope_missing=true`. Unlike the HBONE listener, an unresolved TCP stream is not dropped: raw TCP stream proxies may carry non-captured traffic, so a missing scope downgrades to mesh-wide enforcement rather than refusing the connection.
