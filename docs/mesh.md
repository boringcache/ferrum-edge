...
| reqwest HTTP/1.1 | One backend socket per in-flight H1 request | Enforced as an in-flight-request slot held dispatch → response-body completion; over-cap sheds 503 |
| reqwest, may negotiate h2 | hyper-util converges on one shared multiplexed socket per destination | **Not gated** — gating would bound streams, not sockets; one socket already satisfies any `maxConnections >= 1` |
