...
`FERRUM_UDP_REPLY_SOURCES` / `FERRUM_UDP_REPLY_SOURCES6` hold `(source address, source port)` pairs, and `FERRUM_UDP_REPLY_SOURCE_GATE` is one shared BPF-visible enable value both tc UDP classifier families consult before either lookup. A datagram to an enrolled pod passes only when the gate is open, its source has an exact family-map entry, and it carries `NODE_WAYPOINT_INBOUND_AUTH_MARK` (configured node sources remain the unchanged alternative). TCP never reads this surface. Exactness remains the boundary, but map contents alone are not authorization: stale keys can survive a failed scan/removal or a later IPv6 failure after IPv4 succeeded, and are inert while the shared gate is closed.

Authorization is bound to the SERVING lifecycle, not to configuration. The publisher is driven by the same `NodeWaypointUdpSteering` instance that owns the steering rules, which is only ever handed destinations whose listeners are actually bound on the accepted generation — so a candidate address never becomes authorized by being parsed. Within one reconcile the previous generation is torn down first, the new reply sources are authorized **before** the rules that will steer datagrams at them, and on teardown the order reverses (rules first, then the authorization) so nothing is ever steered at an address whose authorization has already gone. Withdrawal, listener failure, steering failure, a refused plan, shutdown, and `Drop` all take the same retraction path. Every failure is fail-closed: a publication that cannot be written refuses the whole generation and attempts strict datapath teardown; a teardown failure retains unproven predecessor state for retry instead of claiming the Service path unsteered, and a withdrawal that cannot be completed is never recorded as done, so the next reconcile retries it instead of settling on `Unchanged`.

Ownership remains split: the node-agent is the sole BPF-map writer, while the proxy publishes one canonical, bounded, atomically renamed whole-set manifest and reads the exact `(owner, sequence)` acknowledgement. The node-agent retracts `applied` first, then disables the shared BPF gate before any map mutation. A failed acknowledgement unlink still attempts and, when fencing succeeds, closes the BPF lane, leaves state unknown/unacknowledged, and retries without applying a successor. With the gate closed it replaces IPv4 and IPv6 map storage, re-reads the exact manifest, enables the gate only when both maps match that manifest, and writes `applied` last. Any scan/remove/insert, partial-family, or revalidation failure leaves all residual keys inert. An acknowledgement-write failure closes the gate again. If gate disable itself fails, the node-agent reports a hard residual and does not mutate or acknowledge a new generation; it does not claim stale authorization was revoked. `validate_required` requires both family maps and `FERRUM_UDP_REPLY_SOURCE_GATE`, including for empty withdrawal. Canonical owner/sequence binding, enrolled-pod-address refusal, bounds, redacted closed-set diagnostics, and the proxy's exact acknowledgement check remain unchanged.

*Publishing is not applying.* `NodeWaypointUdpSteering` therefore installs setup rules only once the node-agent has acknowledged the EXACT generation it published. While the acknowledgement is pending, missing, or stale the reconcile reports `PendingAck`, keeps (or reverts) the steering rules absent, and retries on the next ordinary reconcile tick — there is no wait, no sleep, and no blocked runtime worker. An acknowledgement that is later retracted reverts the installed rules on the next pass. Teardown is the mirror: the exact-name rule teardown must **succeed** before the empty generation is published, and the withdrawal is not reported `Removed` until the node-agent acknowledges that empty generation. A failed rule teardown therefore retains the outgoing authorization and retries rules-first; a successfully published but unacknowledged withdrawal keeps being retried rather than settling. A publication or acknowledgement-read failure is a distinct `Failed` outcome that keeps the datapath fail closed. Without this gate every add/update had a steered-but-unanswerable window, and a node-agent that never converged left the new steering installed indefinitely while the pod-veth guard still dropped every reply.
