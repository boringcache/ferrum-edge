...
| `trafficPolicy.connectionPool.http.http1MaxPendingRequests` | Supported (honest reinterpretation — max concurrent in-flight HTTP/1.1 requests) | Projects onto the selected target's effective `Proxy.pool_http1_max_pending_requests`. Ferrum caps concurrent in-flight reqwest/H1 requests per `(host, policy port, selected subset)` because reqwest exposes no true connection-pending hook. A sharded `CachePadded` gate rejects overflow with a backend-neutral 503; its RAII permit releases at response headers or on every error, deadline, cancellation, and retry exit. Retry attempts reacquire independently. Direct H2, gRPC, H3, HBONE, and mesh-mTLS do not consult the H1 gate. Precedence is per-port > selected subset > top-level, and length-prefixed subset keys isolate distinct subset *names* (and unmatched destinations) on the same `(host, policy port)`. **Caveat:** the H1 lane key has no upstream/Service identity, so two Services selecting the same pods under a conventionally-named subset (e.g. both `v1`) with different caps still share one lane (issue #3778); the separate `http2MaxRequests` breaker is already keyed by logical destination. Zero rejects K8s and native/file mesh validation. |
...
**Lane identity.** The counter is keyed by `(namespace, logical destination, DestinationRule policy port, selected subset, config generation)`. The logical destination is the referenced upstream id — the Kubernetes Service / DestinationRule cluster — or the route id for a direct-backend proxy; it is deliberately **not** a resolved host or IP, so every endpoint of one destination shares one budget (connection count, pool sharding, and LB rotation cannot multiply it) while two Services that select the same pods keep independent budgets. The policy port is `dispatch_policy_port_for_target`, so a `targetPort` remap does not split a destination's budget and an explicit `portLevelSettings[{port}]` cap is visible to every frontend. `None` (unmatched destination) is a distinct lane from every named subset, and the subset component is length-prefixed so a hostile subset name cannot forge a sibling's lane.
...
**Reload and retirement.** The lane carries the request's config generation, so updating or deleting a DestinationRule applies the new limit to future attempts while permits held under the previous generation keep decrementing their own lane (never double-decrementing, never orphaning). Check-and-reserve, release, and at-zero eviction all run under the same `DashMap` shard lock, so a lane is evicted the moment its count returns to zero and destination/subset/generation churn cannot grow the map for the gateway's lifetime.
