- **reqwest (HTTP/1.1 and ALPN-negotiated HTTP/2)**: `reqwest::Client` fully owns and hides its internal socket pool (`src/connection_pool.rs`), so the cap is enforced at reqwest's **connector** — the one place a new physical socket is dialed — through the vendored `ClientBuilder::connection_admission` hook (`docs/upstream-reqwest-patches/003-connection-admission-hook/`). A checkout that reuses an idle pooled socket never reaches the connector, and an HTTP/2 stream multiplexed onto an existing connection never reaches it either, so **reuse and multiplexing take no slot**. The token the hook returns is owned by the connection object handed to hyper, so the slot retires exactly when that socket dies: handshake failure, normal close, idle eviction, pool drain, cancellation, and shutdown. That is what makes the bound *physical*: after a request completes, reqwest keeps its socket open and idle, and the slot stays held for it.
  - **One ceiling across pool keys.** A single admission hook is installed on **every** pooled `reqwest::Client`, so divergent reqwest pool keys for one destination (TLS material, the `rcfg` client-behavior suffix, the forced-H1 ALPN discriminator, `upstream_subset`) contend for the same slots instead of each getting their own ceiling.
  - **Lane publication.** The connector only sees the dial `(host, port)` from the request URI; the cap and the DestinationRule **policy** port live on the dispatching `Proxy`. The dispatch path therefore publishes the lane (dial `(host, port)` → policy port + cap) immediately before handing the request to reqwest, and **only when a cap is configured** — an uncapped destination touches nothing on the hot path, and publication is idempotent and allocation-free on repeat. Counting happens on the policy port, so a `targetPort` remap shares the destination's one lane with the WebSocket and raw-TCP paths.
  - **Reload/update/delete.** Lanes carry a config epoch that the single config-publish chokepoint (`mirror_request_epoch_wrappers`) advances, so a `maxConnections` an operator removed stops being enforced without a withdrawal sweep over every host a proxy ever dialed; a cap that still exists is re-published by the next dispatch, before the dial that dispatch can cause.
  - **Over-cap posture.** The connector refuses the dial and reqwest surfaces it as a connect error whose source chain carries the marker; `backend_conn_limit::is_backend_connection_limit_error` recognizes it and the dispatch answers `503` classified `DispatchPolicyRejected`, so — exactly like the `http1MaxPendingRequests` shed — it stays neutral to backend health, circuit breaker, and adaptive concurrency, and is not re-driven as a connect-failure retry. The retry path applies the identical treatment.
  - **Residual.** Warmup probes (`FERRUM_POOL_WARMUP_ENABLED`) run before any dispatch has published a lane, so a warmup socket is admitted uncapped; it is then reused by real traffic rather than added to it.# Mesh Mode
...
| HTTP/3 | Pooled QUIC connection(s), UDP transport | Enforced per constructed QUIC connection; slot held on the pooled handle |
...
- **Stream-family (TCP / TCP+TLS / TCP-passthrough)**: each accepted stream dials one dedicated backend socket whose lifetime equals the relay session. Enforced at dial with an RAII counter on the shared `BackendConnectionLimiter`, held on `TcpProxyMetrics.backend_inflight` (`src/proxy/tcp_proxy.rs`).
...
- **Pooled multiplexed transports (direct H2, gRPC, native HTTP/3, HBONE, mesh-mTLS)**: the slot is reserved at the exact moment a NEW physical connection is about to be constructed — after the pool has already missed its cache, so reuse never takes a slot — and is then handed to that connection's own lifecycle owner: the spawned hyper/`h2` connection-driver task for the H2-family pools, and the pooled `H3PooledConnection` handle for QUIC. Consequences, all of them exact rather than approximated:
  ...
  - **Idle eviction, unhealthy replacement, `clear()`, `force_drain_svid_generation` (SVID rotation), config reload/update/delete, and shutdown** all drop the pooled handle, which ends the driver, which releases the slot. There is no separate retirement path to keep in sync.
  - **Over-cap create** returns a pre-wire, pool-side refusal classified `ConnectionPoolError` (`Http2PoolError::MaxConnectionsExceeded`, `GrpcBackendUnavailableKind::MaxConnections`, `HbonePoolError::MaxConnectionsExceeded`), so `retry_on_connect_failure` may rotate to another LB target with its own lane — matching the raw-TCP over-cap posture. The direct-H2 and gRPC pools first fall back to an already-established shard, so a destination whose cap is smaller than `http2_connections_per_host` simply converges on `cap` connections and multiplexes onto them instead of failing requests.
- **reqwest HTTP/1.1**: `reqwest::Client` fully owns and hides its internal socket pool (`src/connection_pool.rs`) and exposes no connection-close hook, so Ferrum cannot hold a slot for a socket's lifetime there. It can hold one for the *request's* lifetime, and on a dispatch **known** to be HTTP/1.1 (`reqwest_dispatch_is_http1_only` — a `DO_NOT_UPGRADE` proxy, a plaintext `http` backend, an HTTPS backend the capability registry already classified H1-only, or `pool_enable_http2: false`) the two coincide: one in-flight H1 request occupies exactly one backend socket, and hyper-util only dials when checkout finds no idle socket, so bounding concurrent in-flight H1 requests to N bounds concurrently-open H1 sockets to N. The slot rides `ReqwestBackendRequestGuard`, which the streaming path already hands to the response body, so it spans dispatch → response-body completion (the true socket-busy window) rather than being released at response headers like the separate `http1MaxPendingRequests` gate. Over the cap the request is shed with `503` before any dial, classified `DispatchPolicyRejected` so it stays neutral to backend health, circuit breaker, and adaptive concurrency. The connection-failure retry path re-enters the same gate per attempt.
- **reqwest dispatch that may ALPN-negotiate h2** is the one deliberate residual: its requests multiplex onto a single shared socket per destination, so an in-flight-request gate there would bound streams rather than sockets. hyper-util converges that path on one connection per destination, which already satisfies any `maxConnections >= 1` (`0` is rejected at translate time). A socket-exact bound would require forking reqwest's connector — its `Conn` type is sealed and `connector_layer` cannot attach a drop-guard to the returned connection.
