...
`waypoint_gateway_class` is emitted only for ServiceWaypoint slices with a known `Gateway.spec.gatewayClassName`. The DP reconstructs it before `mesh_authz` cold-path retain so GatewayClass `targetRefs` exact-match fail closed over xDS (missing, malformed, oversized, duplicate, or removed carriers never reuse a stale class).
...
| `TargetRefs { attachments }` | Istio `AuthorizationPolicy.spec.targetRefs` attachment scope. Supported attachments are same-namespace (or ReferenceGrant-authorized) `Service` / `ServiceEntry`, same-namespace waypoint `Gateway` (`istio-waypoint` / `ferrum-waypoint`), and root-namespace `GatewayClass`. Service/ServiceEntry attachments match by exact destination service namespace/name membership (fail closed when membership is missing) — shared selector labels alone never broaden a policy onto a sibling Service. Gateway attachments match the named waypoint; GatewayClass attachments match only the waypoint's exact `spec.gatewayClassName` (`istio-waypoint` never attaches to `ferrum-waypoint`, and vice versa). Missing targets, unsupported group/kind, empty attachments, `targetRefs` on PeerAuthentication/RequestAuthentication/Telemetry/ProxyConfig, and `selector`+`targetRefs` together fail closed at translation/config validation — targeted policies are never broadened into namespace or mesh-wide scope. |
