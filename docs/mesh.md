...
- `failoverPriority` is an ordered list of workload-label keys (`key`) or key/value overrides (`key=value`). When set it **replaces** the default region/zone/subzone tiers with label-match priority tiers: endpoints matching all N configured labels with the source (or the configured override values) have priority P(0); matching the first N-1 labels yields P(1); matching none yields P(N). Matching is prefix-ordered — the nth label is considered only when the first n-1 matched. Missing labels on either the source or an endpoint compare as empty strings (Istio map-lookup semantics), so missing-on-both matches and missing-on-one mismatches. Duplicate entries are accepted and evaluated as independent ordered steps (with an operator-visible warning). Well-known topology keys (`topology.kubernetes.io/region`, `topology.kubernetes.io/zone`, `topology.istio.io/subzone`, `topology.istio.io/network`, `topology.istio.io/cluster`, `kubernetes.io/hostname`) resolve from explicit workload/endpoint labels and, for region/zone/subzone/network/cluster, from derived locality / network / cluster metadata stamped at materialization. Source labels are projected onto `Upstream.source_labels` at slice apply; endpoint ranks are precomputed into the load-balancer cache and recomputed on endpoint / locality / label reload (`update_targets` / config delta). Pair with `outlierDetection` or active health so lower-priority tiers are selected only after higher tiers become unhealthy — without a health signal every endpoint stays healthy and only P(0) is ever chosen. FerrumAccepted status surfaces that advisory in `deferred_fields` when `failoverPriority` is set without `outlierDetection`.

The K8s translator treats `distribute`, `failover`, and `failoverPriority` as mutually exclusive Istio locality-LB modes. Combined modes are rejected at admission instead of being accepted and resolved by runtime precedence. The translator validates each accepted entry at admission — invalid locality strings, slash-malformed locality strings, non-terminal locality wildcards, malformed failover region names (including slash-containing or slash-suffixed values), `from == to` self-failovers, empty `to` maps, empty `failoverPriority` arrays, non-string `failoverPriority` entries, and malformed `key` / `key=value` entries (empty key, leading/trailing whitespace) return a translator error rather than silently dropping the policy or degrading to another locality mode.
