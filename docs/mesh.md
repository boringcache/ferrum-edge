...
rustls/SPIFFE verifier text are **not** in the client body — they are on the
process's own `error!` log line, which is where an operator (and only an
operator) should read them. `error_class=tls_error` in the transaction log is
unchanged, so retry, circuit-breaker, and passive-health accounting behave
exactly as before.
