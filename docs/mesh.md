...
**Stream scoping**: raw **TCP** stream connections through a node-waypoint proxy follow the same per-pod scoping wiring as HTTP/HBONE. The TCP stream accept loop resolves each accepted connection's `SO_COOKIE` through the shared `NodeWaypointIdentityResolver`, looks up that pod's `PolicyScopeCache`, and stamps it onto the connection context so namespace-scoped and selector-scoped `AuthorizationPolicy` DENY/ALLOW rules are enforced for the correct source pod. The **same GAP-2M staging caveat called out above for HBONE applies here**: the resolver's cookie records are populated today by the connect-side `orig-dst` bridge, but the accept path looks up the accept-side cookie, so until the GAP-2M sockops/sk_lookup bridge lands the resolver is not hit by the accept path and every TCP node-waypoint stream resolves `None`. A connection whose source pod identity cannot be resolved (the GAP-2M gap above, no node-agent enrollment yet, non-Linux, unknown cookie, slice/identity race) falls back to **mesh-wide-only** policies — the safe default — and the transaction records `mesh_authz.scope_missing=true`. Unlike the HBONE listener (which fails closed and drops the connection), an unresolved TCP stream is **not** dropped: raw TCP stream proxies may carry non-captured traffic, so a missing scope downgrades to mesh-wide enforcement rather than refusing the connection. Once GAP-2M lands the wiring will start enforcing scoped policies on TCP streams without further proxy-side changes.

**UDP/DTLS limitation**: **UDP and DTLS** stream connections through a node-waypoint proxy are always scoped mesh-wide for authorization. Per-pod scoping cannot be wired for UDP without a new capture path: node-waypoint identity is keyed by the per-connection TCP socket cookie that the eBPF `connect4`/`connect6` cgroup hooks stamp with the source pod, there are no UDP capture hooks, and a UDP stream proxy demultiplexes every client off one shared frontend socket that carries a single cookie — so there is no per-source-pod cookie to resolve. Operators who rely on namespace- or selector-scoped ALLOW or DENY policies for UDP/DTLS stream traffic in node-waypoint topology should be aware that those scoped policies are silently skipped; only mesh-wide policies are evaluated for UDP. TCP and HTTP/HBONE are scoped per source pod.
