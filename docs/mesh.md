...
| `FERRUM_MESH_SVID_ROTATION_DRAIN_SECONDS` | `0` | Seconds to wait after a backend client SVID rotation before force-draining old-generation backend pool entries. `0` keeps existing connections until normal idle/health cleanup. SVID rotations only — a committed gateway **trust** replace/withdraw fences new admission, synchronously evicts affected idle/cached pool entries, and stops accepting removed roots for new handshakes; already-issued long-lived transport handles are not forcibly terminated (issue #3859). Does not consult this window (see `docs/cp_dp_mode.md`) |
