...
| Cross-cluster protocol parity — HTTP-family, WebSocket, raw TCP, UDP/opaque DTLS, and multi-port per-port SNI aliases are supported on the documented topology transports. Ambient native gRPC remains intentionally fail-closed because HBONE has no HTTP/2 trailer path (#2023); use Sidecar mesh-mTLS for native gRPC. Full live two-cluster TPROXY/netns coverage remains part of the UDP live-fixture work, while materialization and destination relay behavior are in-tree covered. | [#2010](https://github.com/ferrum-edge/ferrum-edge/issues/2010) | `docs/mesh.md` cross-cluster egress sections + "Multi-Port Cross-Cluster SNI Aliases" + protocol matrix |
