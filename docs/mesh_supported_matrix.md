...
- **GA track — Ferrum-native sidecar mesh.** `Sidecar` topology + native
  ...
  A no-CA mesh (`FERRUM_MESH_CA_BACKEND=none`) **fails startup closed** (no
  identity ⇒ no mTLS ⇒ PERMISSIVE would accept plaintext) unless
  `FERRUM_MESH_ALLOW_NO_CA=true`, and `FERRUM_MESH_PRODUCTION_MODE=true` refuses
  it unconditionally — so the GA path cannot silently degrade to unauthenticated
  plaintext.
