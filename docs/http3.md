...
decoded response field-section ceiling to `min(the buffered non-`DATA` frame
ceiling, 1 MiB − 1)`. The buffered-frame ceiling is twice the frontend request
policy; the absolute cap keeps QPACK's 32-byte-per-field accounting from
...
| Backend decoded response field-section ceiling | `min(buffered non-`DATA` frame ceiling, 1 MiB − 1)` | On pooled backend connections, QPACK decoding stops before a compact response can expand into enough fields to exceed `http::HeaderMap`'s 32,768-entry capacity (QPACK accounts 32 bytes per decoded field). |
