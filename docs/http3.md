...
- **One deadline domain.** Opening the backend stream and waiting for response headers share a single absolute `dispatch_deadline_at` (the client `grpc-timeout`, else `backend_read_timeout_ms`) — the same total budget the drain-then-read path used. A header-wait expiry is attributed by the pump's `upload_complete` flag: still uploading is a stalled client (health-neutral), complete is a slow backend (504 read timeout). Forwarded byte count is never the test, because a valid zero-message client-streaming RPC forwards no body bytes.
- **One cancellation domain.** Every pump await races a shutdown signal, and the relay retires the pump on *every* exit — completion, `after_proxy` reject, oversized response, header-write failure, and header-wait failure — so the spawned task can never outlive the RPC. An upload that never reached a clean FIN resets the backend request direction with `H3_REQUEST_CANCELLED`.
...
- **A cancelled receive half is released by `Drop`, not by `STOP_SENDING`.** Retiring a pump can cancel a frontend `recv_data` / `recv_trailers` mid-poll, and h3-quinn parks its `quinn::RecvStream` inside a `ReusableBoxFuture` while the read is `Pending`, leaving its own slot as `None`; calling `stop_sending` in that state unwraps the `None` and aborts the process under `panic = "abort"`. Both request-upload pumps — the native one and the cross-protocol H3→H2 pump in `dispatch_grpc_streaming` — therefore carry the receive half's ownership state (`stream_util::H3RequestRecvState`) as an explicit value and call `halt_request_body_if_idle`. Cancellation on a *backend* write or FIN leaves the receive half idle and still gets the graceful `STOP_SENDING(H3_NO_ERROR)`; the parked case is closed by `quinn::RecvStream::drop`, which issues `STOP_SENDING(0)`. Nothing about cancellation, the request-body ceiling, or the backend reset is relaxed to achieve this.
...
- **Streaming request trailers** — after H3 request DATA reaches EOF, the H3-to-H2 pump reads the optional request trailer block and emits it as `Frame::trailers` through `GrpcBody::Channel` before clean H2 END_STREAM. The native H3 relay uses the same sanitizer. Hop-by-hop names, reserved gateway assertions, credentials, forwarding identity, the initial-section-only `Via` intermediary chain, and the gateway-owned `Early-Data` marker are removed; malformed trailer blocks fail the upload closed rather than being converted into a clean truncated request. Application metadata such as request checksums remains eligible for forwarding.
