...
- **One deadline domain.** Opening the backend stream and waiting for response headers share a single absolute `dispatch_deadline_at` (the client `grpc-timeout`, else `backend_read_timeout_ms`) — the same total budget the drain-then-read path used. A header-wait expiry is attributed by **who the gateway was waiting on**, not by whether the upload finished: on a duplex relay a healthy bidirectional client keeps its request direction open for the whole RPC, so "upload not complete" is the normal steady state and would otherwise excuse every backend that accepts a stream and never sends HEADERS. `classify_h3_grpc_header_wait_expiry` charges the BACKEND (504 + `ReadWriteTimeout`, so CB / passive health / adaptive concurrency / LB fallback all see it) when the upload completed, when the backend itself halted the upload, when the pump is parked on a backend write/FIN, or when upload progress advanced during the wait; only a wait in which the client produced *nothing* is the neutral client stall. That last case includes the valid zero-message / trailers-only client-streaming RPC. Progress is a step counter snapshotted **before the upload pump is spawned**, so scheduler interleaving cannot erase an early client frame; forwarded byte count is never the test, because such an RPC forwards no body bytes at all.
