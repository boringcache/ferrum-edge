...
The request pump and response relay run concurrently, so client-streaming and H3-to-H2 bidirectional RPCs can receive backend DATA before downstream EOF. The bounded sends race bridge shutdown: a backend early response/error, client reset, deadline, or completed response halts the H3 receive half and drops the H2 body instead of leaving a blocked pump. `max_grpc_recv_size_bytes` and any narrower route body ceiling are counted incrementally on DATA only; an overflow errors the H2 body (RST rather than a truncated clean END_STREAM) and is reported as `RESOURCE_EXHAUSTED`, neutral to backend health.
