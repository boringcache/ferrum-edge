...
node. Last Ready endpoints are retained across a rolling restart while any
trusted NodeWaypoint proxy pod remains in the snapshot, so destination metadata
is not stripped solely because one node's proxy is briefly not Ready; removing
every trusted waypoint pod clears that inventory and withdraws metadata. The
proxy Pod must live in `FERRUM_K8S_CONTROLLER_NAMESPACE`, carry
...
retains the last Ready endpoint while any trusted waypoint pod remains;
