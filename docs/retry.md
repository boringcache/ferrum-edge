...
A client `grpc-timeout` is anchored once at request receipt into an absolute monotonic deadline (independent of whether the `grpc_deadline` plugin is installed). Retries and backoff consume that same Instant, and each attempt forwards a decremented remaining `grpc-timeout` rather than re-arming the original relative header. Retry attempts also reuse the real collected `HeaderMap` so duplicate metadata lines and opaque field values stay byte-identical to attempt 1.
