...
The backup output is directly compatible with `POST /batch` (additive) and `POST /restore` (full replacement). Before replacement, restore takes a best-effort snapshot of the current namespace. Database inserts are chunked into 1,000-record transactions for large-scale imports; if the delete or any insert fails, Ferrum clears the partial state, reapplies the snapshot, and returns `500 Internal Server Error` with a `rollback` field (`completed` / `incomplete` / `unavailable` / `not_needed`). Rollback is `unavailable` when the prior config was already invalid and could not be snapshotted — restore still proceeds so it can repair the namespace, but a later failure then requires manual recovery. `not_needed` means the clear failed atomically (SQL / replica-set MongoDB), so nothing was deleted and the prior config was retained without any re-import. API specs are admin-only metadata that a config rollback cannot restore; a failed restore reports `api_specs_not_restored` and `api_specs_lost` (the dropped specs' `id` + `proxy_id`) so operators can clear the restored resources and re-submit them via `POST /api-specs`.
