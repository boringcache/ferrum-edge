...
**`proxy_name` contract:** `proxy_name` is live display metadata for the stable `proxy_id`, not part of the in-memory registry key and not a Prometheus series-splitting label. A name-only reload therefore keeps counter continuity: subsequent traffic refreshes the stored name from current proxy metadata on each record hit. Pricing changes still create distinct pricing-generation entries (price bits remain in the key). When overlapping generations collapse into one JSON proxy object or Prometheus series, both formats select the same authoritative name — the name from the most recently updated generation, with lexicographic tie-break — independent of map iteration or insertion order.
