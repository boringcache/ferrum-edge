...
# `admin_writes_enabled` is the config-database mutation signal (CRUD/specs/
# batch/restore); managed TLS/ACME stores are independent and are not gated by
...
Creating a new ACME certificate or account record returns `409 Conflict` once `FERRUM_TLS_ACME_MAX_CERTIFICATES` / `FERRUM_TLS_ACME_MAX_ACCOUNTS` is reached; overwrite and delete stay available so capacity can be reclaimed. An authoritative shared ACME store document that already exceeds `FERRUM_TLS_STORE_MAX_DOCUMENT_BYTES` returns `500 Internal Server Error` on every route that tries to read it, including `GET` and `DELETE` — it is unreachable server state rather than a caller payload. A candidate write that would cross the same ceiling returns `413 Payload Too Large` before rename and leaves the previous document intact. Both diagnostics name the ceiling only, never record IDs, domains, paths, or material.
...
Creating orders requires the optional `acme` Cargo feature. Every ACME endpoint must be an absolute `https` URL without userinfo or a fragment. Hosts must use an ordinary DNS name or a canonical IP literal; percent-encoded hosts and ambiguous legacy numeric IPv4 spellings (including integer, abbreviated, mixed-radix, and overflow forms) are rejected before DNS or network activity. Ferrum deliberately applies a fail-closed same-origin transition policy: `newNonce`, `newAccount`, `newOrder`, account, order, authorization, challenge, finalize, certificate, and optional directory endpoints must use the configured directory's normalized host and effective port. Different paths and queries are allowed; a host or port transition is rejected. Persisted credentials must contain that exact normalized directory URL and a same-origin account URL. Legacy credentials containing an embedded `urls` endpoint set are rejected and must be recreated from the configured directory.

Every new ACME connection bypasses the DNS cache and has one 30-second wall-clock budget covering the complete fresh A/AAAA lookup plus all concrete TCP attempts. Ferrum screens the entire answer for non-public addresses before applying an explicit 64-address cardinality limit; mixed or excessive answers are rejected whole and never truncated. Each approved candidate is checked again immediately before its concrete socket is opened, and each attempt receives only the shared deadline's remaining time. This covers stalled resolution, candidate-count amplification, DNS rebinding, and mixed public/private answers without first-answer pinning. The original hostname remains the rustls SNI and certificate-verification identity. Redirects are not followed and any redirect response is rejected.

Every ACME response body, including an issued certificate chain, is limited to 1 MiB while Hyper streams it. An oversized declared `Content-Length` is rejected before collection, and streamed/chunked responses fail as soon as they cross the cap. The identical connection, response, and endpoint boundary applies to manual order creation/finalization, persisted `order_url` values, forced renewal, and automatic renewal. A rejected operator URL returns `400 Bad Request`; a hostile endpoint discovered while processing an ACME response fails the operation without contacting that destination.

`challenge_type` defaults to `http01`; set it to `tls_alpn01` to prepare TLS-ALPN-01 challenge material or `dns01` to prepare DNS-01 TXT material. HTTP-01 responses include public key-authorization values and challenge paths. TLS-ALPN-01 responses include the SNI identifier, token, `acme-tls/1` ALPN protocol, and base64url SHA-256 digest embedded in the validation certificate's critical `acmeIdentifier` extension. DNS-01 responses include the identifier, token, TXT record name, and TXT value. While an order is `pending_challenges`, `ready`, or `processing`, proxy listeners serve HTTP-01 key authorizations at `/.well-known/acme-challenge/{token}` before normal route matching, and TLS listeners return the TLS-ALPN-01 validation certificate when the ClientHello offers only `acme-tls/1` and SNI matches an active challenge. DNS-01 TXT records must be published in authoritative DNS before finalization. Account credentials are persisted in the order store for follow-up ACME calls but are never returned.
...
Creating a new managed record returns `409 Conflict` once `FERRUM_TLS_MANAGED_MAX_RECORDS` is reached; overwrite and delete stay available so capacity can be reclaimed. An authoritative `managed-tls.json` document that already exceeds `FERRUM_TLS_STORE_MAX_DOCUMENT_BYTES` returns `500 Internal Server Error` on every route that tries to read it, including `GET` and `DELETE` — it is unreachable server state rather than a caller payload. A candidate write that would cross the same ceiling returns `413 Payload Too Large` before rename and leaves the previous document intact. Both diagnostics name the ceiling only, never record IDs, paths, or material.
...
Create, rename, and delete are serialized across gateway processes by a **global namespace-registry admission lease** plus one lease per affected name, taken in a fixed order (global first, then names ascending) so concurrent creates, deletes, and a two-name rename cannot deadlock. Each backend requires that exact canonical lease-key sequence before mutating anything; an empty, incomplete, duplicated, reordered, or substituted set is a retryable `503` with nothing applied. The committing transaction then re-verifies every held lease's owner and generation against the datastore's own clock; a lost or stolen lease aborts with the same `503`.
...
1. **Schema rules** — exact per-plugin paths, with `*` wildcards for arbitrary maps and arrays and case/delimiter-insensitive segment matching that also equates compact spellings (`customHeaders`, `custom-headers`, `custom.headers`, `custom_headers`, and `customheaders` are one path). Rules classify a value as a wholesale secret, a structural endpoint URL, a Redis URL, or a librdkafka producer-property map.
...
- **`hbone`** — gateway-to-mesh HBONE support for upstream targets tagged `mesh.hbone=true`. `"supported"` means the gateway can open HTTP/2 CONNECT over SPIFFE mTLS to the sidecar port, `"unsupported"` skips the tunnel after definitive identity/TLS configuration evidence, and `"unknown"` is used when the target is not HBONE-tagged, the gateway has no SVID, the probe has not completed, or a cold probe hit only a transient DNS/connect/timeout or post-TLS HTTP/2 preface/handshake failure. Those transient failures preserve an existing verdict and never permanently poison HBONE during a rolling peer restart.
...
- Manually resolving an incident where the cache is known stale (e.g., backend came back online after a QUIC failure downgraded `h3` or a definitive HBONE identity/TLS configuration failure downgraded `hbone` to `unsupported`).
...
`GET /mesh/policy-denies/recent` is JWT-authenticated and mesh-only. It returns the top-N most recent `mesh_authz` deny events grouped by the `(rule, source, destination, reason, reevaluation)` tuple, for ad-hoc triage. `reevaluation: true` marks an HBONE admission-fence revocation of an already-admitted live tunnel (issue #5042) rather than a request a peer made; it is always present and is part of the grouping key, so the two never merge into one row. The recorder is a process-singleton bounded FIFO ring (`FERRUM_MESH_POLICY_DENY_LOG_CAPACITY`, default `10000`) written only on the `mesh_authz` deny branch — allowed requests never touch it, and a denied request takes the recorder mutex once to push the event (so under a deny storm the cost is on the deny path, not on normal traffic). The endpoint reads a one-shot snapshot, filters to a recent `window`, groups by the 5-tuple, sorts by count descending, and truncates to `limit`. Identity / route / policy metadata only; no request bodies, headers, or credentials. Set `FERRUM_MESH_POLICY_DENY_LOG_CAPACITY=0` to disable the recorder (the endpoint still serves an empty `grouped` array).
...
field with ``unknown field `labels` ``. **v0.9.5 is the first tagged release
with resource labels**; operators and clients should use v0.9.5 or later.
See the [v0.9.5 upgrade guidance](upgrade_guide.md#upgrading-to-095).
