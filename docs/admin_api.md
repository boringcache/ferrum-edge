...
- Manually resolving an incident where the cache is known stale (e.g., backend came back online after a QUIC failure downgraded `h3` or a definitive HBONE identity/TLS/protocol failure downgraded `hbone` to `unsupported`).
