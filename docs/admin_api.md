...
Every new ACME connection bypasses the DNS cache, resolves the complete A/AAAA set through Ferrum's resolver, and rejects the entire result if any address is not public. Each approved candidate is checked again immediately before its concrete socket is opened, and only those concrete addresses are tried. This covers DNS rebinding and mixed public/private answers without first-answer pinning. The original hostname remains the rustls SNI and certificate-verification identity. Redirects are not followed and any redirect response is rejected. The identical boundary applies to manual order creation/finalization, persisted `order_url` values, forced renewal, and automatic renewal. A rejected operator URL returns `400 Bad Request`; a hostile endpoint discovered while processing an ACME response fails the operation without contacting that destination.
