...
**GenericPool create coalescing.** Direct-H2, gRPC, H3, and reqwest pools also coalesce concurrent cache-miss creates per pool key in `GenericPool` (`src/pool/mod.rs`): one creator, N−1 waiters. On success, waiters re-check the cache. On failure, the creator's error is captured as a cloneable `SharedPoolCreateError` (message + coarse kind, no non-cloneable sources or secrets) and broadcast to every waiter for that generation, so a burst against a hard-down backend fails in about one connect budget instead of serially redialing. Creator cancellation does not broadcast a failure — waiters re-elect. There is no durable negative cache; a later independent request starts a new creation generation.
