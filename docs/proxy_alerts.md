...
- Background eviction additionally bounds historical state inside a preserved instance: sliding-window samples and expired cooldown timestamps older than `max(1h, max rule window/cooldown/recovery)` are swept, and terminal `Healthy` recovery rows are dropped. Active/Recovering incidents for proxies that remain in the active set are not TTL-reset.
