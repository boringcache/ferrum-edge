...
- Background eviction additionally bounds historical state inside a preserved instance: every sweep reapplies the published proxy-generation ownership map so an isolated old-generation write that lands just after commit-time retention is removed on the next cadence; sliding-window samples and expired cooldown timestamps older than `max(1h, max rule window/cooldown/recovery)` are swept; and terminal `Healthy` recovery rows are dropped. Active/Recovering incidents for the currently published generation remain owned by retention and are not TTL-reset.
