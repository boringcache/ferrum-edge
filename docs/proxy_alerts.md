...
- **Duplicate delivery**: alert delivery is at-least-once at the endpoint. Transport timeouts, connection errors after the request was written, and cancellation after bytes left the process are all reported as failure or abandonment even though the endpoint may have received and acted on the send — so a retry, or a compensating Trigger after an uncertain Resolve, can deliver the same alert twice. **Webhook and email consumers must be idempotent or duplicate-tolerant**; deduplicate on rule + proxy + event action + fired-at rather than on delivery count, and read `abandoned_total` as "delivery state unknown", not "not delivered". Full contract in `docs/notifications.md`.
