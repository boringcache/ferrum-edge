...
- Plugin state (sliding-window counters, cooldown timestamps, recovery state machines) is per-instance. **Full rebuilds** (plugin definition changes, global rebuilds, or cascade-deleted proxy-group instances) construct a fresh instance and therefore reset state. **Incremental plugin-cache updates** deliberately preserve unchanged global and proxy-group instances so live counters survive unrelated proxy edits; after those commits, preserved instances retire cooldown/recovery/window rows for proxies that left their active set (deleted, renamed, or removed from a proxy-group). Re-creating the same proxy ID after removal therefore cannot inherit a prior cooldown, Active incident, or Recovering incident. Proxy-scoped instances are dropped with their proxy and do not need that retention pass.
