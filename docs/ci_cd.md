...
Published gateway binaries use raw asset names such as `ferrum-edge-linux-x86_64` with adjacent `.sha256` sidecars. Pin an explicit tag (`latest` for the current prerelease stream, or `v0.2.0` for a stable semver release). Do not rely on GitHub's `/releases/latest` redirect or the `releases/latest` API endpoint while the only published tag is a prerelease.
