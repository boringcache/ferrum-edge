...
- **Without a backend-observed `Content-Length`**: unknown-length bodies keep streaming. Every collector and streaming adapter uses the **effective route ceiling** (the strictest active bound of per-route `response_size_limiting` and `FERRUM_MAX_RESPONSE_BODY_SIZE_BYTES`), not the global limit alone. The size-limited adapter yields an error once the running count exceeds that ceiling; clients that already observed the backend status will see a truncated/reset body rather than a rewritten JSON `502`. Adapter selection uses the canonical length captured before `after_proxy`, so a hook-authored `Content-Length` cannot skip this path.
- **`response_size_limiting` plugin**: publishes a per-route ceiling that always bounds unknown-length streaming responses frame by frame through the core adapters above. `require_buffered_check` controls only whether the complete final post-transform size must be proven before response headers are committed — it does not decide whether the route ceiling applies. See [`response_size_limiting`](plugins.md#response_size_limiting) for operator-facing configuration; the plugin module doc in `src/plugins/response_size_limiting.rs` is the authoritative enforcement reference.
...
Proxy request/response size limits do **not** cover TLS control-plane material. Ferrum enforces one authoritative ceiling on every certificate, private-key, CA bundle, CRL, OCSP, and JWKS material source before whole-value buffering:

| Variable | Type | Default | Hard maximum | Description |
|----------|------|---------|--------------|-------------|
| `FERRUM_TLS_MAX_MATERIAL_SIZE_BYTES` | `usize` | `4194304` (4 MiB) | `4194304` (4 MiB) | Applies to local files, inline PEM, compiled external-secret provider values (`vault://` / `aws://` / `azure://` / `gcp://`), Kubernetes Secret bytes, managed-store records, and ACME-store records. |
...
- **Stability.** Two independent open/read cycles must observe matching file identity and byte-identical content, and those probes are separated by a **20ms settle interval** (the same delay used between unstable retries). Back-to-back probes can both complete inside one truncate/write or delete/recreate window; the settle interval is what makes that window visible. Instability retries a bounded number of times (at most five attempts), then fails closed. Worst-case synchronous sleep is 180ms plus the capped I/O — a cold-path cost (startup, SIGHUP, mesh file load), not a per-request or per-byte delay. Atomic replace (write-temp, fsync, rename) remains the supported publication method.
