...
- **Without a backend-observed `Content-Length`**: unknown-length bodies keep streaming. The size-limited adapter yields an error once the running count exceeds the limit; clients that already observed the backend status will see a truncated/reset body rather than a rewritten JSON `502`. Adapter selection uses the canonical length captured before `after_proxy`, so a hook-authored `Content-Length` cannot skip this path.
