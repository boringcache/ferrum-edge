...
- **Admin surface.** A logical create ceiling is caller-facing and returns `409 Conflict` (overwrite and delete remain available). A whole-document byte refusal is *not*: `GET`, list, and `DELETE` raise it too, from shared state the caller never sent, so it returns `500 Internal Server Error` alongside an unreadable or unparseable document rather than `413`. Only bounded *admitted material*, decided against the request body, keeps `413`.
