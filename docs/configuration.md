...
| `FERRUM_LOG_BUFFER_BYTES` | No | `33554432` | Per-sink aggregate byte budget. Admission provisionally reserves `FERRUM_LOG_MAX_RECORD_BYTES` before serialization, then shrinks that reservation to the actual serialized length until write completion. Clamped between `FERRUM_LOG_MAX_RECORD_BYTES` and 1 GiB |
