...
| `FERRUM_AUDIT_RETENTION_MAX_ROWS` | No | `100000` | Per-namespace row cap for durable `audit_events`. Each namespace keeps the newest N events ordered by deterministic `(ts, id)` and prunes older rows with indexed, chunked deletes. Valid range: `1`–`10000000`. One namespace never prunes another |
