...
|---|---|---|---|---|---|---|
| `AuthorizationPolicy` | Yes (incl. `targetRefs` → Service/Gateway/GatewayClass, same-namespace; `targetRefs` → ServiceEntry rejected fail-closed) | Yes (`ISTIO_CRDS`) | Yes | Yes (`MeshPoliciesCarrier` + `WaypointGatewayClassCarrier`) | Yes (incl. `notMethods`/`notPaths`/`notHosts`/`notPorts` + bounded port wildcards + `targetRefs` waypoint attachment) | Yes |
