...
| `FERRUM_AUDIT_RETENTION_MAX_ROWS` | No | — | Optional per-namespace row cap for durable `audit_events`. When set, each namespace keeps the newest N events ordered by deterministic `(ts, id)` and prunes older rows with indexed, chunked deletes. Valid range: `1`–`10000000`. Unset disables the cap. One namespace never prunes another |
