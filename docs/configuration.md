...
| `FERRUM_K8S_WATCH_GATEWAY_API_CRDS` | No | `true` | Enable watching Gateway API CRDs (`gateway.networking.k8s.io`: GatewayClass/Gateway/HTTPRoute/GRPCRoute/TCPRoute/TLSRoute/ReferenceGrant/BackendTLSPolicy, plus core Secret/ConfigMap/Service/EndpointSlice material for certificate and backend TLS refs) in the CP K8s controller. Only effective when `FERRUM_K8S_CONTROLLER_ENABLED=true` |
