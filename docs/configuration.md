...
| `FERRUM_LOG_BUFFER_CAPACITY` | No | `4096` | Per-sink maximum admitted records (stdout/access logs share one sink; stderr is separate). Clamped to 1–65,536. Admission is lossy and non-blocking when full |
| `FERRUM_LOG_BUFFER_BYTES` | No | `33554432` | Per-sink aggregate byte budget reserved by admitted records. Clamped between `FERRUM_LOG_MAX_RECORD_BYTES` and 1 GiB |
