...
| `FERRUM_DB_FAILOVER_ALLOW_WRITES` | No | `false` | Allow Admin API mutations while the active pool points at a `FERRUM_DB_FAILOVER_URLS` entry. Default `false` fails closed (503) so failover-window writes cannot be silently erased when the primary recovers and republishes. Set `true` only for synchronously replicated multi-primary topologies; accepted failover writes then fence automatic primary failback until process restart after reconciliation. Polling and reads stay available either way. Topology/divergence signals use redacted URLs only |
