...
| `FERRUM_ADMIN_SPEC_EXTERNAL_REFS_FILE_ROOT` | No | — | Absolute filesystem jail for `file:` / relative external `$ref` targets. Empty disables file refs |
| `FERRUM_ADMIN_SPEC_EXTERNAL_REFS_ALLOWED_ORIGINS` | No | — | Comma-separated HTTPS origins (`https://host[:port]`) allowed for network external `$ref`s |
| `FERRUM_ADMIN_SPEC_EXTERNAL_REFS_ALLOW_HTTP_ORIGINS` | No | — | Comma-separated explicit HTTP origins permitted only when listed (fixture/dev). Empty disables plain HTTP |
