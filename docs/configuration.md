...
| `FERRUM_MESH_NODE_WAYPOINT_IN_NETNS_LISTENERS_ENABLED` | No | `false` | Enable per-pod-netns outbound capture listeners for node-waypoint (ztunnel-style). When `true`, the mesh proxy opens a `127.0.0.1:15001` listener inside each enrolled pod's network namespace to capture that pod's outbound traffic. Default off |
| `FERRUM_MESH_NODE_WAYPOINT_POD_REGISTRY_DIR` | No | `/run/ferrum/node-waypoint-pods` | Directory where the node-agent publishes the enrolled-pod registry (one file per pod, name=pod_uid, contents=cgroup path) for the mesh proxy's in-netns capture listeners to consume |
