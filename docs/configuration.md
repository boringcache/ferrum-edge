...
Translation notes: Istio `AuthorizationPolicy` resources preserve Istio's action semantics. An `ALLOW` policy with no `rules` is treated as allow-nothing for the selected workload, so it creates a mesh authorization rule that never matches instead of accidentally broadening access. `DENY` and `AUDIT` policies with no `rules` remain no-ops. `spec.targetRefs` is supported for waypoint attachment: typed `Service` (core), waypoint `Gateway` / `GatewayClass` (`gateway.networking.k8s.io`), and `ServiceEntry` (`networking.istio.io`) references resolve against the translated inventory with selector/`targetRefs` exclusivity, same-namespace or ReferenceGrant-authorized cross-namespace boundaries, exact GatewayClass matching (`istio-waypoint` never attaches to `ferrum-waypoint`), exact destination Service/ServiceEntry membership (shared selector labels alone never broaden), and fail-closed rejection when a target is missing, deleted, empty, or unsupported — including native/file/xDS config validation that rejects `targetRefs` on PeerAuthentication/RequestAuthentication/Telemetry/ProxyConfig until those resources gain their own end-to-end attachment implementation. Ferrum never rewrites a targeted policy into broader selector/namespace scope. Accepted policies report resolved `targetRefs` in `status.ferrum.translation`.
...
|---|---|---|---|---|---|---|
| `AuthorizationPolicy` | Yes (incl. `targetRefs` → Service/Gateway/GatewayClass/ServiceEntry) | Yes (`ISTIO_CRDS`) | Yes | Yes (`MeshPoliciesCarrier`) | Yes (incl. `notMethods`/`notPaths`/`notHosts`/`notPorts` + bounded port wildcards + `targetRefs` waypoint attachment) | Yes |
