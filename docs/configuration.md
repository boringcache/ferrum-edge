...
| `FERRUM_ACME_RENEWAL_LEASE_TTL_SECONDS` | No | `900` | Lifetime of the shared per-certificate ACME renewal claim recorded in `tls-leases.json` under `FERRUM_TLS_MANAGED_STORE_PATH`. Exactly one instance holds a certificate's claim at a time, so replicas sharing the store directory never renew the same certificate concurrently. A crashed holder's claim expires after this long and another instance takes over, so the value must comfortably exceed one order/finalize cycle including DNS-01 propagation. Clamped to 60 seconds minimum and 2 hours maximum |
| `FERRUM_TLS_STORE_LOCK_TIMEOUT_SECONDS` | No | `10` | How long a managed TLS / ACME store mutation waits for the cross-instance advisory lock on the shared store directory before failing closed. A timeout is reported as a store write failure and the mutation is not applied. Clamped to 1 second minimum and 120 seconds maximum |
| `FERRUM_TLS_STORE_INSTANCE_ID` | No | generated per process | Identity this instance stamps on shared managed TLS / ACME renewal claims. Pin it to a stable per-replica value (for example the pod name) so a restarted replica reclaims its own live claim instead of waiting for it to expire; leave unset for a random per-process identity. Only `A-Z a-z 0-9 - _ . :` are kept and the value is truncated to 128 characters |
