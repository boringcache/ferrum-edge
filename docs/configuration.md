...
| `FERRUM_ADMIN_JWT_MAX_TTL` | No | `3600` | Maximum accepted token lifetime for externally minted Admin API JWTs. Both the nominal lifetime (`exp - iat`) and the remaining lifetime against verifier time (`exp - now`) must not exceed this value, and `iat` later than verifier time plus clock-skew leeway is rejected, so future-shifted timestamps cannot extend real validity. `0` intentionally disables the lifetime cap |
