...
| `FERRUM_ADMIN_AUDIT_FALLBACK_PATH` | No | `./ferrum-admin-audit` | Directory for the bounded local audit fallback file (`admin-audit-fallback.json`) used when a security-sensitive backup audit cannot be written to the primary `audit_events` store (including cached-backup paths while the primary database is unavailable). Capacity is capped; events never contain backup payload bytes or secrets. Distinct from the general mutation pipeline in #2421 |
