...
| `FERRUM_ADMIN_HTTPS_PORT` | No | `9443` | Admin API HTTPS port. Set to `0` to disable the admin HTTPS listener in every serving mode, even when `FERRUM_ADMIN_TLS_CERT_PATH`/`FERRUM_ADMIN_TLS_KEY_PATH` are still configured — the certificate and key are not loaded, no TLS reload watcher is started, and no socket is bound. The only exception is not reachable from configuration: an in-process embedder that passes file mode an already-bound admin HTTPS socket through `ServeOptions` keeps that socket (a pre-bound listener wins over the port setting), and the `ferrum-edge` binary never passes one |
