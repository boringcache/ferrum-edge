...
| `FERRUM_ADMIN_AUDIT_ENABLED` | No | `false` | Enable Admin API audit events. Successful mutations still use the bounded best-effort queue (distinct from retention #2996 and delivery-loss hardening #2421). `GET /backup` additionally requires a durable security record before releasing unredacted configuration: it awaits a synchronous `audit_events` insert when a database backend is available, otherwise appends to the bounded local fallback under `FERRUM_ADMIN_AUDIT_FALLBACK_PATH`, and returns `503` without exporting if neither sink admits the event |
| `FERRUM_ADMIN_AUDIT_FALLBACK_PATH` | No | `./ferrum-admin-audit` | Directory for the bounded local audit fallback file (`admin-audit-fallback.json`) used when a security-sensitive backup audit cannot be written to the primary `audit_events` store (including cached-backup paths while the primary database is unavailable). Must be a non-symlink directory; data/lock targets must be non-symlink regular files. On Unix the directory/file permissions are enforced owner-only (`0700`/`0600`) and publication uses a collision-resistant temp file plus directory sync under a cross-process lock acquired without waiting (Unix non-blocking exclusive `flock`; Windows immediate share denial). In-process exclusion likewise uses non-blocking acquisition; lock contention or poisoning fails closed so `GET /backup` cannot hang on a wedged holder. Capacity is capped; events never contain backup payload bytes or secrets. Distinct from the general mutation pipeline in #2421 |
