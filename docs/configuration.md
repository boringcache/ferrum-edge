...
| `FERRUM_ADMIN_AUDIT_ENABLED` | No | `false` | Enable database-backed audit events for successful Admin API mutations. When a durable spool is configured (the default), the mutation response is acknowledged only after the audit event is fsynced to `FERRUM_ADMIN_AUDIT_SPOOL_DIR`; insertion into `audit_events` is asynchronous with bounded retry and idempotent replay on the event id. Delivery is **at-least-once with a stable id**, and every backend insert is idempotent on that id, so replayed events converge to exactly one durable row. Distinct from retention (#2996) |
| `FERRUM_ADMIN_AUDIT_SPOOL_DIR` | No | `/var/lib/ferrum/audit-spool` | Directory holding the durable audit spool: `pending/` (fsynced, awaiting insertion), `failed/` (retained unrecoverable records for operator remediation), `tmp/` (reconciled at startup). Must be writable and should live on a persistent volume, otherwise a restart loses the pending backlog. Set to an empty value to disable durable delivery entirely and fall back to a bounded in-memory queue — a committed mutation can then lose its audit event across a crash. The gateway warns loudly at startup and authenticated `/health` reports `status: degraded` with `audit_pipeline.durability: memory` for as long as that mode is in force. An empty value is rejected when `FERRUM_ADMIN_AUDIT_UNAVAILABLE_POLICY=fail_closed`. Only used when `FERRUM_ADMIN_AUDIT_ENABLED=true` |
| `FERRUM_ADMIN_AUDIT_UNAVAILABLE_POLICY` | No | `fail_open` | What the Admin API does when audit events cannot be durably recorded. `fail_open` — mutations proceed; the failure is counted, logged with a static reason (no path or OS error), and reported through authenticated `/health` and `/metrics`. `fail_closed` — subsequent audited admin mutations are refused with `503` (and `audit_unavailable_reason`) until the pipeline recovers; the gate is evaluated **before** the write, so a mutation that cannot be audited is never performed. `fail_closed` requires a non-empty `FERRUM_ADMIN_AUDIT_SPOOL_DIR` and makes an unusable spool a startup failure. Accepted values: `fail_open`, `fail_closed` |
...
| `FERRUM_ADMIN_AUDIT_SPOOL_MAX_RECORDS` | No | `100000` | Ceiling on durable pending audit records. Once reached, further durable handoffs fail (a drop under `fail_open`, refused mutations under `fail_closed`) rather than filling the volume. Valid range: `1`–`10000000` |
| `FERRUM_ADMIN_AUDIT_RETAINED_MAX_RECORDS` | No | `10000` | Ceiling on retained unrecoverable audit records under `<spool>/failed/`. When full, the newest unrecoverable record is discarded so the oldest evidence is preserved, and `ferrum_admin_audit_events_dropped_total{reason="retained_capacity"}` increments. Valid range: `1`–`1000000` |
| `FERRUM_ADMIN_AUDIT_MAX_DELIVERY_ATTEMPTS` | No | `10` | Bounded delivery-attempt budget per audit event, counted across process restarts. Retries use exponential backoff from 250 ms capped at 30 s. On exhaustion the record is moved to `<spool>/failed/` for operator remediation instead of being dropped. Valid range: `1`–`1000` |
...
|---|---|---|---|---|---|---|
| `AuthorizationPolicy` | Yes | Yes (`ISTIO_CRDS`) | Yes | Yes (`MeshPoliciesCarrier`) | Yes (incl. `notMethods`/`notPaths`/`notHosts`/`notPorts` + bounded port wildcards) | Yes |
