...
| `FERRUM_LOG_DELIVERY_MAX_TASKS` | No | `4096` | Aggregate admission budget for deferred terminal, mirror, and deadline-cleanup Tokio tasks (the pre-queue registry layer). Clamped to 1–1,048,576. When exhausted, new admissions are rejected immediately (non-blocking), counted on `ferrum_observability_delivery_rejected_tasks_total`, and logged with a rate-limited warning on the caller thread; permits release as tasks complete or are cancelled |
