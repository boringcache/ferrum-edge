...
| `FERRUM_LOG_DELIVERY_MAX_TASKS` | No | `4096` | Aggregate admission budget for deferred terminal, mirror, and deadline-cleanup Tokio tasks (the pre-queue registry layer). Clamped to 1–1,048,576. When exhausted, new admissions are rejected immediately (non-blocking), counted on `ferrum_observability_delivery_rejected_tasks_total` and the budget-specific `ferrum_observability_delivery_capacity_rejected_tasks_total`, and logged with a caller-thread warning that is sampled and bounded to at most one line per five seconds per process; permits release as tasks complete or are cancelled |
