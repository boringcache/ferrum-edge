...
| `FERRUM_DB_CONFIG_BACKUP_PATH` | No | — | Path to externally provided JSON config backup. Used as startup fallback ONLY when every configured database URL fails with a transient connectivity/resource/connect-timeout error. A non-transient schema/auth/config/query failure fails startup instead of bootstrapping from the backup, so a broken primary is never masked by stale on-disk config. |
...
| `FERRUM_DB_READ_REPLICA_URL` | No | — | SQL read replica URL for eligible admin-only reads. Runtime config polling and writes always use the active primary/failover pool. The configured read replica is eligible only while the configured primary topology is active; Ferrum suppresses it during failover and makes it eligible again after primary failback. MongoDB read preferences are ignored by Ferrum's config store |
