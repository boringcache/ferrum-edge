...
| `FERRUM_TRUSTED_PROXIES` | No | — | Comma-separated trusted proxy CIDRs/IPs. Ferrum trusts `X-Forwarded-For` from these direct peers for client-IP resolution and the rightmost `X-Forwarded-Proto` value when it is `http` or `https` for the original browser-facing scheme. That accepted scheme drives secure-cookie and authentication URL decisions and the `X-Forwarded-Proto`/RFC 7239 `Forwarded` metadata regenerated for backends. List only proxies that overwrite or safely append these headers. |
