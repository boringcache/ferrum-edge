...
| `FERRUM_ADMIN_AUDIT_SPOOL_MAX_RECORDS` | No | `100000` | Ceiling on this process generation's durable audit records (prepared + pending combined). Once reached, further durable handoffs fail (refused mutations under `fail_closed`, an accounted unaudited mutation under `fail_open`) rather than filling the volume. A counter that reports "full" is always confirmed by a real directory scan before a mutation's evidence is refused. Valid range: `1`–`10000000` |
