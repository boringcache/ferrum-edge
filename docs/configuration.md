...
| `FERRUM_TRUSTED_PROXIES` | No | — | Comma-separated trusted proxy CIDRs/IPs. Ferrum trusts `X-Forwarded-For` from these direct peers for client-IP resolution and `X-Forwarded-Proto: https` for the original browser-facing scheme. List only proxies that overwrite or safely append these headers. |
