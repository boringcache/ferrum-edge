...
| `FERRUM_DP_CP_GRPC_TOKEN_FILE` | No | — | **DP / mesh / xDS client side.** Path to an externally issued bearer token presented instead of minting one, so no signing key exists on the node at all. Re-read on every connection attempt, so a rotating projected token needs no restart. The issuing authority — not this node — sets `ns`, `iss`, `exp`, and (for native `MeshSubscribe`) the local-mesh `aud`. Cross-cluster mesh remote discovery still self-mints and needs `FERRUM_CP_DP_GRPC_JWT_SECRET` |
