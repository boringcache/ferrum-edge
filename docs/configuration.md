...
ACME outbound traffic always uses Ferrum's controlled HTTPS connector. All RFC 8555 endpoint URLs must remain on the configured directory's normalized host and effective port; credential-directory drift and legacy credential endpoint sets fail closed. Each new connection uses a fresh complete DNS answer, requires every candidate to satisfy the hard public-only backend egress policy, rechecks the selected candidate at dial time, and retains the hostname for certificate verification. Redirect responses are rejected. Manual issuance, stored order completion, forced renewal, and automatic renewal share this boundary.
