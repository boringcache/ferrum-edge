...
| `FERRUM_ADMIN_HTTPS_PORT` | No | `9443` | Admin API HTTPS port. Set to `0` to disable the admin HTTPS listener in every serving mode, even when `FERRUM_ADMIN_TLS_CERT_PATH`/`FERRUM_ADMIN_TLS_KEY_PATH` are still configured — the certificate and key are not loaded, no TLS reload watcher is started, and no socket is bound |
