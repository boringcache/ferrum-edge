...
| BPF UDP relay authorization maps and gate | `FERRUM_UDP_REPLY_SOURCES`, `FERRUM_UDP_REPLY_SOURCES6`, `FERRUM_UDP_RELAY_CGROUPS`, `FERRUM_UDP_REPLY_SOURCE_GATE` | Exact `(source address, source port)` pairs a SERVING NodeWaypoint UDP/DTLS listener may reply from, the NodeWaypoint relay pod's own cgroup-v2 ids (the non-forgeable sender proof both UDP arms require FIRST — the node-agent resolves them from this node's cgroup hierarchy using the pod UID the proxy publishes, and refuses any id belonging to an enrolled workload), plus one shared BPF-visible gate both tc UDP classifier families consult before any of the three. The node-agent retracts `<pod registry dir>/.udp-reply-src/applied`, disables the gate, replaces the relay-cgroup map and then both source families while every entry is inert, revalidates the exact bounded manifest, enables the gate, and only then writes `applied`. After successful fencing, any unlink, scan, remove, insert, partial-family, revalidation, or acknowledgement-write failure leaves the gate closed and retryable; stale keys left by a failed removal are inert, not proof of a narrower coherent set. If the gate cannot be disabled, the node-agent reports a hard fencing failure and neither mutates nor acknowledges a successor. All three maps are required for NodeWaypoint readiness, including an ACTIVE generation with zero ClusterIP sources (the bound headless/VIP-less listener: relay cgroups are written, the source maps stay empty, the gate opens) and an INACTIVE withdrawal (both maps cleared, gate closed). |
...
traffic remains pass-through. Every UDP admission additionally requires a
NON-FORGEABLE sender proof the TCP arm does not have (issues #3956, #3957):
...
`FERRUM_UDP_REPLY_SOURCES6` (the source-pinned reply, whose address on the
steered Service path is a Service ClusterIP). The relay auth mark is still
required in every case, and those maps hold exact addresses with exact ports,
never a CIDR or a service range. All three maps sit behind the shared
