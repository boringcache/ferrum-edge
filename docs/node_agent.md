...
- **Valid attachments.** The runtime supplies `cni.dev/valid-attachments` as `(containerID, ifname)` tuples. Ferrum requires the reserved field even when the list is empty, bounds ingestion (attachment count, field length, safe character set), and rejects omitted, misspelled, hostile, or path-like input before reconciliation.
