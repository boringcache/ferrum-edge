...
| BPF UDP reply-source maps | `FERRUM_UDP_REPLY_SOURCES`, `FERRUM_UDP_REPLY_SOURCES6` | Exact `(source address, source port)` pairs a SERVING NodeWaypoint UDP/DTLS listener may reply to an enrolled pod from (issue #3286). A NodeWaypoint UDP/DTLS reply is source-PINNED to the local address the client addressed, which on the steered Service path is the Service ClusterIP and therefore never a configured node IP — so without these maps the relay's marked, correctly scoped reply is dropped by the pod-veth guard. Consulted by the tc **UDP arm only** and only as the SOURCE half of the existing proof: the NodeWaypoint inbound auth mark is still required, and TCP does not read them. Written solely by the node-agent, reconciled every 250 ms from the mesh proxy's published generation at `<pod registry dir>/.udp-reply-src/desired` — the proxy's bpffs mount is read-only by design — as a whole-set replacement that removes stale entries before inserting new ones. The generation is one bounded, strictly parsed, atomically renamed manifest, so a partially rewritten set can never be observed as a complete one. The node-agent retracts `<pod registry dir>/.udp-reply-src/applied` before touching a map, applies both families, re-reads the exact manifest, and writes `applied` only if that generation is still current; the proxy binds that proof back to its own canonical manifest before installing steering. A generation naming an enrolled pod address, an over-bound set, a malformed or unreadable manifest, either required map absent (including for an empty withdrawal), any scan/remove/insert error, or any partial-family result refuses the WHOLE generation (never a narrowed subset) and produces no acknowledgement. |
