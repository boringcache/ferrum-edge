...
- **Ownership.** Successful CNI ADD records Ferrum ownership of that attachment in memory and, when the CNI listener is enabled, in a crash-safe durable file beside the configured CNI socket (`cni-owned-attachments.v1` under the socket parent, typically `/var/run/ferrum/`). GC removes only Ferrum-owned attachments absent from the valid set. Watcher-only enrollments and other node-agent generations are never swept. On node-agent restart, well-formed durable records are rehydrated before GC can act; malformed, oversized, truncated, symlinked, or path-like durable state fails closed (GC errors without sweeping) and never echoes raw hostile identifiers. ADD that cannot durably persist an ownership claim returns an error and does not leave a false in-memory GC claim (kubelet may retry). Durable ownership is cleared only after removal-blocking cleanup succeeds; a durable-update failure retains ownership for retry and must not report success for that clear.
- **Idempotency / partial failure.** Repeat GC with the same valid set is a no-op success once every stale Ferrum-owned attachment is fully cleaned. When BPF/rule/map teardown leaves removal-blocking state for a stale candidate, GC reports an error, retains Ferrum ownership (including durable state), and a later GC re-drives the pending-removal retry path until cleanup is complete. Rehydrated ownership after a process crash identifies Ferrum-owned stale attachments for GC; full BPF map cleanup still depends on live or retained attachment snapshots when those keys are available.
