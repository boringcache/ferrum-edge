...
> **Helm chart users:** the allowlist-only `0.0.0.0` behavior above applies to the **raw env** contract. The `charts/ferrum-mesh` node-agent DaemonSet always renders `FERRUM_ADMIN_BIND_ADDRESS` from `nodeAgent.admin.bindAddress` (default `127.0.0.1`) whenever node-agent admin is enabled, so the bind is always an explicit signal and the allowlist-only branch never engages through the chart. To expose `/metrics` for cluster scraping via Helm, set **`nodeAgent.admin.bindAddress=0.0.0.0`** (and `nodeAgent.admin.allowedCidrs` / metrics auth to the scrape sources); setting only the allowlist leaves a loopback listener and scraping fails.

When ambient NodeWaypoint and the node agent run on the same host-network nodes, their admin listeners must use distinct `FERRUM_ADMIN_HTTP_PORT` / `FERRUM_ADMIN_HTTPS_PORT` values or one listener must be disabled with port `0`. The Helm chart rejects equal host-network admin HTTP ports, and equal HTTPS ports only when both sides would actually bind HTTPS (complete admin TLS). The binary also treats an enabled admin-listener bind failure as fatal startup rather than continuing to report ready.
