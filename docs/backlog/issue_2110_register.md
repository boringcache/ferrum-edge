...
| SPIFFE Workload API JWT-SVID delegated mint (SPIRE backend) | [#3617](https://github.com/ferrum-edge/ferrum-edge/issues/3617) | Implemented for `FERRUM_MESH_CA_BACKEND=internal` (mint / bundles / validate, ES256, configured signing material, provable rotation overlap) behind `FERRUM_MESH_WORKLOAD_API_ENABLED`. `spire` now serves bundles + validate from the agent's own `FetchJWTBundles` stream; delegated `FetchJWTSVID` stays fail-closed `UNIMPLEMENTED` there — a terminal boundary needing SPIRE's delegated-identity/admin API, since proxying the agent RPC would substitute Ferrum's own SPIFFE ID. Empty JWT bundle streams remain non-conformant in every posture |
...
| Gateway API port-aware route representation | [#3612](https://github.com/ferrum-edge/ferrum-edge/issues/3612) | |
