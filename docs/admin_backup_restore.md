...
- **Canonical JWT credentials**: Consumer `jwt` entries in input, backup, and restore contain exactly one `secret` string (32-4096 characters) for HS256. Algorithm selectors, public keys, and JWKS fields are not Consumer credential forms and are rejected; RSA/EC/JWKS verification belongs to the separate `jwks_auth` plugin configuration.
