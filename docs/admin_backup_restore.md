...
- **Rollback replays that exact raw state.** The compensating plugin/association batches remain namespace-serialized but bypass normal mTLS DNS admission only while replaying the captured snapshot. This prevents a pre-existing case ambiguity from being deleted and then blocking restoration. Normal batch and restore imports always use full admission.
