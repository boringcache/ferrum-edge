...
- **Rollback replays that exact raw state.** One persistent namespace-scoped datastore guard spans the compensating clear and every Consumer, proxy, plugin, and association replay batch. Only the owning rollback may borrow that guard; normal DNS-sensitive writers and a non-owning replay fail closed until it is released. On MongoDB, the owner also pins the exact connection generation that acquired the guard, so reconnect/failover cannot move a later replay batch to a different bundle. The guarded replay bypasses normal mTLS DNS admission only for the captured snapshot, preventing a pre-existing case ambiguity from being deleted and then blocking restoration. Normal batch and restore imports always use full admission.
