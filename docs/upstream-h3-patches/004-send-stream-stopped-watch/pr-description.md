...
- add `quic::SendStreamStopped` so transports can expose peer `STOP_SENDING` as a `&self` + `'static` return-position `impl Future`
...
- do not box a trait-object future per call
