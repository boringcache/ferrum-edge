...
event carries `action` as the **effective** outcome (`blocked`, `monitored`, or
`disabled`, matching `waf.action` metadata) and `rule_action` as the configured
rule action (`enforce`, `monitor`, or `disabled`).
