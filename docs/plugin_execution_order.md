...
An RPC deadline discovered while buffering an upload is finalized through this same rejection path even when it occurs before authentication, authorization, `before_proxy`, or backend dispatch. Non-replacing decorators and committed observers run exactly once against the canonical status-4 result, gRPC-Web translation happens only after those headers are finalized, and the rejection is logged before the frontend returns. Response-replacing hooks cannot overwrite an already-selected terminal deadline.
...
`on_response_committed` is buffered-only and observe-only. It receives mutable request context plus the final client-visible status, headers, and body after every `on_final_response_body` hook and any rejection replacement. It cannot mutate or reject the response. Exporters use it for record construction while retaining fail-closed sink admission in an earlier rejecting hook. Each opted-in hook runs exactly once. If the gRPC deadline expires inside one committed hook, the gateway first replaces the outcome with the terminal `DEADLINE_EXCEEDED` representation, then invokes every remaining committed hook once against that replacement so later audit and cleanup exporters cannot lose the transaction. The proxy uses a precomputed per-protocol committed-hook list, so normal buffered requests do not rescan the full plugin chain.
