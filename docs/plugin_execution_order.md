...
`soap_ws_security` keeps AuthN-band priority 1500. A configuration that establishes a principal (`username_token`, `x509_signature`, or `saml`) is an auth plugin: it opts into `requires_request_body_before_authenticate` and validates the SOAP message in the `authenticate` phase, publishing `authenticated_identity` and a namespace-correct `identified_consumer` so `access_control` (2000) and consumer-scoped `rate_limiting` (2900) evaluate the SOAP principal rather than the source IP. A timestamp-only configuration establishes no principal, is not an auth plugin, and keeps validating in `before_proxy` after request-body buffering — the two phases are selected by configuration and never both run. Both forms also run `on_final_request_body` before backend dispatch to refuse a message whose bytes changed after validation. Composition admission rejects an identity-establishing instance on a proxy with `auth_mode: multi` alongside another auth plugin (multi-auth would skip or override the message gate) and alongside `compression` with `decompress_request` (authentication precedes body normalization).
