...
6. **`ai_semantic_cache` (2996)** runs after guardrails and after route-dispatch plugins (`ai_stream_router`, `mcp_gateway`, `a2a_gateway`, `mesh_route_dispatch`), so exact and semantic cache hits observe the accepted backend-visible prompt and the effective destination/provider identity before short-circuiting outbound provider dispatch.
