...
For a deadline-bound buffered backend response, the core establishes a response-header provenance boundary before the first `after_proxy` hook. Backend fields begin outside the terminal-response set; each completed trusted hook records only fields it added, changed, removed, or explicitly owns (for example, the configured correlation-header name). If a later normalizer, body transform, or committed observer exhausts the deadline, the terminal response is rebuilt from that gateway-owned set rather than from a header-name allowlist. Backend cookies, credentials, representation metadata, arbitrary fields, and spoofed decorator names are discarded. Cache state, discarded-representation metadata, transport framing, and prior gRPC terminal fields remain owned by the terminal response and are removed even when a completed gateway hook wrote them; only the narrow `Vary: Origin` compatibility contract can cross independently. Protocol framing is then regenerated and deterministic initial-response policies are reapplied. A later generic sanitation pass never erases another completed trusted decorator, while a pending or cancelled hook cannot publish partial mutations.
