...
The plugin is also active on three response phases, which is where result policy and irreversible SSE visibility are decided. `after_proxy` refuses a `tools/call` whose response headers cannot yield a bounded, inspectable JSON representation for `validation.validate_tool_results`; `on_final_response_body` runs the authoritative fail-closed body enforcement and then makes the single aggregate-SSE reservation; and `on_response_committed` publishes that reservation only when the committed response is still the empty `202` it reserved against. All three act only on requests the same `mcp_gateway` instance admitted in `before_proxy`, so instances with disjoint `endpoint.path` scopes on one proxy never apply their response policies to each other's traffic.
