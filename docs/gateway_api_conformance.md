...
| Selectorless/headless Services | Yes | With pod discovery enabled, backends resolve ready EndpointSlice addresses directly; a named Service `targetPort` resolves against EndpointSlice port names, but the `backendRef.port` itself is numeric-only — see [backendRef port and zero-weight semantics](#backendref-port-and-zero-weight-semantics). A headless Service (`spec.clusterIP: None`) with no ready EndpointSlice addresses falls back to Cluster DNS of the Service on the numeric `targetPort`, not the Service port: headless DNS returns pod IPs, kube-proxy does not DNAT them, and dialing the Service port is a `connection_failure`. ClusterIP Services keep the Service-port DNS fallback because kube-proxy maps `ClusterIP:servicePort` onto `podIP:targetPort`. |
...
- **Headless Service DNS fallback dials `targetPort`.** Ready EndpointSlice
  ...
  against EndpointSlice port names) for that headless DNS fallback. Translator
  tests `http_route_headless_selectorless_service_uses_ready_endpoint_slice_targets`
  and `http_route_headless_selectorless_dns_fallback_dials_target_port` pin
  both shapes.
