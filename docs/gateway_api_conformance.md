...
| Selectorless/headless Services | Yes | With pod discovery enabled, backends resolve ready EndpointSlice addresses directly; a named Service `targetPort` resolves against EndpointSlice port names, but the `backendRef.port` itself is numeric-only — see [backendRef port and zero-weight semantics](#backendref-port-and-zero-weight-semantics). A headless Service (`spec.clusterIP: None` / `spec.clusterIPs: ["None"]`) with no ready EndpointSlice addresses falls back to Cluster DNS of the Service on the numeric `targetPort`, not the Service port: headless DNS returns pod IPs, kube-proxy does not DNAT them, and dialing the Service port is a `connection_failure`. That substitution requires the explicit `"None"` sentinel; a Service that simply has no VIP recorded (ExternalName, pending, malformed, incomplete) keeps the Service port. ClusterIP Services keep the Service-port DNS fallback because kube-proxy maps `ClusterIP:servicePort` onto `podIP:targetPort`. |
...
- **Headless Service DNS fallback dials `targetPort`.** Ready EndpointSlice
  addresses are preferred. When that expansion is empty, a ClusterIP Service
  still dials `service.namespace.svc.<cluster-domain>:servicePort` (kube-proxy
  DNATs to `targetPort`). A headless Service has no ClusterIP, so the same
  fallback would resolve to pod IPs and then connect to the Service port —
  HTTP 502 `connection_failure` on the upstream
  `HTTPRouteServiceTypes` `/headless-manual-endpointslices` case. Ferrum
  therefore dials the numeric `targetPort` (or a named `targetPort` resolved
  against EndpointSlice port names) for that headless DNS fallback, and only
  when the Service declares the explicit `"None"` sentinel. Translator tests
  `http_route_headless_selectorless_service_uses_ready_endpoint_slice_targets`,
  `http_route_headless_selectorless_dns_fallback_dials_target_port`,
  `http_route_headless_cluster_ips_none_dns_fallback_dials_target_port`, and
  `http_route_non_headless_no_vip_dns_fallback_keeps_service_port` pin the
  EndpointSlice, explicit-headless, and non-headless/no-VIP shapes.
