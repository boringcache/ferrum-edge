...
| `BackendLBPolicy` / `XBackendTrafficPolicy` session persistence | Partial (Ferrum black-box / translation; not an upstream conformance feature claim) | On the pinned `v1.5.1` experimental channel `BackendLBPolicy` was **removed** and replaced by `XBackendTrafficPolicy` (`gateway.networking.x-k8s.io`). Ferrum watches both shapes: historical `BackendLBPolicy` (`gateway.networking.k8s.io/v1alpha2`) when that CRD is still installed, and `XBackendTrafficPolicy` on the current pin. `sessionPersistence` (Cookie core / Header extended) projects onto generated route Upstreams as `consistent_hashing` + `hash_on: cookie:…` / `header:…`, forcing Upstream materialization even for single-backend rules so sticky `Set-Cookie` injection runs on the live LB path. Route-rule `sessionPersistence` overrides a Service-targeted policy. `cookieConfig.lifetimeType: Session` emits a browser session cookie (no `Max-Age`); `Permanent` requires `absoluteTimeout` → `Max-Age`. `idleTimeout`, non-Service `targetRefs`, Header+`absoluteTimeout`/`cookieConfig`, and `retryConstraint` fail closed with field-specific diagnostics — a policy carrying `retryConstraint` is rejected entirely (`Accepted=False` / `UnsupportedValue`) and no portion of it (including `sessionPersistence`) is applied. Policy `status.ancestors` reports `Accepted`/`UnsupportedValue`. Upstream profile/features remain unchanged — this is not advertised as a Gateway API conformance claim. |
