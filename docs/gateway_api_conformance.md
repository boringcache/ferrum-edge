...
| Non-Gateway parents | A `UDPRoute` whose `parentRefs` name only non-Gateway parents — a GAMMA `Service` parent, a mistyped `kind`, an unrecognized `group` — also opens nothing. Ferrum implements no non-Gateway parent for `UDPRoute`, and such a route names no managed Gateway so it is not a status candidate either; falling back to the backend port would bind an unannounced north-south UDP relay. Only a route with **no** `parentRefs` at all keeps the fallback. This is stricter than `TCPRoute`/`TLSRoute`, whose historical gate arms on a Gateway parent alone |
