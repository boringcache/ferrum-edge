...
| Config database | MongoDB config stores are refused in their entirety. For PostgreSQL and MySQL, an explicitly configured `FERRUM_DB_TLS_MODE` of `disable`, `allow`, `prefer`, or `require` is refused — none of them authenticate the server certificate chain — leaving `verify-ca` and `verify-full` as the only admitted values |
