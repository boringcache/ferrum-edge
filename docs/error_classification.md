...
| (4) Post-connect ambiguous failure | RST or rustls error (including a handshake-named alert) when no independent setup-phase evidence exists and the request was not canceled | `ConnectionReset` | `true` |
...
|---|---|---|---|
| HTTP/1.1 (reqwest) | [`classify_reqwest_error`](../src/retry.rs) | `&reqwest::Error` | `is_connect()` / `is_timeout()` typed methods → typed source-chain walk for io/TLS/DNS. **Four-way boundary:** (1) connect-phase RST/refused with no rustls → `ConnectionRefused`; (2) typed rustls during the independently known connect phase → `TlsError`; (3) hyper `is_canceled()` with no rustls → `ConnectionPoolError` (issue #4406 live reqwest path; issue #3578); (4) post-connect RST or rustls error → `ConnectionReset`. Missing `close_notify` is matched *before* rustls so it stays `ConnectionClosed` (#4051). Post-connect `UnexpectedEof` / incomplete message → `ConnectionClosed`. Bounded substring fallback inside the `is_connect()` branch |
