...
Missing or duplicate anchors, duplicate resource `$id`s, malformed fragment encodings, and refs whose URI is not declared in the document fail closed. One schema expansion is also capped at 500,000 materialized values and the generated-config byte budget; accumulated generated operations are checked against that byte budget as they are built. Shallow repeated-reference fan-out therefore returns HTTP 422 `SchemaTooLarge` before it can grow exponentially. See [api_specs.md](api_specs.md) for the full importer contract.
...
Every fixed-field object in the plugin config is closed. Unknown keys at the root and in `bypass`, `error_response`, each `operations[]` entry, `request_body`, and multipart Encoding Header Objects are rejected at construction with a path-qualified error and a spelling suggestion. The `x-ferrum-validate.bypass` importer object is closed by the same key set. Because the plugin is registered `FailClosed`, a rejected config never becomes the running policy: direct Admin writes return HTTP 400, file mode fails startup, and a DP keeps its last known-good generation.
