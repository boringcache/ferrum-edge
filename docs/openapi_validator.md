...
- `application/x-www-form-urlencoded` bodies are parsed into object fields. Default serialization is `style=form, explode=true` (repeated keys). When the generated config preserves Encoding Objects, runtime honors `style` / `explode` / `allowReserved` for each property: `explode=false` splits arrays and alternating object key/value pairs with the declared delimiter, `form` + `explode=true` rebuilds objects from their emitted key/value parameters, and `deepObject` rebuilds objects from `name[key]=value` pairs. Delimiters are identified before percent-decoding so an encoded data character such as `%2C` remains part of one value. Unsupported Encoding Object combinations are rejected when the plugin config is admitted.
- `multipart/form-data` bodies are parsed with MIME delimiter-line rules (boundaries match only as `--boundary` lines, not arbitrary substrings) and quoted header-parameter parsing (semicolons and escapes inside quotes are preserved; duplicate parameters, duplicate/invalid headers, and empty parts fail closed). File parts can validate as UTF-8 binary strings or as metadata objects with `filename`, `content_type`, `size`, and UTF-8 `content`; use the metadata-object shape for arbitrary non-UTF-8 file bytes. Explicit `style` / `explode` encoding reconstructs array and object properties from the corresponding part names and delimiters. Encoding Object `contentType` and `headers` are enforced when present, and present header values are validated against their complete compiled JSON Schemas. Header Object `required` is honored and defaults to `false`; an absent optional part header is not a validation failure.
- Form and multipart scalar conversion is composition-aware: `oneOf` / `anyOf` / `allOf` do not preselect the first branch. Compatible scalar coercions are attempted in a fixed order (integer, number, boolean, string) independent of branch order, and `allOf` object properties are merged so later branches keep their declared types. Ambiguous values that coerce to more than one JSON type prefer the more specific numeric/boolean form when that form still satisfies the schema after validation.
