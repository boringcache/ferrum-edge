...
- `multipart/form-data` bodies are parsed with MIME delimiter-line rules (boundaries match only as `--boundary` lines, not arbitrary substrings) and quoted header-parameter parsing (semicolons and escapes inside quotes are preserved). File parts can validate as UTF-8 binary strings or as metadata objects with `filename`, `content_type`, `size`, and UTF-8 `content`; use the metadata-object shape for arbitrary non-UTF-8 file bytes. Encoding Object `contentType` and `headers` are enforced when present.
