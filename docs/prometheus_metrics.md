...
| `ferrum_oauth2_introspection_cache_admission_skips_total` | counter | `class`, `reason`, `namespace` | `oauth2_introspection` | `documented_only` | `always` | Valid active/inactive results not retained because fixed-work admission could not complete; class and reason are closed enums. |
| `ferrum_oauth2_introspection_cache_cleanup_work_total` | counter | `namespace` | `oauth2_introspection` | `documented_only` | `always` | Bounded eviction-index tickets examined; no token or digest labels. |
| `ferrum_oauth2_introspection_cache_entries` | gauge | `class`, `namespace` | `oauth2_introspection` | `documented_only` | `always` | Active or negative cache entries retained across live plugin generations. |
| `ferrum_oauth2_introspection_cache_evictions_total` | counter | `class`, `reason`, `namespace` | `oauth2_introspection` | `documented_only` | `always` | Cache evictions by the fixed `capacity`/`expired` reason set. |
| `ferrum_oauth2_introspection_cache_retained_bytes` | gauge | `class`, `namespace` | `oauth2_introspection` | `documented_only` | `always` | Logical retained bytes across live generations, including fixed eviction indexes and published entry reservations. |
