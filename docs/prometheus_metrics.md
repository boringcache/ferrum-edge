...
| `ferrum_mesh_slice_drift_data_planes` | gauge | `state`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Tracked mesh data planes by CP-side slice convergence state (`converged`/`drifted`/`rejecting`/`pending`/`disconnected`). Closed-set labels only — never per-DP. |
