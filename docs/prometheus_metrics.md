...
| `ferrum_notification_delivery_abandoned_at_deadline_total` | counter | `channel_type` | `notifications` | `documented_only` | `always` | Notification deliveries abandoned when reload retirement or the global shutdown drain deadline cancelled the task. |
| `ferrum_notification_delivery_attempted_total` | counter | `channel_type` | `notifications` | `documented_only` | `always` | Notification delivery attempts admitted past the dispatch semaphore (includes retries' parent task once). |
