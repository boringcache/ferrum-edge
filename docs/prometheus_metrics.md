...
| `ferrum_mesh_hbone_inner_pool_events_total` | counter | `protocol`, `event`, `namespace` | `mesh` | `documented_only` | `conditional` | Inner application connections reused inside fenced HBONE tunnels, by inner wire protocol and outcome. |
...
| `ferrum_mesh_hbone_tunnel_revocations_total` | counter | `proxy_id`, `reason`, `namespace` | `mesh` | `documented_only` | `conditional` | Live HBONE tunnels revoked by the admission fence because the current policy generation or peer credential would no longer admit their CONNECT. |
