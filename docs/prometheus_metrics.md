...
| `ferrum_mesh_hbone_tunnel_revocations_total` | counter | `proxy_id`, `reason`, `namespace` | `mesh` | `documented_only` | `conditional` | Live HBONE tunnels revoked by the admission fence because the current policy generation, peer credential, or enforced revocation list would no longer admit their CONNECT. |
