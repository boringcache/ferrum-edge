...
| `ferrum_cp_dp_trust_active_generation_age_seconds` | gauge | `namespace` | `cp_dp_trust` | `documented_only` | `conditional` | Seconds since the CP last accepted (replaced or confirmed) its trust generation, on a monotonic clock. |
| `ferrum_cp_dp_trust_degraded` | gauge | `namespace` | `cp_dp_trust` | `documented_only` | `conditional` | Whether the CP is authorizing with a trust generation it could not revalidate (1) or not (0). |
...
| `ferrum_cp_dp_trust_reload_attempts_total` | counter | `namespace` | `cp_dp_trust` | `documented_only` | `conditional` | CP/DP trust-bundle reload attempts that produced a verdict since process start. |
| `ferrum_cp_dp_trust_reload_consecutive_failures` | gauge | `namespace` | `cp_dp_trust` | `documented_only` | `conditional` | Consecutive refused reload attempts since the last accepted generation. |
...
| `ferrum_cp_dp_trust_reload_rejections_total` | counter | `reason`, `namespace` | `cp_dp_trust` | `documented_only` | `conditional` | Reload attempts refused, by closed reason. The previous verifier was retained. |
| `ferrum_cp_dp_trust_reload_worker_running` | gauge | `namespace` | `cp_dp_trust` | `documented_only` | `conditional` | Whether the trust-bundle reload worker is alive (1) or stopped/failed (0). |
