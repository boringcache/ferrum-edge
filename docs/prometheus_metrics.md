...
| `ferrum_node_agent_cni_calls_total` | counter | `verb`, `outcome`, `namespace` | `node_agent` | `documented_only` | `conditional` | Node-agent CNI plugin RPC outcomes by verb (`add`/`del`/`check`/`gc`) and outcome (`success`/`rejected`/`error`). |
