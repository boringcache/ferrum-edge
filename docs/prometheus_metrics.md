...
| `ferrum_node_agent_ingress_interface_configured_count` | gauge | `namespace` | `node_agent` | `documented_only` | `conditional` | Number of explicitly configured ingress interfaces (names are never labels). |
| `ferrum_node_agent_ingress_interface_expected_count` | gauge | `namespace` | `node_agent` | `documented_only` | `conditional` | Number of interfaces required by the proved node/CNI route topology (names are never labels). |
