...
**Deleted route still served:** scrape `watch_deletes_total` and `config_publications_total` around the deletion. Neither advancing means the control plane never observed the withdrawal (a stalled watch; recovery is bounded by `FERRUM_K8S_WATCH_IDLE_RELIST_SECS`). Both advancing while traffic continues means the control plane published and the fault is downstream — CP→DP distribution or listener rebuild — not the watch.
