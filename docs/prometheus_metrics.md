...
| `ferrum_mesh_metric_series_overflow_total` | counter | `family`, `gateway_namespace` | `mesh` | `documented_only` | `conditional` | Mesh metric series admissions dropped because the per-family live-series budget was exhausted (exact CEL values retained until the budget; overflow has fixed family labels only). |
