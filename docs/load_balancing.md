...
**Concurrency model:** Each WRR lane (parent upstream, subset, or port override) keeps a **bounded multi-fingerprint cache** of precomputed smooth-WRR orders behind `ArcSwap` slots, plus an `AtomicU64` selection counter. Steady-state picks for any cached healthy-set fingerprint are wait-free across Tokio workers (slot scan + atomic counter) and share one global interleaving for that lane, so long-run ratios match configured weights without a per-request blocking mutex — including when per-proxy passive health or retry exclusions alternate among a few recurring fingerprints. On a cache miss, a publisher uses `try_lock` to store into a round-robin slot; contending missers build a local schedule for that selection only and never block. Schedules are a pure function of healthy-set fingerprint and immutable target weights on the current `LoadBalancer` generation (config reload swaps the balancer `Arc`), so no invalidate flag is required. The counter wraps with `% order.len()` and does not bias ratios. Schedule length is capped (8192) so pathological huge weights truncate and repeat a bounded prefix; at most a fixed number of fingerprint schedules are retained per lane.
