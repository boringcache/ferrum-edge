...
**Concurrency model:** Each WRR lane (parent upstream, subset, or port override) keeps a **bounded multi-fingerprint cache** of precomputed smooth-WRR orders behind `ArcSwap` slots. Every cached healthy set owns an independent set of cache-line-padded `AtomicU64` selection-counter shards (one shard per concurrent worker thread), preventing alternating per-proxy health or retry fingerprints from aliasing onto one counter parity and preventing small healthy sets from bouncing a single shared cache line under concurrency. Steady-state picks are wait-free across Tokio workers (slot scan + sharded atomic counter). Each shard walks the same precomputed smooth-WRR order, so long-run ratios match configured weights without a per-request blocking mutex; workers intentionally do **not** share one global interleaving. On a cache miss, a publisher uses `try_lock` to store into a round-robin slot; contending missers build a local schedule for that selection only and never block. Schedules are a pure function of exact healthy-set membership and immutable target weights on the current `LoadBalancer` generation (config reload swaps the balancer `Arc`), so no invalidate flag is required and the >128-target path cannot reuse a schedule after a hash collision. Each shard counter wraps with `% order.len()` and does not bias ratios. Schedule length is capped (8192) so pathological huge weights truncate and repeat a bounded prefix; at most a fixed number of healthy-set schedules are retained per lane.
