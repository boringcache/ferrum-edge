...
**Concurrency model:** Each WRR lane (parent upstream, subset, or port override) keeps a **bounded multi-fingerprint cache** of precomputed smooth-WRR orders behind `ArcSwap` slots. Every cached healthy set owns an independent `AtomicU64` selection counter, preventing alternating per-proxy health or retry fingerprints from aliasing onto one counter parity. Steady-state picks are wait-free across Tokio workers (slot scan + atomic counter) and share one interleaving for that healthy set, so long-run ratios match configured weights without a per-request blocking mutex. On a cache miss, a publisher uses `try_lock` to store into a round-robin slot; contending missers build a local schedule for that selection only and never block. Schedules are a pure function of exact healthy-set membership and immutable target weights on the current `LoadBalancer` generation (config reload swaps the balancer `Arc`), so no invalidate flag is required and the >128-target path cannot reuse a schedule after a hash collision. Each counter wraps with `% order.len()` and does not bias ratios. Schedule length is capped (8192) so pathological huge weights truncate and repeat a bounded prefix; at most a fixed number of healthy-set schedules are retained per lane.
