...
**Cleanup mechanism:** TTL-based expiration through an exact `BTreeMap` age index. The lookup map and age index share the same immutable nonce allocation, so the index does not retain a second nonce string and cannot accumulate stale queue records. A nonce is retained only after its PasswordDigest verifies, and its encoded length is checked before Base64 decoding. When either the entry cap or retained-key-byte cap is reached, the age index is walked from its oldest end and **only entries already past `nonce.cache_ttl_seconds` are reclaimed**; the walk stops at the first still-live entry. There is no forced eviction of live entries — a nonce claimed inside its TTL keeps replay protection for the whole window. One request reclaims at most 64 indexed entries and every examined/removed entry costs O(log n); no request scans the lookup map or constructs candidates proportional to `max_cache_size`. If that bounded reclamation cannot free entry *and* byte room — or state is inconsistent/poisoned, or checked accounting fails — the request fails closed (HTTP 401) and the nonce is not recorded. A rejected claim leaves the retained set unchanged, so a retry after entries expire is a normal fresh claim. Size the two caps for peak authenticated PasswordDigest rate × TTL: under-provisioning surfaces as `401` rejections rather than as a silent replay window. Admission, reclamation, and retained-byte accounting share one narrow mutex so concurrent claims cannot overshoot either hard cap (same-key races resolve as replay or in-place refresh without a new reservation).
