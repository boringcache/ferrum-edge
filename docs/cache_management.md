...
**Config fields:** `nonce.max_cache_size` (default 10,000), `nonce.cache_ttl_seconds` (default and enforced minimum `username_token.created_max_age_seconds + 2 x username_token.created_clock_skew_seconds + 1`, i.e. 901s with the defaults — see [`soap_ws_security`](plugins.md#the-claim-must-outlive-the-token)), `nonce.max_encoded_length` (default 512), and `nonce.max_total_cache_bytes` (default 8,388,608). These are the canonical names; there is no `nonce_replay_protection` alias and that object is rejected as an unknown configuration key.

**Cleanup mechanism:** TTL-based expiration through an exact `BTreeMap` age index. The lookup map and age index share the same immutable nonce allocation, so the index does not retain a second nonce string and cannot accumulate stale queue records. A nonce is retained only after its PasswordDigest verifies, and its encoded length is checked before Base64 decoding. When either the entry cap or retained-key-byte cap is reached, the age index is walked from its oldest end and **only entries already past the scope's retention are reclaimed**; the walk stops at the first still-live entry. There is no forced eviction of live entries — a nonce claimed inside its window keeps replay protection for the whole window, and because that window is a monotone high-water mark over every plugin generation sharing the process replay scope, a reload to a shorter retention cannot expire or refresh a claim another generation made. One request reclaims at most 64 indexed entries and every examined/removed entry costs O(log n); no request scans the lookup map or constructs candidates proportional to `max_cache_size`. If that bounded reclamation cannot free entry *and* byte room — or state is inconsistent/poisoned, or checked accounting fails — the request fails closed (HTTP 401) and the nonce is not recorded. A rejected claim never removes a live entry; any bounded expired-entry reclamation remains committed, so retries converge safely after enough state has expired. Size the two caps for peak authenticated PasswordDigest rate × TTL: under-provisioning surfaces as `401` rejections rather than as a silent replay window. Admission, reclamation, and retained-byte accounting share one narrow mutex so concurrent claims cannot overshoot either hard cap (same-key races resolve as replay or in-place refresh without a new reservation).
...
| `soap_ws_security` | `nonce.cache_ttl_seconds` | `901` (derived) | Nonce claim retention; defaults to and must be at least `created_max_age_seconds + 2 x created_clock_skew_seconds + 1` |
