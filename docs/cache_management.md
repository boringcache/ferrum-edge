...
**Cleanup mechanism:** TTL-based expiration (each record's native TTL by default, or `FERRUM_DNS_TTL_OVERRIDE_SECONDS` when set), stale-while-revalidate (serves old IP while refreshing in background), a background refresh task that keeps entries warm with a per-cycle soonest-expiry cap, 15s per-lookup timeout, and `Delay` ticks, a failed DNS retry task that re-attempts resolution of error-cached entries with exponential error-TTL backoff, age eviction at `FERRUM_DNS_STALE_TTL`, a shared concurrency bound (`FERRUM_DNS_MAX_CONCURRENT_REFRESHES`) for SWR and proactive refresh plus the same per-cycle selection/concurrency cap for failed retries, independent 5s capacity eviction so a slow resolver cannot starve `FERRUM_DNS_CACHE_MAX_SIZE`, and over-capacity eviction that prefers error entries over live success entries.
