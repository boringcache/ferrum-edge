...
- OpenAPI 3.x request schemas from `requestBody.content.{mediaType}.schema`, preserving `encoding` for `application/x-www-form-urlencoded` and `multipart/form-data` in the strict generated `{schema, encoding}` shape (unsupported media types, unknown properties, invalid field types/styles, reserved headers, case-insensitive duplicate multipart encoding header names, Header Object `schema`/`content` exclusivity and exactly-one media type for `content`, and other unsupported combinations fail closed).
