...
The object form is a **closed** fixed-field object. Accepted keys are exactly `mode`, `request`, `response`, `validate_request`, `validate_response`, `bypass`, `fail_on_unknown_operation`, `fail_on_missing_response_schema`, `max_body_bytes`, `error_response`, and `error_truncate_chars`; `request` and `response` accept only `enabled` and `content_types`. Any other key — including `operations`, which is always regenerated from the document — is rejected with HTTP 400 and a spelling suggestion. Unknown keys used to be copied verbatim into the generated plugin config, so a misspelled enforcement control deployed successfully with the weaker default still in force. The generated plugin config itself is closed the same way at construction; see [openapi_validator.md](openapi_validator.md#strict-config-admission).
