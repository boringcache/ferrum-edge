...
- **Files**: only under `FERRUM_ADMIN_SPEC_EXTERNAL_REFS_FILE_ROOT` (absolute). Paths are canonicalized; symlink escape and `..` traversal out of the root fail closed. Non-regular files are rejected. Client errors never echo raw host paths.
- **Network**: HTTPS origins listed in `FERRUM_ADMIN_SPEC_EXTERNAL_REFS_ALLOWED_ORIGINS` only (plus optional explicit HTTP fixture origins). Embedded credentials and query strings are rejected. Destinations are screened through Ferrum's backend egress / DNS-rebinding boundary (`BackendEgressPolicy`); localhost, link-local, private, metadata, multicast, and unspecified addresses fail closed for HTTPS. Redirects are constrained to the same policy and a redirect budget. Ambient admin credentials, cookies, bearer tokens, and proxy credentials are never forwarded.
- **Budgets**: per-document and aggregate byte caps, document count, reference count, URI length, nesting depth, connect/request/total timeouts, and allowed Content-Types. Unknown, invalid, or oversized inputs fail closed with field-specific, redacted diagnostics.

**Snapshot / last-good:** successful admission persists a gzip-compressed normalized document set with per-document content digests and an aggregate `external_ref_digest`. Cache keys include policy digest and canonical document identity. A failed PUT/extract retains the previously accepted generation (transactional replace / no write). Specs are admin-only metadata (not CP→DP published); one tenant/namespace cannot read another's snapshot. Backup section version `2` carries optional snapshot bytes; version `1` backups remain restorable without them.
