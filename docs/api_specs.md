...
The same dedicated restore boundary is used when `DELETE /proxies/{id}` directly removes a proxy owned by an API spec and late compensation is required. The stamped spec-owned resources, any orphaned hand-owned upstream, and hand-owned plugins removed by the proxy cascade are restored through the compensation contract, not replayed as a normal client submission or separate writes. Before direct proxy deletion, Ferrum re-reads the current upstream and every cascade plugin through ownership-preserving queries. A resource stamped to another API spec, or one that disappears during this guarded precheck, returns 400 without deleting anything; an ownership-stripped runtime snapshot is never allowed to relabel that resource as hand-owned during compensation. When the ownership-stripped namespace snapshot and the owning API-spec snapshot contain the same upstream ID, the authoritative API-spec snapshot supplies the restored ownership metadata. `DELETE /api-specs/{id}` also snapshots a current hand-owned upstream when the imported spec has no owned upstream or its proxy has drifted to one. A shared hand-owned upstream that survived either delete remains authoritative and is reused by the restored proxy after its stable identity is verified.
