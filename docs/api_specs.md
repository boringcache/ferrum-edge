...
Before deletion, Ferrum re-reads every plugin that the proxy cascade would remove through an ownership-preserving admin query. Proxy-scoped configs remain valid without a reverse entry in `proxy.plugins`; compensation restores those configs in the same transaction while preserving their unattached state. The restore snapshot is validated before persistence: a cascade plugin tagged to another API spec, a global plugin carrying the deleted proxy ID, a proxy-group plugin carrying a proxy ID, or an embedded `proxy.plugins` association naming a missing config returns 422 without deleting anything. This keeps categorically unrestorable or malformed persistent graphs available for operator repair instead of misreporting them as transient races or discovering them only during compensation.
