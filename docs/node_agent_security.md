...
| `ferrum-mesh-cni-cleanup` / `-wait` identity (ServiceAccounts, Role, RoleBinding) | for the lifetime of the release | none of their own | Ordinary release resources, deliberately not hook resources: Helm deletes `hook-succeeded` hooks when a later hook in the phase fails, so identity held that way would disappear exactly when a retry needs it. Grants are unchanged by this; the cleanup ServiceAccount still has no Role at all. |
