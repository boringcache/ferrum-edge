...
- **Pre-routing lookup is sound because equivalence is proven, not assumed.** This plugin short-circuits ahead of the route-dispatch plugins, so the destination it binds is the pre-dispatch one. Every input those plugins route on — method, authority, path, raw query, request headers, and the exact backend-visible request body — is bound into the fingerprint, so two requests that would select different effective destinations cannot present equal fingerprints, and a fingerprint mismatch is a conflict rather than a replay. `mcp_gateway`, whose rewrite derives from live upstream discovery state rather than from the request, cannot be proven equivalent this way and is refused outright by composition validation
...
| `cache_key_include_query` | bool | `true` | Include the exact raw query string in the cache key. It is bound as a length-framed field of the key digest, without parsing, sorting, percent-decoding, or normalizing |
...
- When a store would exceed `max_total_size_bytes`, oldest entries are evicted until the eligible representation fits; only an entry larger than the whole cap is refused outright. Both the byte-cap and the entry-count paths pop an insertion-ordered queue, so their cost is bounded by the number of entries actually evicted rather than by cache size, and stale entries cannot trap the byte budget.
...
- **The base partition binds every stable backend-visible dimension**: the effective *post-routing* destination (proxy id/namespace, listen path, upstream id or direct host/port/scheme, route authority, rewritten path — `response_caching` runs after every route-dispatch plugin, so this is the destination that will serve a miss), the request authority/method/path/raw query, and the caller-authorization partition below.
- **Caller authorization, not a display subject.** Authenticated callers bind the authentication mechanism, the resolved identity and consumer, the peer SPIFFE identity, and SHA-256 digests of every credential header presented (`Authorization`, `Proxy-Authorization`, `Cookie`, `X-API-Key`, `API-Key`, `APIKey`, `X-Goog-Api-Key`, `X-Forwarded-Authorization`, `X-Amz-Security-Token`, `X-Auth-Token`, `X-Access-Token`). Two tokens that resolve to the same `sub` with different scopes, audiences, or tenancy claims therefore land in different partitions. Anonymous callers bind their canonical peer address per `anonymous_caller_scope`. This is unconditional — `cache_key_include_consumer` cannot disable it.
- **The complete Vary tuple is digested**, including each dimension's name, a framed presence flag (an absent header is distinct from an empty one), and its value — not only the subset classified as sensitive. Credential and operator-redacted values are still reduced to `sha256-…` before entering the digest.
- **Requests that declare a body bypass the cache.** A shared cache selects a stored representation by method + target + `Vary`; nothing in that selection witnesses a request body, and lookup happens before the final backend-visible body exists. Any `Transfer-Encoding`, or a non-zero/unparsable `Content-Length`, yields `X-Cache-Status: BYPASS` for both lookup and storage.
