...
In `terminate` mode, applicable end-to-end function response headers (including repeated `Set-Cookie`, benign `Location`, `Retry-After`, `ETag`, and `Content-Disposition`) are returned with the function status/body. Hop-by-hop, `Connection`-listed, transport-managed, provider-control, and credential-bearing fields are stripped. A `Location` is also removed when its relative or absolute destination exposes the configured function's signed path/query components, including percent-encoded or nested redirect targets. Percent decoding has a fixed work budget; residual encoding beyond that budget fails closed by removing the redirect rather than forwarding an unproven target. Unrelated same-origin and cross-origin redirects remain observable. Ferrum recomputes framing; HEAD, 204, and 304 responses have no body. If a later response transform replaces the body, the shared lifecycle removes stale ranges, validators (including `ETag` and `Last-Modified`), digests/checksums, and content-bound signatures; unmodified responses retain them. `206 Partial Content` is not transformed because its bytes are only a selected range, so its status, `Content-Range`, validators, and body remain coherent.
