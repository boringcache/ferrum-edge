...
| `respect_no_cache` | bool | `true` | Bypass cache lookup when the client sends `Cache-Control: no-cache` or `no-store`. A header containing only honored refresh members addresses the existing entry partition; mixed refresh plus arbitrary/unimplemented directives stays bound as origin-visible context. When disabled, the entire request `Cache-Control` value stays bound because it is not an honored cache operation |
