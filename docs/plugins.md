...
- Response-header-side, decided as soon as the backend headers arrive: a status outside `cacheable_status_codes` (or one whose caching semantics this plugin does not implement), a `Content-Range` partial answer, a `Set-Cookie` response, and an origin-selected `text/event-stream` are released before any body byte is retained. An event stream therefore flows to the client incrementally even when the client never sent `Accept: text/event-stream`.
- Responses whose store-side refusal *also evicts an existing entry* — `Cache-Control: no-store`/`private`/`no-cache`, a zero freshness lifetime, an unusable `Vary` — deliberately stay buffered so that eviction still happens.
