...
| `grpc_message` | no | String; CR/LF normalized to spaces and trimmed before emission, then bounded by the same 8 KiB trailer **wire**-value ceiling as every other terminal value (measured on the sanitized bytes) |
...
| `trailers` | no | Object of string trailer values. At most 32 entries; each **wire** value ≤ 8 KiB; valid HTTP field names/values. Reserved / protocol-owned names (`grpc-status`, `grpc-message`, `grpc-status-details-bin`, `content-type`, hop-by-hop, `grpc-encoding`, `grpc-accept-encoding`, …) are rejected — use the dedicated fields above |
...
This is the only plugin rejection allowed to put a body on a native gRPC stream, and the authorization is request-scoped provenance rather than response shape: the gateway records the exact frame and terminal trailers this contract produced, and preserves them only for a byte-identical body. Any other rejection — including one whose body happens to be a well-formed unary frame and whose headers claim `application/grpc` plus `grpc-status` — normalizes to trailers-only, as does this one if a later `after_proxy` decorator rewrites the body. Client-visible trailers come only from the validated contract above, so gateway-managed and decorator-added response headers (CORS, header policy, internal bridge fields) stay in the initial HEADERS block and can never surface as custom trailers.

Because the terminal trailers are not part of the response header map, any component that persists a finalized representation must retain them alongside it or the representation is incomplete. `request_deduplication` does exactly that: it captures the terminal trailers only when the response body is byte-identical to the frame the contract authored, retains them in a private field of the cached entry (charged against `max_entry_size_bytes` / `max_total_size_bytes` and serialized into the Redis replay record), and restores the request-scoped authorization from that private representation immediately before serving a replay. A persisted representation is re-validated against every contract bound on read and fails closed — refusing the replay — rather than replaying a DATA frame with fabricated terminal metadata; an ordinary cached response can never acquire the authorization, whatever its body shape or headers.
