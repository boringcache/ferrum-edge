...
When the absolute deadline is exhausted, the gateway returns gRPC status `DEADLINE_EXCEEDED` (status code 4). If H2 or H3 response headers were already committed but no DATA bytes were forwarded, it emits a terminal status-4 trailer frame; after partial DATA it aborts the stream because a complete gRPC message boundary cannot be assumed. Deadline-capable streaming relays remove an upstream `Content-Length` before committing headers because the terminal replacement has a different representation length.
