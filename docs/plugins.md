...
| `providers[].output_claim_headers` | Object[] (optional) | Istio `RequestAuthentication.jwtRules[].outputClaimToHeaders` projection: an ordered list of `{header, claim}` pairs (a list, not a map, because one claim may be published to several headers). Destinations are gateway-owned exactly like `claim_headers` — stripped from the inbound request before validation and set only from a validated claim. Duplicate destinations, reserved/credential-bearing destinations, and invalid header names are rejected at config load; string, number, boolean, and scalar-array claims are published, and anything else (or a header-illegal value) leaves the destination absent |
