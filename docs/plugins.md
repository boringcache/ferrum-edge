...
Stored/hit `Content-Encoding`, body bytes, and `Vary: Accept-Encoding` keying are asserted by `test_backend_vary_accept_encoding_caches_binary_variant` in `tests/unit/plugins/response_caching_tests.rs`. The cache-hit short-circuit and preservation of the stored representation through compression's rejection-finalization boundary are covered in `tests/unit/plugins/compression_tests.rs` by `test_response_cache_hit_cannot_bypass_required_406` and `test_response_cache_hit_preserves_identity_when_acceptable`.
