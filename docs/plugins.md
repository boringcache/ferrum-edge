...
**Aggregate SSE multiplexing.** In `aggregate_router` mode with `sessions.sse_multiplexing` (default `true`), a client may open one `GET` SSE listener per live downstream session (`Accept: text/event-stream` plus the configured session header). The gateway attaches a generation-scoped multiplexed session broker that routes JSON-RPC events by bounded request/stream identity (canonicalized JSON-RPC string/number ids), applies per-session stream cardinality and queue/replay byte caps (`sessions.sse_max_*`), and tears the listener down on client disconnect, session `DELETE`, idle eviction, or plugin reload/update/delete so events cannot cross generations. Duplicate listeners for the same session return `409`; missing/unknown/stale sessions and invalid/oversized/unrepresentable stream ids or `Last-Event-ID` values fail closed with field-specific, value-redacted diagnostics (`mcp.sse.error` metadata carries only fixed reason tokens — never the raw id, header, or payload). When a listener is attached, gateway-handled responses such as `ping` may be delivered as SSE `message` events with the POST acknowledged as `202`; without a listener (or when multiplexing is disabled) the ordinary JSON response path is preserved. `notifications/cancelled` with `params.requestId` cancels an open stream identity. Transparent mode continues to forward `GET` upstream unchanged. Reloading or deleting the plugin instance drops the broker and ends in-flight SSE bodies; a replacement generation starts empty.
