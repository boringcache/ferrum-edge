...
>
> When the backend returns a successful (2xx) response but a **later plugin rejects it** — either a response-body plugin (for example `ai_response_guard` blocking the completion) or a lower-priority `after_proxy` plugin (for example `response_size_limiting` rejecting an oversized 200 by `Content-Length`, before `ai_rate_limiter` runs) — the provider call has already consumed tokens, so `ai_rate_limiter` deliberately **keeps the reservation charged** rather than releasing it — the rejected-but-served generation is not free. The original backend status is captured before the `after_proxy` chain runs, so this holds even when the rejecting plugin runs before `ai_rate_limiter`. (A genuine gateway rejection that never produced a backend response, e.g. a failed auth check, still releases the reservation.)

> **Pre-reservation uses the pre-transform request body.** The estimate is computed in `before_proxy` from the inbound (buffered) request body. If a `request_transformer` body rule runs later in the pipeline and *raises* an output cap (`max_tokens` / `max_completion_tokens` / `max_output_tokens`) or appends prompt content, the backend-visible request can be larger than what was reserved, so concurrent transformed requests can briefly oversubscribe the budget. Post-response reconciliation corrects the charge to actual usage, so this is a bounded, self-correcting window rather than a persistent bypass. If exact up-front reservation matters for a proxy that inflates the body in a transform, set the final cap before `ai_rate_limiter` (e.g. via `ai_request_guard`'s `default_max_tokens`) so the inbound body already reflects it. The limiter only counts text-bearing fields for the prompt estimate and **excludes inline binary payloads** (base64 `image_url`/`source`/`inline_data`/`input_audio` data and any well-formed `data:` URL — `data:[<mediatype>][;base64],<payload>`), so multimodal/vision requests are not falsely rejected by oversized image bytes. Ordinary prose that merely begins with `data:` (e.g. a chat message `"data: my notes"`) lacks the structural `,` separator and is still counted as text.

> **Compressed request bodies are reconciled-only, not pre-reserved.** When the inbound request carries a non-identity `Content-Encoding` (`gzip`/`br`/…) and the `compression` plugin's request decompression is enabled, the buffered body is still the *compressed wire bytes* at `before_proxy` time — request decompression runs later, in `transform_request_body`. A token estimate derived from those bytes would be wrong/tiny, so `ai_rate_limiter` **skips the estimate-based pre-reservation** for compressed requests and falls back to the plain budget check (no body-derived reservation, but an already-exhausted window still rejects). Post-response reconciliation charges the actual provider-reported usage, so compressed requests are debited correctly *after* the call rather than reserved before it. The limiter does not decompress the body itself. This mirrors how `ai_request_guard` skips JSON inspection of compressed bodies, and is the same phase-ordering trade-off as the pre-transform note above — bounded and self-correcting via reconciliation, not a persistent bypass.
