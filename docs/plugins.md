...
When `mode` is `terminate` and the client request is native gRPC (`application/grpc` / `application/grpc+…`, classified by the shared native-gRPC content-type helper), the function must return HTTP 2xx with a JSON object. Ferrum frames a single uncompressed unary DATA message and owns reserved terminal metadata:
...
| `grpc_message` | no | String; CR/LF normalized to spaces and trimmed before emission |
...
| `status_details_base64` | no | Standard base64 for `grpc-status-details-bin` (revalidated and re-encoded; value capped at 8 KiB) |
| `trailers` | no | Object of string trailer values. At most 32 entries; each value ≤ 8 KiB; valid HTTP field names/values. Reserved / protocol-owned names (`grpc-status`, `grpc-message`, `grpc-status-details-bin`, `content-type`, hop-by-hop, `grpc-encoding`, `grpc-accept-encoding`, …) are rejected — use the dedicated fields above |
