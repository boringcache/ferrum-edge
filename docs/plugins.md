...
> **Compressed request bodies are reconciled-only, not pre-reserved.** When the inbound request carries a non-identity `Content-Encoding` (`gzip`/`br`/…) and the `compression` plugin's request decompression is enabled, the buffered body is still the *compressed wire bytes* at `before_proxy` time — request decompression runs later, in `transform_request_body`. A token estimate derived from those bytes would be wrong/tiny, so `ai_rate_limiter` **skips the estimate-based pre-reservation** for compressed requests and falls back to the plain budget check (no body-derived reservation, but an already-exhausted window still rejects). Post-response reconciliation charges the actual provider-reported usage, so compressed requests are debited correctly *after* the call rather than reserved before it. Because a compressed body yields no body-derived reservation, `ai_rate_limiter` also does not request pre-buffering of it (a co-located plugin that needs the body, e.g. `ai_request_guard`, can still force buffering). The limiter does not decompress the body itself. This mirrors how `ai_request_guard` skips JSON inspection of compressed bodies, and is the same phase-ordering trade-off as the pre-transform note above — bounded and self-correcting via reconciliation, not a persistent bypass.
