...
phase, not `before_proxy` (advisory `GHSA-4vr5-4wm3-x5xv`). The mirrored headers
and body are byte-identical to what the primary backend receives: request-body
transforms have already run, so a field `request_transformer` was configured to
remove or redact never reaches the shadow destination, and every final
request-policy hook (WAF body rules, OpenAPI request schema, `body_validator`,
the post-transform `request_size_limiting` ceiling) has already accepted the
request — a request Ferrum rejects is never mirrored. Because the metered length
is the post-transform length, a transform that inflates the body past
