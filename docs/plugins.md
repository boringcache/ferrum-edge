...
Caches final client-visible HTTP responses in gateway memory. The cache key binds the effective destination, complete backend-visible request target and header view, mandatory caller-authorization context, and any backend `Vary` dimensions.
...
- **The base partition binds every stable backend-visible dimension**: the effective *post-routing* destination (proxy id/namespace, listen path, upstream id or direct host/port/scheme, route authority, rewritten path — `response_caching` runs after every route-dispatch plugin, so this is the destination that will serve a miss), original authority, method, path, effective outbound query, every request header except hop-by-hop/framing fields Ferrum regenerates, and the caller-authorization partition below. Credential header values enter only as SHA-256 digests. Tracing, correlation, tenancy, versioning, and other ordinary origin-visible fields are not assumed irrelevant.
