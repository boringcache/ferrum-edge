...
**Route-level header precedence:** when `mesh_route_dispatch` publishes `request_transform` rules onto `RequestContext`, every enabled `request_transformer` instance on the proxy applies only its static header/query rules first. Proxy core then applies the matched route list exactly once after the last eligible (enabled) instance, so route-level set/add/remove wins over later static conflicts under multiple same-type instances and priority overrides. A disabled RTDS instance is not eligible and does not consume the list. Setting `apply_route_overrides: true` allows an empty `rules` array so the VirtualService translator can auto-emit an eligible consumer when no operator transformer exists.
