...
| `streaming.tokenizer` | string | required for `tokens` | `inspect` + `window: tokens` only. Bounded deterministic tokenizer: `chars4` (~4 Unicode scalars/token, same estimate as `ai_rate_limiter` / `ai_prompt_compressor`), `whitespace` (Unicode whitespace-delimited runs), or `unicode_words` (alphanumeric runs; each non-whitespace non-alphanumeric scalar is its own token — including CJK). Not a provider BPE model; rejected when `window` is not `tokens` |
...
| `streaming.overlap_tokens` | uint | `min(32, max_window_tokens-1)` | `inspect` + `window: tokens` only. Complete tokens of already-cleared prose re-inspected with the next window. Must be strictly less than `max_window_tokens`. Rejected when `window` is not `tokens` |
