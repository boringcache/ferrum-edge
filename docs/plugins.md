...
**Behavior:**
- Logical idempotency keys are scoped by the stable plugin-config identity, matched proxy namespace and ID, peer SPIFFE identity when present, authenticated identity when `scope_by_consumer: true`, and the idempotency header value. Namespace is part of the logical digest even under a custom Redis prefix, so equal resource IDs in separate namespaces never share ownership or replay state. The stored key is a framed SHA-256 digest, not a delimiter-joined raw string
...
- **Authorized requests need an explicit shared-cache opt-in (RFC 9111 §3.5).** A request is treated as authorized when it carries an `Authorization` header *or* when Ferrum authenticated it itself — so a gateway that forwards bearer tokens to a backend which validates them is covered even though no gateway identity exists. Without `Cache-Control: public`, `must-revalidate`, or `s-maxage` on the response, the response is not stored at all, so backend-side revocation, expiry, and scope changes take effect immediately instead of being masked for the entry's lifetime. Hashing the credential into a Vary dimension or setting `cache_key_include_consumer` isolates keys but is not a substitute for origin permission and does not by itself permit storage. The decision reads the same live/restored header view the cache key is derived from, so a request-side transformer that adds or removes the credential cannot move a response across the boundary. `Proxy-Authorization` is an intermediary credential and does not trigger this rule; it is still an auto-Vary dimension and is only ever recorded hashed.
...
**Counter storage** (`sync_mode`): gRPC method rate-limit counters support `local` and `redis` only. Database-backed counters are intentionally unsupported. Redis mode uses the shared failover limiter (same single-timestamp sliding-window progress and instance-owned health tasks as `rate_limiting`) and falls back to local counters while Redis is unavailable. Local and Redis-fallback maps are capped at 100,000 keys with sampled below-cap stale pruning (every 1,024 rate checks, cooldown-gated to at most once per second) and immediate over-cap force eviction after prune.
...
- *Records-per-minute is reserved at capture admission.* A transaction that cannot reserve a slot in the current window is dropped at that point rather than re-checked at enqueue, so a record that would have squeezed into the next window boundary is dropped instead. This is the trade that lets a saturated instance stop paying capture CPU; the per-window ceiling itself is unchanged, the window is anchored to the process-monotonic clock, and an uncommitted reservation released after the window rolls never decrements the live counter.
...
- `anthropic` — the OpenAI request is translated to the Anthropic Messages API streaming request (system extraction, user/assistant/tool messages, `max_tokens`, `temperature`, `top_p`, `stop` → `stop_sequences`, `tools`/`tool_choice`, assistant `tool_calls` → `tool_use` blocks, matching `role: "tool"` → user `tool_result` blocks, `stream: true`). Malformed tool-call history (bad argument JSON, missing ids, orphaned tool results, unsupported roles) is rejected with an OpenAI-shaped `400` rather than silently dropped. A `ResponseStreamInspector` then normalizes Anthropic SSE events into OpenAI `chat.completion.chunk` SSE on the fly: `content_block_delta` text deltas → `choices[].delta.content`, tool-use blocks / `input_json_delta` → `choices[].delta.tool_calls`, `message_delta` → the final `finish_reason` chunk and (when `normalize_response_stream: true`) a terminal usage chunk, followed by `data: [DONE]`. Successful termination requires Anthropic `message_stop` (or an explicit provider `error` event); clean EOF before that state, and malformed complete/trailing SSE event data, surface an `upstream_error` SSE frame plus exactly one `[DONE]` instead of synthesizing a success-only sentinel. Terminal `message_stop` / provider `error` / fail-safe paths `Terminate` the inspector driver so upstream read stops immediately. The normalizer is robust to chunk splits — it accumulates raw bytes and only transcodes complete SSE events — and bounds that accumulation at 1 MiB per event: a provider that streams a pathological or never-terminated SSE event fails safe with a terminal SSE error event + `data: [DONE]` instead of buffering without bound. Unknown Anthropic event types are ignored for forward compatibility.
...
**Metadata keys written:** `ai_stream_router.enabled`, `ai_stream_router.claimed`, `ai_stream_router_claimed`, `ai_request_streaming`, `suppress_backend_consumer_identity_headers`, `ai_stream_router.provider`, `ai_stream_router.provider_type`, `ai_stream_router.model`, `ai_stream_router.request_translated`, `ai_stream_router.normalized_response_stream`, `ai_stream_router.provider_content_encoding` (when a residual provider coding must be decoded), and `ai_stream_router.fallback_attempts`.
...
- Anthropic request translation is text-first for message content in this MVP (top-level `tools`/`tool_choice` and OpenAI tool-call / tool-result history are translated; non-text message-content parts are dropped). Multimodal content parts and legacy OpenAI `function_call` / `role: "function"` shapes remain unsupported and fail closed when present in a way that cannot be represented safely.
...
- **Plugin ordering and prompt privacy**: This plugin runs at priority 2996, after request size/rate limiting, `ai_prompt_shield`, WAF, body/OpenAPI validation, `ai_request_guard`, and route-dispatch plugins (`ai_stream_router`, `mcp_gateway`, `a2a_gateway`, `mesh_route_dispatch`), but before request transformers and `ai_federation`. Exact/semantic keys therefore bind the canonical route template (`listen_path`), request path, any post-routing rewrite path, and the effective destination/provider identity that will serve a miss. Exact cache hits and semantic embedding calls see the backend-visible request body after earlier admission plugins have accepted or transformed it. Cache hits still consume request-count rate-limit budget and pass through earlier admission/fault-injection plugins before short-circuiting. Semantic mode still sends prompt text to the configured embedding endpoint; use an approved provider or private/auth-aware embedding proxy for sensitive prompts.
...
**Centralized mode** (`sync_mode: "redis"`): Token budgets are shared across all gateway instances so consumers cannot exceed limits by spreading requests across data planes. Uses the same two-window weighted approximation (single-timestamp subsecond elapsed fraction) and automatic fallback as `rate_limiting`, including instance-owned Redis health tasks that stop on plugin drop/reload. Compatible with any RESP-protocol server: Redis, Valkey, DragonflyDB, KeyDB, or Garnet. Namespace-aware key prefix prevents collisions when gateways with different `FERRUM_NAMESPACE` values share the same Redis cluster. Database-backed token counters are intentionally unsupported.
...
**Memory protection:** The plugin tracks per-client state in a `DashMap` capped at 100,000 entries. Resident entry count is maintained by an `AtomicUsize` coordinated with insert and remove (expiry prune and forced eviction), so steady per-datagram capacity checks never call `DashMap::len()` or take every state-shard read lock — including Redis mode, which still observes only the atomic local/fallback count. When the cap is exceeded, only datagrams from already-tracked client IPs are forwarded — datagrams from new IPs are dropped without inserting state. This strict result remains in force for the datagram that wins cleanup, so a spoofed-source-IP flood cannot alternate one insertion with one full-map eviction and bypass the guard. Stale entries (idle for `window_seconds × 2`, minimum 10 seconds) are pruned by a sampled periodic sweep (every 100,000 datagram hooks) even while the map is below the hard cap; over-cap observations force-evict remaining keys at most once per second after that prune. Both sweep paths share an atomic `compare_exchange` gate so concurrent scans cannot pile up under load, and Redis-fallback local maps use the same below-cap prune / over-cap force-eviction split.
