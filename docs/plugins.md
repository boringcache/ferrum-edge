...
- **Native gRPC always runs frame validation.** A zero-length transport body, or anything shorter than the five-byte gRPC frame header, is rejected. The single exception is a gRPC **Trailers-Only** reply, whose complete terminal metadata rides in the initial HEADERS block and which legitimately carries no message.
...
Media types outside `content_types` / `response_content_types`, and allowlisted media types no configured rule can inspect, still pass through untouched — the advisory closes fail-open holes without widening applicability. Request-side inspection reads the raw buffered bytes rather than the shared UTF-8 string copy, so a body the gateway could not represent as text can no longer look like "no body at all".
