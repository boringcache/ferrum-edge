...
- `sampling.max_records_per_minute` must still have budget in the current window. The limiter decision is therefore taken when the backend-visible body arrives rather than at enqueue time: a saturated instance stops paying capture CPU and stops retaining excerpts for records it would drop, and those transactions report `sink_status: dropped` exactly as a limiter drop at enqueue did. A record whose capture was skipped is never emitted as a hash-less envelope even if the window rolls over mid-transaction.
...
- *Records-per-minute is decided at capture admission.* A transaction that finds the window already saturated is dropped at that point rather than re-checked at enqueue, so a record that would have squeezed into the next window boundary is dropped instead. This is the trade that lets a saturated instance stop paying capture CPU; the per-window ceiling itself is unchanged, and the window is anchored to the process-monotonic clock.
