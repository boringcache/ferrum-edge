...
`AgentCard.url` (3) is held to a stronger proof: it must parse as an absolute `http`/`https` URL with a real host and no embedded credentials, bounded to 4096 bytes and free of ASCII whitespace and control characters. A prefix test would not do — `https://`, `https://:8080/a2a`, and `http://` followed by a control character all begin with those bytes without being a URL, and a serialized `AgentInterface` (which is what field 3 holds in the A2A 1.0 layout) is frequently valid UTF-8, so this check is what actually distinguishes a URL string from a submessage. A value that fails it is treated as a layout mismatch. The proof is the WHATWG grammar rather than a stricter spelling rule, so a form the standard accepts is accepted here too — `http:///a2a` is host `a2a`, because the special-authority-ignore-slashes state consumes the extra slash. That is safe: only the verdict is used, a preserved URL is re-emitted as the backend's own bytes rather than a normalized form, and a discovery client resolves it under the identical rule.
