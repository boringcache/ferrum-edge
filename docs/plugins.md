...
- **The lookup is pre-routing, and that is handled structurally rather than argued away.** This plugin short-circuits ahead of every route-dispatch plugin, so the destination it binds is the pre-dispatch one. Binding the inputs those plugins route on — method, authority, path, raw query, request headers, and the exact backend-visible request body — does *not* make that equivalent to the post-dispatch destination: a route decision is a function of the request **and** the live route policy, and a deduplication record outlives that policy (it is read back after a configuration reload, after a restart, and under `sync_mode: redis` by replicas on different generations). Two byte-identical requests separated by a route policy change select different destinations while presenting equal fingerprints. Rather than assume that away, every plugin that can still change the destination after the lookup is refused at composition time (see below)
...
- **`request_deduplication` cannot be enabled on the same proxy as any route-dispatch plugin.** `ai_stream_router`, `mcp_gateway`, `a2a_gateway`, and `mesh_route_dispatch` all select or rewrite the effective destination *after* deduplication has already decided whether to replay, and nothing available at lookup time witnesses the route policy they will apply. The composition is rejected at config admission (admin API `400`, fatal in file mode, rejected update in DP) and again at plugin-cache construction, with an error naming both plugin config IDs and the proxy. Deduplication's priority is deliberately not moved after them: it exists to refuse re-execution of a side effect *before* any of it happens, and a late lookup would defeat that. Disable one of them on the proxy; to keep both behaviors, put them on separate proxies
...
- **Plugin ordering and prompt privacy**: This plugin runs at priority 2996, after request size/rate limiting, `ai_prompt_shield`, WAF, body/OpenAPI validation, `ai_request_guard`, and route-dispatch plugins (`ai_stream_router`, `mcp_gateway`, `a2a_gateway`, `mesh_route_dispatch`), but before request transformers and `ai_federation`. Exact/semantic keys therefore bind the shared canonical effective destination partition (including the proxy namespace) and the backend-visible request context that will serve a miss. The only request headers excluded from that binding are transport/hop-by-hop framing fields the backend never receives as sent (`Connection`, `Keep-Alive`, `Proxy-Connection`, `TE`, `Trailer`, `Transfer-Encoding`, `Upgrade`; `Host` is bound separately as the canonical authority) and per-request tracing/correlation identifiers (`traceparent`/`tracestate`, `b3`, all `X-B3-*`, `X-Request-Id`, `X-Correlation-Id`, `Correlation-Id`, `X-Trace-Id`, `X-Amzn-Trace-Id`), which carry a fresh value on every request by construction and would otherwise make every entry a singleton. Every other header — custom, tenancy, routing, versioning, representation — is a key dimension, so a conservative miss is preferred over an unbound dimension. Exact cache hits and semantic embedding calls see the backend-visible request body after earlier admission plugins have accepted or transformed it. Cache hits still consume request-count rate-limit budget and pass through earlier admission/fault-injection plugins before short-circuiting. Semantic mode still sends prompt text to the configured embedding endpoint; use an approved provider or private/auth-aware embedding proxy for sensitive prompts.
