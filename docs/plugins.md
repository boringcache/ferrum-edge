...
>
> **Prompt estimate coverage (fail-closed whole-body walk).** Pre-dispatch prompt characters are counted by walking the already-parsed request JSON once, summing every billable string value, every visited object member name (tool/function JSON Schema property names and nested schema keys are billed by providers as prompt input), and JSON scalar literals (`null` / `true` / `false` / numbers at their serialized widths, so large schemas cannot omit numeric/`boolean` keywords at scale). That single pass covers known billed shapes (chat/`messages`, Responses `input`/`instructions`, Gemini `contents`/`systemInstruction`, Cohere `message`/`chat_history`/`preamble`/`documents`, TGI `inputs`, Titan `inputText`, Anthropic/Bedrock `system`, tool/`functions` schemas, RAG fields, Azure "On Your Data" `role_information`, and any other nested prose) **and** unknown provider-native textual siblings — a present recognized field never suppresses an unknown sibling, and each visited string value, member name, or scalar literal is counted at most once (distinct alias keys over-reserve rather than omit). The estimate is a conservative `chars/4` reservation heuristic, not provider tokenizer parity. Exclusions are **shape/value aware**, not name-only: the limiter skips **numeric** token-cap/control members (`max_tokens` / `max_completion_tokens` / `max_output_tokens` and the other recognized output-cap key spellings when the value is a JSON number), **structurally recognized inline binary payloads** (OpenAI `image_url` / `input_audio` / `file_data` and Gemini `inline_data`/`inlineData` values that are data/http(s) URLs, media-typed/`format` `data` objects, or long base64 payloads — including member names nested under those skipped subtrees — and any well-formed `data:` URL — `data:[<mediatype>][;base64],<payload>`), and Anthropic **binary** `source` blocks whose `type` is `base64`, `url`, or `file`. A tool-schema property that merely reuses one of those spellings (e.g. `parameters.properties.max_tokens` with a long `description`, or `properties.image_url` / `properties.source` with `type:"string"`) is counted fail-closed. An Anthropic **text** document block (`source: {type: "text", media_type: "text/plain", data: …}`) is counted as real prompt input. Ordinary prose that merely begins with `data:` (e.g. a chat message `"data: my notes"`) lacks the structural `,` separator and is still counted as text.
