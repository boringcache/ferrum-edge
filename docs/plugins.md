...
Neither mode is an all-must-pass (AND) chain. `single` and `multi` differ only in how a rejection is handled: `single` returns the first plugin's rejection immediately, while `multi` keeps trying later mechanisms and only rejects if none establishes a principal. Both stop at the first mechanism that establishes a principal, so a proxy cannot be configured to require two mechanisms to both succeed. Do not read `auth_mode: multi` as multi-factor authentication.

Because a principal can nevertheless be established by more than one code path, every mechanism that reaches a request principal commits through one shared boundary that enforces same-principal binding. If a factor is accepted while a *different* canonical principal is already committed for the request, the request is rejected with `403` before the authorization phase rather than retaining one identity and discarding the other. Principals are compared canonically, never by display name: a gateway Consumer by its namespace plus stable Consumer ID, and an external principal by the canonical identity string its mechanism verified (issuer + subject, SPIFFE ID, or equivalent). A Consumer and a separately presented external identity are never assumed to be the same person; they are bound only when a single credential established both. The same rule guards stream (TCP/TLS, UDP/DTLS) `on_stream_connect` authentication.
