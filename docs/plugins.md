...
`mode: monitor` with zero enforcing rules remains valid. SSRF metadata/private-IP
and dangerous-scheme signatures cover both request bodies and decoded query
values at paranoia 1 (`FE-SSRF-001`/`002` and the `-Q` mirrors); see
