...
**Response reverse mapping.** For routed `resources/read`, `tools/call`, and `prompts/get` requests, buffered JSON-RPC results use the same per-session catalog bindings as request routing to map upstream resource URIs back to public `mcp://{server}/...` URIs. This covers `resources/read` `contents[].uri`, tool-result `resource_link` items, embedded resource content, and resource content returned by prompts. Tool/prompt routing also loads resource-template bindings for the selected upstream on first use so dynamic resource links can be routed back through the gateway without unrelated servers delaying the call. Unknown or ambiguous bindings, a catalog refresh racing an in-flight response, encoded bodies, and malformed JSON pass through unchanged. Routed calls strip `Accept-Encoding` to request inspectable identity JSON; an upstream that still returns an encoded response is released without buffering. Origin validators and integrity hashes are removed from responses eligible for rewriting because they describe the upstream-native body. Internal response-rewrite bookkeeping is excluded from transaction metadata even when MCP metadata emission is enabled. `text/event-stream` responses are never buffered or rewritten, and JSON responses larger than `validation.max_upstream_response_bytes` (default 4 MiB) pass through unchanged.
