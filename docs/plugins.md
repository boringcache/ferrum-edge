...
- **Semantic index refresh**: The local HNSW snapshot is immutable and rebuilt in batches using a blocking worker thread for HNSW construction. The first semantic entry is indexed immediately; later semantic inserts/removals mark the snapshot dirty and rebuild at most once every 30 seconds. Reads also check whether a dirty index is due for refresh before semantic lookup. Very recent entries may therefore exact-hit before they become eligible for semantic hits.
- **Plugin ordering and prompt privacy**: This plugin runs at priority 2980, after request size/rate limiting, `ai_prompt_shield`, WAF, body/OpenAPI validation, and `ai_request_guard`, but before `ai_federation`. Exact cache hits and semantic embedding calls therefore see the backend-visible request body after those admission plugins have accepted or transformed it. Semantic mode still sends prompt text to the configured embedding endpoint; use an approved provider or private/auth-aware embedding proxy for sensitive prompts.
