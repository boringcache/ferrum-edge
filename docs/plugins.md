...
**JSON-RPC batches.** A top-level JSON array is admitted as a JSON-RPC 2.0 batch. Empty arrays are rejected with a single `-32600 Invalid Request` response (not an array). Before session, catalog, or upstream work, the gateway enforces `validation.max_batch_bytes` (default 1 MiB) on the aggregate request body and `validation.max_batch_items` (default 32) on member count. Each member is then validated independently for object shape, `validation.max_batch_item_bytes` (default 256 KiB), and the same method/session/schema/tool-policy rules as a singleton: non-object or nested-array members yield a bounded `id:null` Invalid Request while valid siblings continue. Response-bearing results preserve JSON-RPC `id` values and input order; gateway-handled notifications produce no response entry; an all-notification batch of gateway-handled members returns HTTP 202 with an empty body. Assembled batch responses are bounded by `validation.max_batch_response_bytes` (default 1 MiB) and fail closed without returning a partially oversized body.

In `aggregate_router` mode, only gateway-handled (synthetic) members are executed inside the batch: `initialize`, catalog lists, `ping`, denied tool policy results, and other terminal gateway responses. Members that would route to an upstream MCP server (`tools/call`, `resources/read`, `prompts/get`, and passthrough unknown methods) are **not** dispatched from `before_proxy` — that path would bypass later configured plugin phases (`a2a_gateway`, `mesh_route_dispatch`, `ai_semantic_cache`, request transformers, final request-body hooks, and ordinary proxy response phases) and receive weaker policy than the equivalent singleton. Those members return JSON-RPC `-32009` and must be issued as singleton requests so the full plugin chain applies. Upstream-bound notification-only batches similarly fail closed (they do not return empty 202). Multiple `initialize` members that mint distinct downstream sessions in one batch fail closed with `-32010` because one HTTP response header cannot represent multiple new sessions; a session minted by an earlier initialize is propagated to later members through the configured `sessions.downstream_session_header` (matched case-insensitively) without accepting forged client or upstream session ids.

In `transparent_proxy` mode, every member is validated independently; when all members are valid MCP request/notification envelopes the original batch body Continues to the single upstream through the normal proxy/plugin chain. If any member is invalid, forwarding a partially rewritten batch is impossible, so the gateway returns a synthetic per-item error array (invalid members get Invalid Request; valid siblings get a bounded not-forwarded error) and does not dial upstream. Changing these limits on reload/update reconstructs admission; deleting the plugin removes batch handling with the instance.
