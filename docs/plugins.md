...
An admitted card is then **schema-validated before any field is rewritten**. Every known 0.3 field — `name`, `description`, `url`, `preferred_transport`, `additional_interfaces` (including each `AgentInterface`'s `url` and `transport`), `protocol_version`, and `signatures` — must carry the wire type the 0.3 schema declares, and each singular field may appear at most once; a wrong wire type or a duplicated singular field fails closed rather than being preserved verbatim beside rewritten siblings. As defense in depth, a URL field that is not an absolute `http`/`https` URL is treated as a layout mismatch. Unknown fields with any valid wire type are preserved untouched. The protobuf decoder is strict about hostile framing too: over-long, over-range, and non-canonical (redundantly padded) varints, field numbers of `0` or above the protobuf maximum `2^29 - 1`, out-of-range length prefixes, and the group wire types are all rejected rather than truncated or resynchronized against.
...
| Version/layout | `agent_card_protobuf_url_layout_mismatch` | A rewritable URL field is not an absolute `http`/`https` URL, so the layout is not 0.3. |
...
| Schema | `agent_card_protobuf_string_invalid` | A string field the rewriter reads is not valid UTF-8. |
