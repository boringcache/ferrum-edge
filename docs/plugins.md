...
*Bounded lifetime and teardown.* An attached listener lives at most `sessions.sse_listener_max_lifetime_seconds` (default 300) and emits a keepalive comment every `sessions.sse_keepalive_seconds` (default 15), which is how a vanished peer is discovered and how an orphaned listener is always reclaimed. Teardown never uses a best-effort lock: client disconnect (dropping the response body), session `DELETE`, TTL expiry, cap eviction, and dropping the plugin instance on reload/update/delete all end the stream. Broker session cardinality mirrors the MCP session store exactly — `initialize` admits both under one critical section and **fails closed** with JSON-RPC `-32013` rather than minting a session that could never attach its stream.

Duplicate listeners for the same session return `409`; missing/unknown/stale sessions return the ordinary `404` that prompts re-initialization. Diagnostics are fixed low-cardinality tokens in `mcp.sse.error` / `mcp.sse.cancel` — never a session id, JSON-RPC id, header value, or event payload. Transparent mode continues to forward `GET` upstream unchanged, and with `sse_multiplexing: false` aggregate `GET` keeps its `405`.
