...
| `invalidate_on_unsafe_methods` | bool | `true` | After a non-error response (status below 400) to an unsafe method (`POST`, `PUT`, `PATCH`, `DELETE`, and any extension/custom method, which fails closed as unsafe), invalidate cached entries for the same matched proxy, normalized/transformed Host/authority partition, and path prefix (including descendants). Safe methods that are not in `cacheable_methods` (such as `OPTIONS`, or `HEAD` under a GET-only set) bypass without invalidating. Error responses and requests that never receive a final non-error status do not invalidate |
...
- Unsafe methods (`POST`, `PUT`, `PATCH`, `DELETE`, and unrecognized extension/custom methods, which fail closed as unsafe) invalidate cached entries for the matched path under the **same normalized/transformed Host/authority partition used by cache lookup** when `invalidate_on_unsafe_methods` is enabled. Invalidation is deferred until `after_proxy` observes a non-error response (status below 400), matching RFC 9111 §4.4 — a failed or unauthorized mutation, or any 4xx/5xx, does not evict. Safe methods that are absent from `cacheable_methods` (such as `OPTIONS`, or `HEAD` under a GET-only set) bypass without invalidating.
