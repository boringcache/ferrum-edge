...
>
> **Prompt estimate coverage (fail-closed whole-body walk).** Pre-dispatch prompt characters are counted by walking the already-parsed request JSON once, summing every billable string value **and** every visited object member name (tool/function JSON Schema property names and nested schema keys are billed by providers as prompt input). That single pass covers known billed shapes (chat/`messages`, Responses `input`/`instructions`, Gemini `contents`/`systemInstruction`, Cohere `message`/`chat_history`/`preamble`/`documents`, TGI `inputs`, Titan `inputText`, Anthropic/Bedrock `system`, tool/`functions` schemas, RAG fields, Azure "On Your Data" `role_information`, and any other nested prose) **and** unknown provider-native textual siblings — a present recognized field never suppresses an unknown sibling, and each visited string value or member name is counted at most once (distinct alias keys over-reserve rather than omit). The estimate is a conservative `chars/4` reservation heuristic, not provider tokenizer parity. The limiter **excludes** token-cap/control fields (`max_tokens` / `max_completion_tokens` / `max_output_tokens` and the other recognized output-cap key spellings), **inline binary payloads** (base64 `image_url`/`inline_data`/`input_audio` data, binary `source` blocks — including member names nested under those subtrees — and any well-formed `data:` URL — `data:[<mediatype>][;base64],<payload>`), so multimodal/vision requests are not falsely rejected by oversized image bytes. An Anthropic **text** document block (`source: {type: "text", media_type: "text/plain", data: …}`) is the exception — its prose is real prompt input the provider bills, so it **is** counted, while binary image/PDF `source` blocks are still skipped. Ordinary prose that merely begins with `data:` (e.g. a chat message `"data: my notes"`) lacks the structural `,` separator and is still counted as text.
