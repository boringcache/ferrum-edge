...
| Parameter | Type | Default | Description |
|---|---|---|---|
| `db_path` | String | (required) | Path to a MaxMind `.mmdb` file no larger than 512 MiB. The declared sizes of all distinct MMDB paths in one validation generation, the snapshots retained by a resulting cache generation, and the peak of live plus in-flight candidate snapshots must each total no more than 512 MiB. |
