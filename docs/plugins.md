...
**Never blocks by default.** Enqueue is non-blocking; a full buffer or a failing sink drops records (warned) unless the operator opts into `on_buffer_full: reject` / `on_sink_error: reject`. Buffered-response records fail with `503` at emission time. An AI request already selected to emit a success record (sampling winner or known request-side guardrail override) while streaming capture is enabled is preflighted during staging and after final request-body hooks; it fails with `503` before a downstream short-circuit or backend dispatch when the queue is already full or the sink is already unhealthy. This covers explicit `stream: true`, `Accept`-driven SSE, and providers that choose SSE themselves, while streaming capture that is disabled or intentionally not sampled keeps its configured behavior. Error and response-side guardrail overrides are not knowable at request preflight, and a sink or queue failure that begins afterward cannot retract an already-committed stream. Under `on_sink_error: reject`, **any** non-2xx collector response marks the sink unhealthy — including non-retryable 4xx (401/403/413, e.g. an expired sink token) whose batch is discarded rather than retried, so records silently lost at the collector stop subsequently audited traffic instead of letting it flow unaudited. Rejected transactions still build and enqueue their audit record — the background flush of those records is the recovery probe, so a successful batch send automatically restores sink health and stops the rejects. **Safe defaults:** `redacted_body`, no raw headers, no client IP, 64 KiB body caps.
