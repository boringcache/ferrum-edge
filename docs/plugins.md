...
In CP mode, full-load validation claims any locally validated MMDB handoff only for the duration of control-plane snapshot validation and then releases it. The non-proxying control plane never retains the snapshot waiting for a plugin-cache build that cannot occur.
