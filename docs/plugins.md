...
**Response reverse mapping.** For routed `resources/read`, `tools/call`, and `prompts/get` requests, buffered JSON-RPC results use the same per-session catalog bindings as request routing to map upstream resource URIs back to public `mcp://{server}/...` URIs. This covers `resources/read` `contents[].uri`, tool-result `resource_link` items, embedded resource content, and resource content returned by prompts. Unknown or ambiguous bindings and malformed JSON pass through unchanged. `text/event-stream` responses are never buffered or rewritten, and JSON responses larger than `validation.max_upstream_response_bytes` (default 4 MiB) pass through unchanged.
