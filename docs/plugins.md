...
- **Framing outside the contract.** A response labeled `application/grpc` on a request that is *not* native gRPC has no enrolled method naming its message type, and the protobuf rewrite path declines it. Its length-prefixed frames are not a JSON/SSE/text document either, so `scan_fields: all` must not scan them as raw text and promise a redaction that can never be applied: enforcing actions reject and `warn` records `grpc_framed_response_requires_grpc_contract`, matching what content mode already did for the same response.
