...
The **provider identity** is semantic, not positional. It is a digest of the provider's token trust anchor — its `issuer` plus its single configured JWKS source (`jwks_uri`, `discovery_url`, or canonicalized inline `jwks`) — so:
...
- two providers that share an issuer and JWKS source are one trust anchor and deliberately converge on one lane, so a duplicated provider entry cannot launder a second acceptance of one proof;
- providers with different issuers or different JWKS sources stay fully isolated from each other;
- remote `jwks_uri` / `discovery_url` values are bound by their canonical parsed form (scheme, host, default-port-normalized port, path, query), so equivalent spellings accepted by URL parsing — host case, an explicit default port — stay on one lane. Issuer matching remains **exact** and is not URL-canonicalized.

Security-irrelevant configuration — `audiences`, `required_scopes`, `required_roles`, claim/header mappings, token locations, `forward_original_token`, `jwks_max_stale_seconds`, `dpop_replay_max_entries`, `dpop_replay_scope`, and `dpop_clock_skew_secs` — is deliberately **not** part of the identity. Binding it would make an ordinary authorization edit reopen every live proof, which is strictly worse than the isolation it would buy. The fixed retention horizon below already dominates the widest admissible clock skew, so a widened skew can never outrun a marker.
...
| `replay_max_entries` | usize | `100000` | Capacity this `process` authority enforces against the shared replay lane. At capacity only expired markers are reclaimed; if none can be, the request is refused. An equivalent reload applies the new cap without forgetting live markers |
