...
plugin_configs:
  ...
  # Proxy — targets public-frontend via proxy_id; must also appear under that proxy's plugins
  - id: frontend-cors
    plugin_name: cors
    scope: proxy
...
When this plugin generates a header because the client omitted a valid inbound value (or the inbound value was rejected and replaced), that generation is recorded in the same private request-scoped correlation state. `response_caching` omits only that generated header/value from the shared-cache key and Vary digest so otherwise-identical anonymous GETs can HIT, while still forwarding the header upstream and echoing it downstream as configured. A client-supplied value on the same header remains an origin-visible cache dimension. Public metadata and extra request headers cannot mark a value as generated. Provenance is bounded by the number of admitted instances, is not logged as a separate marker, and is dropped with the request context (including retry clones of that context).
...
> **Local counters survive a compatible reload.** Local (and Redis-fallback) enforcement state is owned by the policy — `namespace` + plugin type + plugin-config id — not by the plugin instance a config generation happened to construct. A full reload (file-mode `SIGHUP`, a data plane receiving a full control-plane snapshot, the database-mode fallback after a rejected delta), a change or deletion of *any* global plugin config in the namespace, an edit to the owning proxy's plugin associations, or a rebuilt shared HTTP client all reconstruct the plugin instance; none of them resets a consumed budget any more. Compatibility is decided on the **effective enforcement semantics after parsing**, never on raw configuration syntax: an omitted optional field and its explicit effective default, a spelling the parser normalizes (case, surrounding whitespace, a gRPC method path with or without its leading `/`, `spiffe` vs `spiffe_identity`, `requests_per_minute: 60` vs `window_seconds: 60` + `max_requests: 60`), and an omitted rate map vs an explicit empty one all describe one budget and keep one set of counters. A **semantic** change does start fresh state, so a retired generation can never charge against the replacement: for `rate_limiting` the parsed limit dimension, the default window list, and the per-consumer window lists; for `graphql` the limit dimension and the parsed type/operation rate maps; for `grpc_method_router` the limit dimension and the normalized per-method rate map; for `ai_rate_limiter` the effective `token_limit`, `window_seconds`, `count_mode`, `limit_by`, `provider`, and `on_unmetered_response`; for `ws_rate_limiting` the effective sustained frame rate and burst capacity; for `udp_rate_limiting` the effective per-window datagram and byte budgets and the window length, where an unbounded axis is distinct from any bounded one. Whether a centralized store is enabled (`sync_mode`) is part of that set for every one of them, and when it *is* enabled so are the effective `redis_failure_policy` and the effective `redis_key_prefix`, because they decide whether the local map may admit at all during an outage and which counter domain it belongs to. With `sync_mode: local` there is no centralized store to lose, so changing `redis_failure_policy` alone changes nothing that is enforced and does **not** reset the local budget. Presentation-only fields (`expose_headers`, `close_reason`) and stateless checks (GraphQL depth/complexity/alias limits, gRPC `allow_methods`/`deny_methods`) are deliberately *not* part of the set — changing them must not hand every caller a fresh budget. Retention is bounded: state is held weakly and released as soon as the last plugin instance using it is dropped, so a removed policy is reclaimed and a policy re-added later starts fresh. Credentials, URLs, usernames, passwords, and TLS material are never part of this decision and never logged. This applies to `rate_limiting`, `graphql`, `grpc_method_router`, `ai_rate_limiter`, `ws_rate_limiting`, and `udp_rate_limiting`.

The default Redis prefix is already determined by the stable policy identity, so it is not hashed again as a compatibility input; this keeps validation and runtime construction equivalent even when their shared HTTP clients carry different namespaces. An explicitly configured `redis_key_prefix` remains a compatibility input and isolates local fallback state. When a semantic change creates a fresh local generation alongside a live one, Ferrum emits one structured info log carrying only `namespace`, `plugin`, and `config_id`; the fingerprint is never logged.
...
- **Local-mode state survives plugin-cache reload generations.** Active idempotency leases, retained completed responses, execution barriers, and their accounting are owned by a stable policy identity (`namespace` + plugin-config id), not by the plugin instance. A routine configuration reload that rebuilds the affected proxy/global plugin list hands the replacement instance the *same* local state, so an in-flight request cannot have its duplicate admitted against an empty map, and a completed entry is not forgotten (GHSA-wmqm-6mxj-gm9p). A scope move (`global` ↔ `proxy` ↔ `proxy_group`) that keeps the resource id keeps the state; two tenants that reuse a bare plugin-config id never share it. Changing `header_name`, switching between `local` and `redis`, or changing `on_redis_unavailable` is a semantic change: the replacement isolates onto fresh state, and the retired generation's late completion lands on the state it took its lease from rather than corrupting the new policy. `ttl_seconds`, `inflight_ttl_seconds`, `max_entries`, `max_entry_size_bytes`, and `max_total_size_bytes` are compatible changes that do not reset the protection domain. Each admitted operation, completion, and execution barrier retains the protection window under which it was created, so shortening `inflight_ttl_seconds` cannot make a replacement generation re-execute an older live operation; new operations use the replacement settings. The registry retains every still-live semantic generation, so an A → B → A policy sequence recovers A's active protection domain instead of admitting against a new empty state. Retention is bounded: an identity's state lives only while a configured instance or an in-flight request still holds it, and removing the policy releases it
...
- **`request_deduplication` and `a2a_gateway` cannot be enabled on the same proxy while `a2a_gateway` derives its public base from the request** — that is, with `discovery.public_base_url` unset and `discovery.trust_forwarded_headers: true`. The refusal is per instance, not by name: with a configured `discovery.public_base_url` the Agent Card rewrite is a pure function of configuration, enrolls in the static digest above, and composes normally. In the request-derived mode the rewritten origin comes from `X-Forwarded-Proto` / `X-Forwarded-Host` / `Host` and, absent a forwarded scheme, from whether the connection carried a TLS SNI hostname; the fingerprint below binds neither the complete forwarded header set nor the transport's SNI, so two requests sharing a replay key can legitimately deserve cards advertising different origins. Set `discovery.public_base_url`, or disable one of them on the proxy
...
| `forward_headers` | String[] | `[]` | Header names to forward to the function (lowercased at config load). Read from the effective `before_proxy` header view, so a field line an earlier plugin removed — `authorization` under `strip_authorization_on_success`, or a gateway-owned `claim_headers` destination with no verified claim — is absent from the payload rather than re-read from the client's original request. `x-real-ip` is additionally dropped whenever the immediate socket peer is not in `FERRUM_TRUSTED_PROXIES`, matching the primary backend request — the payload's `client_ip` field already carries the gateway-resolved address |
...
**Route-level header precedence:** when `mesh_route_dispatch` publishes `request_transform` rules onto `RequestContext`, every enabled `request_transformer` instance on the proxy applies only its static header/query rules first. Proxy core then applies the matched route list exactly once after the last eligible (enabled) instance, so route-level set/add/remove wins over later static conflicts under multiple same-type instances and priority overrides. A disabled RTDS instance is not eligible and does not consume the list. Setting `apply_route_overrides: true` allows an empty `rules` array so the VirtualService translator can auto-emit an eligible consumer when no operator transformer exists. That exact translator-owned consumer is additive to a same-name global transformer, so global static rules remain active; ordinary proxy-scoped transformers keep the normal proxy-overrides-global contract.

**Query mutation contract:** query rules mutate an ordered, duplicate-aware representation of the retained raw wire query (not the single-value `query_params` map). Authentication-owned credential strips (`auth.strip_query_param.*`) are removed from that representation **before** any query rules run, so a rename/update/duplicate cannot relocate or re-encode an authenticated secret onto a new outbound name. The serialized result is published as the outbound query for every primary backend URL builder (H1/H2, native H3, gRPC, WebSocket/CONNECT, cross-protocol adapters, and retries/failover). `request_mirror` snapshots that same funnel and then drops deny-by-default sensitive query pairs from the **mirror** request-target only. The proxy applies the same strips again afterward as defense in depth when composing the canonical final outbound query. Unmodified pairs keep their original percent-encoding, ordering, duplicates, key-without-equals flags, empty values, and literal `+` bytes. Authored names/values use RFC 3986 percent-encoding (`%20` for spaces, never `+`). Duplicate-name semantics (decoded names): `add` appends only when absent; `update` rewrites every matching pair (or appends when absent); `remove` drops every matching pair; `rename` renames every matching pair while preserving value encoding and key-without-equals shape. Configured query `key` / `value` / `new_key` strings must not contain CR or LF. Debug diagnostics log parameter names only — never values. When no query rule mutates the request and no auth credential was stripped from the transform input, the raw wire query is preserved with no extra allocation or re-parse.

**RTDS runtime gate (mesh only).** Setting `runtime_overlay_scope: "<scope>"` lets an operator disable this instance from the mesh runtime overlay via `ferrum.request_transformer.<scope>.enabled`. `false` short-circuits every header, query, and body rule and removes the instance's request-header-copy and body-buffering capabilities for that generation; the disabled instance is also ineligible for route-header finalization, so it cannot suppress route overrides needed by a later enabled consumer. A scope the accepted overlay does not name falls back to `default_enabled` (default `true`, so adopting RTDS is fail-open).
...
Once the backend headers *do* arrive, a representation the body transform provably declines is released to the streaming path instead of being collected up to the response-body ceiling and rejected unread (`GHSA-pwcm-6rh8-f2gh`). The release condition is the exact negation of what `transform_response_body` acts on, so eligibility is unchanged — only the moment the decision is taken moves earlier. Released: any **declared** non-JSON media type (binary downloads, `image/*`, `video/*`, `application/octet-stream`, `text/event-stream`) and the framed gRPC `+json` flavors. Not released, deliberately: an **absent** `Content-Type` (the transform treats it as JSON, so those bytes are still inspected and an untyped unparseable document still fails closed), and a non-identity `Content-Encoding` over a JSON type (releasing opaque wire bytes would forward a representation past a configured redaction instead of failing closed). The downgrade is refused for this plugin whenever a later hook may rewrite `Content-Type` — it has no release it can prove independent of the final label — and a translated gRPC-Web response whose terminal metadata this instance polices stays buffered.
...
Request-side validation buffers by **configured representation, never by request method**. Any request whose `content-type` matches `content_types` (or, for protobuf configs, is `application/grpc`) is buffered and validated, including `GET`, `HEAD`, `OPTIONS`, `DELETE`, `PUT`, `PATCH`, and non-standard methods. Response-only configs do not force request buffering, and a protobuf-only config does not claim ordinary JSON/XML media types.

**Fail-closed representation (`GHSA-2vmr-ww8r-mww3`).** Once a configured rule applies, the plugin never degrades to "pass through" because the data was awkward:

- **Empty bodies are validated, not skipped.** An empty JSON or XML document is invalid and is rejected (`400` request, `502` response). An empty protobuf *message* inside a well-formed five-byte gRPC frame is structurally valid for proto3 and is validated normally.
- **Native gRPC always runs frame validation.** A zero-length transport body, or anything shorter than the five-byte gRPC frame header, is rejected. The single exception is an empty terminal **gRPC error** reply whose hook-visible header/trailer metadata carries exactly one valid non-zero `grpc-status` (`1..=16`) and which legitimately carries no message. `grpc-status: 0`, malformed values, and LF/CR-joined duplicates are not exemptions — a successful unary response, including `google.protobuf.Empty`, still requires a five-byte frame.
- **Non-UTF-8 JSON/XML is rejected**, with a fixed diagnostic (`Request body is not valid UTF-8` / `Response body is not valid UTF-8`) that never reproduces, logs, or echoes any of the offending bytes.
- **A missing request representation fails closed.** If the plugin selected buffering but no buffered body and no transport proof of emptiness reached the hook, the request is rejected with a fixed, value-free diagnostic and an operator `warn` that records no request, header, or body value.
- **Response no-body exemptions are explicit and protocol-correct.** Only `1xx`, `204`, `205`, `304`, and responses to `HEAD` may legitimately carry no content. An ordinary body-bearing success such as `200` with an empty body is a validation failure, not an exemption.
...
except the narrow implemented entry-operation set described below and any
gateway-minted `correlation_id` header/value, mandatory caller-authorization
context, and the complete `Vary` tuple.
...
**Cacheability predictor**: The plugin maintains a bounded FIFO of cache-key variants that were observed to be uncacheable (e.g., responses with `Set-Cookie`, `Cache-Control: no-store`, `private`, or non-cacheable status codes). Subsequent requests for those same variants short-circuit the cache lookup with `X-Cache-Status: PREDICTED-BYPASS`, avoiding the `DashMap` read on the hot path. The predictor is keyed on the *full* cache key (including Vary dimensions), so an uncacheable variant for one `Accept-Encoding` value does not suppress lookups for other variants. When a previously uncacheable variant becomes cacheable (e.g., the backend stops sending `Set-Cookie`), the predictor clears the entry — on the next successful insert, and also on the `PREDICTED-BYPASS` itself, because that request releases the response buffer and therefore performs no store that could re-learn cacheability. Capacity is enforced by popping an insertion-ordered queue in amortized O(1); the predictor never clones and sorts its map at capacity, so high-cardinality uncacheable targets cannot force superlinear maintenance.

**Response buffering is released as soon as the store path is unreachable.** This plugin needs the complete body only when it is actually going to store one, so it does not pin a response onto the buffered path once that is provably impossible (`GHSA-pwcm-6rh8-f2gh`):

- Request-side, decided in `before_proxy`: a non-cacheable method, a request that carries a body, a refused replay partition, a request `Cache-Control: no-store`, a request that advertises `Accept: text/event-stream`, and a `PREDICTED-BYPASS` variant all stream the response instead of collecting it.
- Response-header-side, decided as soon as the backend headers arrive: **every** refusal in the store path is decidable from status plus response headers, so every one of them releases the body. That covers a status outside `cacheable_status_codes`, a status whose caching semantics this plugin does not implement, a `Content-Range` partial answer, `Cache-Control: no-store`/`private`/`no-cache`, a zero freshness lifetime (including a configured `ttl_seconds: 0`, where it applies to every response), a `Set-Cookie` response, a response to an authorized request without an explicit shared-cache opt-in, a representation already stale on arrival, and an unusable `Vary: *`. An origin-selected `text/event-stream` is released too, and is additionally never stored: retaining one holds the whole connection's bytes against the retained-response budget and produces nothing a later request could reuse. Only a response that is genuinely storable by every header rule is still buffered, because only then can the completed body change the answer (`max_entry_size_bytes`).
- Releasing those responses does **not** skip the eviction they imply. The header-only half of the store path — invalidating a superseded entry for a `no-store`/`private`/`no-cache`/zero-freshness/`Vary: *` response, and teaching the cacheability predictor — runs in the `on_final_response_headers` phase, which executes at the end of the `after_proxy` chain for the response the proxy actually selected, on streamed and buffered responses alike. Only that response applies these effects: an attempt a retry later discarded never reaches the phase.
- Because the phase, not the body hook, owns those effects, they also happen exactly once. When some other plugin keeps the body buffered for its own reasons, `on_final_response_body` consumes the phase's staged decision instead of repeating it — and refuses to store an event stream that another plugin's buffering happened to collect. The event-stream refusal reads the **backend-selected** representation, not the live header map, so a later hook that relabels the stream cannot turn it back into a storable entry. The staged decision is namespaced per instance, so multiple `response_caching` instances on one proxy stay isolated.
- The same predicate is the plugin's retry-time release vote, so a proxy whose only buffering plugin is `response_caching` no longer buffers every response merely because retries are configured. Under retries the shared refinement applies the `Content-Type` relabel guard *per plugin* over the hooks that still run after it, so adding a `response_transformer` that may rewrite `Content-Type` no longer pins a backend-selected event stream onto the buffered path for the life of the connection. The retry decision also walks the chain in configured order and projects the later built-in header rules — including a chain-level route response-header rule — so a response a later `no-store`/`private`/`no-cache`/`max-age=0`/`Vary: *` rule makes unstorable is released instead of collected and discarded unread. The projection is refused wholesale when any remaining plugin's header effects cannot be reproduced (a custom plugin), and it takes no effect of its own: it runs on a cloned context, so an attempt a retry later discards still evicts and marks nothing.

A stored entry body is a distinct allocation from the response the client receives, so it takes its own share of the aggregate retained-response budget (`FERRUM_RESPONSE_BUFFER_MAX_TOTAL_BYTES`) and holds it until the entry is evicted and the last in-flight replay of it is finished. If that budget cannot admit the copy, the store is skipped exactly like a `max_entry_size_bytes` refusal — the client still gets its response, the request is a plain miss, and no uncharged bytes are retained.

Two behavior notes follow from the above. A request carrying `Cache-Control: no-store` now stores nothing at all (RFC 9111 §5.2.1.5), where previously only the *lookup* was bypassed. And a `PREDICTED-BYPASS` retires its predictor mark in the same step, so the variant is re-learned on the following request rather than being permanently denied a store attempt; in steady state at most every other request for a persistently uncacheable variant pays for a buffer.
...
| `forward_sensitive_query` | Boolean | `false` | Dangerous opt-in. When `true`, selected origin-bound credential query names may cross to the mirror origin, but only exact decoded names listed in `forward_sensitive_query_allowlist`. Construction is fail-closed: both fields are required together and the allowlist must be non-empty. Does not change the primary backend request-target |
| `forward_sensitive_query_allowlist` | String[] | `[]` | Lowercased exact allowlist of denied sensitive query parameter names to forward when `forward_sensitive_query` is `true`. Each entry is matched against the fully percent-decoded (bounded staged decode, ASCII-lowercased) name (≤256 bytes, at most 64 entries) that the deny-by-default policy actually strips (a built-in credential or a `sensitive_query_patterns` match); non-sensitive names are rejected at construction |
| `sensitive_query_patterns` | String[] | `[]` | Additional lowercased substrings matched against each staged percent-decoded query parameter name that extend the built-in deny-by-default query credential set (each ≤128 bytes, at most 64 entries) |
...
**Query fidelity:** The mirror request-target snapshots the same canonical backend-visible query primary dispatch uses — the `request_transformer` outbound query when present, otherwise the original raw query, after authentication credential strips (`auth.strip_query_param.*`) — then applies a **deny-by-default query credential policy** to that owned snapshot only. Cross-origin query forwarding matches the header posture: a pair is stripped when any staged percent-decoded, ASCII-lowercased name form (raw plus every `percent_decode` layer through a hard cap of 4) is a delimiter-bounded built-in credential segment (`token`, `sig`, `signature` — so `oauth_token` and `x-amz-signature` / `x-goog-signature` are denied, while `continuation_token` and `signal` are not), contains a built-in credential substring (the header substring set plus delimiter variants such as `access_token` / `api-key` / `api+key` / `auth_token`), or matches an operator `sensitive_query_patterns` substring. Case variants, single-encoded, and nested-encoded names (`ACCESS_TOKEN`, `%61ccess_token`, `%2561ccess_token`) are classified at every decode layer; a name that still contains a valid `%XX` escape after the cap, or that is malformed / control-bearing / invalid UTF-8, is denied. A raw `&` segment containing `;` is dropped in full because mirror frameworks disagree on whether literal semicolons introduce another parameter; Ferrum never forwards a segment that the mirror might reinterpret as a hidden credential pair. Decoding is classification-only: retained pairs keep their original percent-encoding, ordering, duplicates, flags, empty values, and literal `+` bytes — they are not rewritten through a form encoder. Empty values and repeated keys of a sensitive name are dropped as whole pairs; an all-sensitive query yields a valid target with no `?` and no leftover `&`. Path and fragment are not taken from the query string. Forwarding a denied name requires the fail-closed pair `forward_sensitive_query: true` plus a non-empty exact decoded-name `forward_sensitive_query_allowlist`. The primary backend request-target is byte-for-byte unchanged. Later plugin mutation of the live context cannot change a snapshot already taken. Stripped names and values are never logged; mirror logs still omit the entire query. The materialised single-value `query_params` map is used only when neither a raw nor outbound query is available, and that fallback still applies the same strip.
...
config:
  ...
  # sensitive_query_patterns: ["x-vault-"]
...
Queue admission first reserves the complete maximum delivery charge, performs an allocation-free counting serialization under `max_entry_bytes`, then serializes a second time into an exact-capacity immutable `Bytes` payload and shrinks the reservation to the exact charge. The queue never retains the original owned record. The shared `BatchingLogger` wraps each flush batch in one immutable `Arc<Vec<QueuedAuditRecord>>` and Arc-clones that handle for every retry attempt and the failed-batch hook — never deep-cloning transcript strings, maps, or vectors. Each HTTP attempt assembles one exact-capacity JSON array from those already-escaped record bytes and sends it as a raw `application/json` body, so reqwest does not serialize or escape it again. Attempts run sequentially. The exact per-record lease is `2 × (serialized_record_bytes + 1) + 128`: one immutable queued payload, one share of the contiguous request body plus JSON-array framing, and a conservative fixed allowance for the queue item, shared allocation, shared batch Arc handle, and lease handles. Consequently `buffer_max_bytes` must be at least `2 × (max_entry_bytes + 1) + 128`. Admission is `(count AND bytes)` and the byte charge is taken **atomically before publication** in every retention state:
...
**Native gRPC capture.** Set `grpc` to enroll specific methods for schema-aware protobuf request/response capture. Without this block the plugin stays HTTP-only and never buffers native gRPC bodies. The block is closed (`descriptor_path`, `methods`, `max_message_bytes`, `max_messages`); each `methods` entry is closed too (`request_type`, `response_type`, `text_fields`) and must name at least one of `request_type` / `response_type`.
...
| `methods[].text_fields` | String[] | *(all strings)* | Optional dotted field paths restricting which string fields are exported. Each path must end at a string field and may not traverse a map field. Repeated fields retain every selected value under indexed paths (`tags.0`, `messages.2.text`). Omit to cover every string field recursively |
...
The field-name half of that contract is the *same* policy the HTTP JSON path applies, evaluated over protobuf structure rather than over a JSON tree, which matters in four places where the exported dotted label is not the name the policy must see:

- **Any ancestor, not just the leaf.** HTTP replaces the whole subtree under a sensitive object key, so `credentials.display_name` and `password_reset.hint` are redacted because `credentials` / `password_reset` match — not because the leaf does.
- **Repeated fields.** A `repeated string api_keys` exports as `api_keys.0`, `api_keys.1`, …; the numeric segment is a position, so the decision is made on the field name and every element inherits it.
- **Map keys.** Map keys are never exported as transcript labels (and `text_fields` cannot select them), but they *are* field names for redaction: a `map<string, string> metadata` entry keyed `authorization` exports under an ordinal path (`metadata.0`, `metadata.1`, … — collision-free, mirroring repeated-field indexing) with its value replaced when the key is sensitive, exactly as the equivalent JSON object key would be. Do not rely on a built-in pattern to catch a generic bearer token — the field-name policy is what covers it. Ordinals are assigned over a **deterministic canonical key order**, not over protobuf map iteration order, so the same message always produces the same labels and the same bounded truncation; the canonical key itself is internal to that ordering and to the redaction decision, and never reaches the record.
- **JSON inside a protobuf string.** A string field carrying tool-call arguments such as `{"api_key": "…"}` is decoded and redacted as JSON before export, so a sensitive key split away from its value cannot bypass the policy.

Framed bodies that are malformed, truncated, compressed with an unsupported encoding, exceed `max_message_bytes` / `max_messages`, exceed the aggregate redaction scan bound, fail protobuf decode, or carry unknown fields omit the excerpt with a compiled-in `request_body_omitted_reason` / `response_body_omitted_reason` (`grpc_framing_error`, `grpc_message_limit`, `grpc_scan_limit`, `grpc_decode_error`, `grpc_descriptor_unavailable`) — never a silent partial capture and never unredacted credential bytes. Descriptor shape is validated at Admin/CP admission without opening the node-local file; mode-aware file-dependency validation and runtime construction resolve the `FileDescriptorSet`. A readable-but-invalid descriptor rejects the candidate configuration; an absent/unreadable descriptor retains enrollment and omits body excerpts at request time.
  ...
  - name: ai_transcript_audit
    config:
      ...
      grpc:
        ...
        max_messages: 64
        methods:
          /assistant.v1.Chat/Complete:
            request_type: assistant.v1.CompleteRequest
            response_type: assistant.v1.CompleteResponse
  ...
  - name: ai_federation
    config:
      streaming:
        ...
        read_timeout_seconds: 0       # optional; 0–86400, 0 = unbounded.
                                      # Omit to inherit the selected provider's
                                      # own read_timeout_seconds.
      providers:
        - name: openai
...
**Streaming bounds and contract enforcement.** A claimed stream retains at most one SSE event — the partial event being assembled, or, once the provider frames its completion, the terminal event alone — capped by `streaming.max_event_bytes`, so retention grows with neither stream length nor transport chunk size: the ceiling is checked on every complete and partial event *before* any of its bytes are retained or released, so a hostile multi-megabyte chunk with no event terminator is refused without being buffered. The contract guard forwards ordinary events verbatim and in order, so time to first token is unaffected.

**The terminal `data: [DONE]` event is held back until the provider's body actually ends.** A client following the OpenAI streaming contract is entitled to stop reading at `[DONE]`, so a marker released before upstream EOF could be accepted as a successful completion while the gateway went on to refuse the stream — the client would never see the error. The guard therefore retains the provider's own terminal event (bounded by the same `max_event_bytes` ceiling) and releases it **verbatim** only at a clean EOF with nothing else outstanding. Any provider byte after that retained marker — a second terminal event, a data event, a comment / `id` / `retry` line, an oversized or non-UTF-8 block, or a trailing partial line at EOF — fails the stream closed and the marker is **dropped**: the client receives one gateway-authored terminal SSE `error` event and **no completion marker at all**. "EOF" here is the end of the *response body* — the terminating chunk on HTTP/1.1 or `END_STREAM` on HTTP/2 — not the end of the connection, and providers emit it immediately after `[DONE]`, so the hold costs one read and does not affect time to first token. A provider that emits `[DONE]` and then leaves the body open is bounded by the streaming read timeout.

A truncated stream (clean EOF with no terminal marker, or a trailing partial event) fails closed the same way. Terminal detection follows the SSE field grammar: an event is terminal only when the **concatenation of all its `data` lines** is exactly `[DONE]`, so `data: [DONE]\ndata: …` is ordinary data and cannot smuggle a payload past the post-terminal check. `CR`, `LF`, and `CRLF` line endings and their legal mixed blank-line forms are all recognized. An event that is not valid UTF-8, a **duplicate** terminal marker, an oversized event, a content-coded stream, and a 2xx that is not `text/event-stream` all fail closed with one gateway-authored terminal SSE `error` event — no invalid or post-terminal provider byte is ever forwarded. Every diagnostic is a fixed-cardinality gateway-authored string; no provider byte, header, model, or URL is ever echoed or logged. Terminal usage is relayed unchanged (usage arrives in its own event *before* `[DONE]`), so OpenAI-compatible clients should send `stream_options.include_usage: true` to receive it.

**Streaming accounting, model resolution, timeouts, and response headers.**

- **Concurrency.** A claimed stream takes one non-queuing `max_concurrent_requests` permit and holds it for the whole life of the response — including a non-2xx provider error body that may still be streaming after response headers. It is returned exactly once on ordinary `[DONE]` completion, non-2xx body termination, client disconnect, a downstream policy cut, a backend error, a pre-header cancellation, or a fail-closed final-body rejection. Exhaustion returns the same `503` (`provider_concurrency_exhausted`) the buffered path returns.
- **Circuit breaking.** Selection reserves a real admission from the provider's circuit, so a half-open circuit still grants exactly **one** concurrent probe no matter how many streams arrive. The reservation resolves exactly once from the complete outcome: a 2xx is a circuit **success** only after the SSE guard proves a valid terminal marker; truncation, an oversized/malformed/post-terminal event, a content-coded or non-event-stream 2xx, a configured `fallback_on_status_codes` status, and a redirect are circuit **failures**; an ordinary non-fallback `4xx` the provider chose to return is a success, exactly as the buffered path scores it. A non-2xx verdict is known at response headers but applied only when the error body ends or the claim drops, so a client disconnect after those headers still preserves the provider score rather than treating the exchange as cancelled. Cancellation before headers frees the half-open slot without scoring the circuit at all.
- **Model mapping.** `model_mapping` and `default_model` apply to streams: the claim owner rewrites the final provider-visible top-level `model` to the resolved value and re-asserts `stream: true`. A body with duplicate JSON members is deliberately **not** rewritten (rewriting would collapse the ambiguity) and still fails closed at the final-body hook.
- **Timeouts.** The committed provider's `connect_timeout_seconds` and `read_timeout_seconds` become the streaming route's dispatch budgets, replacing the matched proxy's `backend_connect_timeout_ms` / `backend_read_timeout_ms`. **`read_timeout_seconds` is a whole-exchange bound on this path** — the shared reqwest dispatch applies it from connect through the last response body byte, not as a per-chunk inactivity timer — so a generation that legitimately runs longer than the provider's per-call budget needs `streaming.read_timeout_seconds`, where `0` removes the gateway-imposed bound entirely. TLS and DNS policy are unchanged: the claim still commits explicit verifying backend TLS for an `https` provider and revokes any inherited `dns_override`.
- **Response headers.** Provider response headers on a claimed stream are reduced at the origin response-header boundary to the same bounded safe allowlist the buffered path publishes (`retry-after`, provider request-id fields, `x-ratelimit-*` / `ratelimit-*` families), with values length-capped, control characters refused, names lower-cased, and a hard count cap. Provider `Set-Cookie`, `Location`, auth challenges, validators/digests/checksums, `Alt-Svc`/`Link` steering, and unknown custom metadata never reach the client. The gateway then authors `Content-Type: text/event-stream` and `Cache-Control: no-cache` and drops provider framing. Because the reduction runs *before* the `after_proxy` chain, gateway response plugins (CORS, `security_headers`, correlation/tracing, rate-limit headers) add their own headers normally.
...
- `anthropic` — the OpenAI request is translated to the Anthropic Messages API streaming request (system extraction, user/assistant/tool messages, `max_tokens`, `temperature`, `top_p`, `stop` → `stop_sequences`, `tools`/`tool_choice`, optional Anthropic `thinking`, assistant `tool_calls` → `tool_use` blocks, matching `role: "tool"` → user `tool_result` blocks, legacy assistant `function_call` → `tool_use` with a deterministic `call_legacy_<index>` id, matching `role: "function"` → user `tool_result` blocks correlated by function name, `stream: true`). Message `content` is closed-shape at claim admission: a string or an array of text-part objects with exactly `type` and `text` may be translated; mixed/extra/unknown fields on a text part, non-object array members, mixed text-plus-image/audio/unknown/malformed parts, null system/developer content, and non-text system/developer content, are rejected with an OpenAI-shaped `400` rather than silently reduced. Intentional assistant null/empty content is preserved only when the same message carries a valid `tool_calls` or legacy `function_call` representation. OpenAI `tool_choice: "none"` is translated to Anthropic `{"type":"none"}` while the request's `tools` list is retained; OpenAI string forms are limited to `none` / `auto` / `required` (`required` → Anthropic `{"type":"any"}`), and named choices must be exactly `{type:"function", function:{name:...}}`. Raw input string `any` is rejected. Optional Anthropic `thinking` is admitted only as closed shapes (`enabled` with `type`+`budget_tokens`, or `adaptive`/`disabled` with exactly `type`); manual `enabled` requires `budget_tokens >= 1024` and `budget_tokens` strictly less than the effective Anthropic `max_tokens` this route forwards. Forced tool use is incompatible with manual extended thinking (`thinking.type: "enabled"`) and is rejected, while adaptive thinking may combine with OpenAI `required` / named choices. Unsupported, malformed, or structurally ambiguous `tool_choice` / `thinking` values are rejected with an OpenAI-shaped `400` at admission rather than silently falling through to the provider default. Malformed tool-call history (bad argument JSON, missing ids, orphaned tool/function results, mixed modern `tool_calls` with legacy `function_call` history, oversized argument strings, unsupported roles) is likewise rejected with an OpenAI-shaped `400` rather than silently dropped. A `ResponseStreamInspector` then normalizes Anthropic SSE events into OpenAI `chat.completion.chunk` SSE on the fly: `content_block_delta` text deltas → `choices[].delta.content`, tool-use blocks / `input_json_delta` → `choices[].delta.tool_calls`, `message_delta` → the final `finish_reason` chunk and (when `normalize_response_stream: true`) a terminal usage chunk, followed by `data: [DONE]`. When the caller constrained the turn with `tool_choice: "none"`, any provider `tool_use` (or `stop_reason: "tool_use"`) fails closed with an `upstream_error` SSE frame instead of being normalized into executable OpenAI `tool_calls` deltas; the restriction is request-local and does not cross generations. Successful termination requires Anthropic `message_stop` (or an explicit provider `error` event); clean EOF before that state, and malformed complete/trailing SSE event data, surface an `upstream_error` SSE frame plus exactly one `[DONE]` instead of synthesizing a success-only sentinel. Terminal `message_stop` / provider `error` / fail-safe paths `Terminate` the inspector driver so upstream read stops immediately. The normalizer is robust to chunk splits — it accumulates raw bytes in a cursor-addressed buffer and only transcodes complete SSE events — and enforces fixed security-hardening bounds before UTF-8 conversion, JSON parse, or transcode: **1 MiB per SSE event** (including when a complete oversized event and its blank-line boundary arrive in one chunk), **8 MiB cumulative Anthropic plaintext** (shared with residual content-coding decode, including per-layer decode of stacked `gzip`/`br` chains), **16 MiB cumulative normalized OpenAI SSE output** (and aggregate residual-decode work), **100000 events**, and **JSON nesting depth 32**. Residual provider `Content-Encoding` chains reuse the shared bounded content-coding decoder (at most **4** layers, **1024:1** amplification). Overflow fails closed with a stable `upstream_error` SSE frame + `data: [DONE]` (diagnostics never echo provider payload). Unknown Anthropic event types are ignored for forward compatibility.
- `google_gemini` — the OpenAI request is translated to a Gemini/`streamGenerateContent` (Vertex-compatible) body (`contents`, optional `systemInstruction`, `generationConfig`, `tools`/`toolConfig`, assistant `functionCall` history and matching `functionResponse` / legacy `role: "function"` results). Message `content` is closed-shape at claim admission: a string or an array of text-part objects with exactly `type` and `text` may be translated; mixed/extra/unknown fields on a text part, non-object array members, mixed text-plus-image/audio/unknown/malformed parts, null system/developer/user content, and non-text system/developer content, are rejected with an OpenAI-shaped `400` rather than silently reduced. Intentional assistant null/empty content is preserved only when the same message carries a valid `tool_calls` or legacy `function_call` representation. The model is URL-scoped via an optional `{model}` endpoint placeholder; when response normalization is enabled the claim injects `alt=sse` if absent so native Gemini uses documented SSE framing, while Vertex-compatible JSON array / object streams remain accepted. A claimed normalizing 2xx response with a missing or unexpected `Content-Type` fails closed through `after_proxy` / stream inspector / buffered normalization (bounded OpenAI-shaped upstream error; non-2xx provider error envelopes still pass through untouched). Recognized `text/event-stream` and `application/json` remain accepted. A `ResponseStreamInspector` normalizes provider frames into OpenAI `chat.completion.chunk` SSE: text parts → `choices[].delta.content` (including multi-candidate `index`), `functionCall` → `tool_calls`, `finishReason` / prompt-block safety → terminal `finish_reason`, `usageMetadata` → a terminal usage chunk when present (a present `usageMetadata` must be an object; present `promptTokenCount` / `candidatesTokenCount` / `completionTokenCount` / `totalTokenCount` must be JSON integers representable as `u64`, with no silent fallback from a malformed preferred field to an alternate; omission remains allowed; published totals use checked addition and overflow fails closed), and provider `error` envelopes → a bounded `upstream_error` frame (message capped; never echoes credentials, tool args, or oversized bodies). Present-but-malformed candidate fields fail closed rather than falling back: a present `index` must be a `u64`, a present `finishReason` must be a string, and a present `content.role` must be the string `model` (missing `index` still uses the documented positional fallback). Content part objects that include a representable `text` or `functionCall` field plus additional unrepresentable fields fail closed instead of emitting the representable subset. OpenAI `tool_choice: "none"` becomes Gemini `functionCallingConfig.mode: "NONE"`; provider function calls under that constraint fail closed. Successful termination requires every candidate that emitted client-visible output to reach a terminal `finishReason` (or a prompt block / explicit error) before EOF — a finished candidate cannot emit later content/tool deltas or a second terminal state, and duplicate candidate indexes within one provider event fail closed. Content parts are closed-shape (`text` / `functionCall` only); unrepresentable or unknown parts (including scalars, media, code, and non-empty metadata-only objects) fail closed with fixed-cardinality diagnostics that never echo provider data. The sole metadata-only exception is an empty JSON object `{}`. Premature EOF and malformed/oversized/deep JSON fail closed under the same SSE bounds as Anthropic (**1 MiB/event**, **8 MiB** plaintext, **16 MiB** normalized output, **100000** events, **JSON depth 32**), including the shared residual content-coding decoder for stacked `gzip`/`br` chains. Because the JSON-array/object fallback is normalized into SSE, the response `Content-Type` is relabelled `text/event-stream` (a provider label that is already `text/event-stream`, including its `charset` parameter, is left verbatim) — clients and intermediaries must never see OpenAI `chat.completion.chunk` frames plus `data: [DONE]` under an `application/json` label. Provider chunk framing is scanned incrementally, so a provider that splits a value across many small TCP segments costs work linear in the stream size rather than quadratic. A `promptFeedback.blockReason` terminal obeys the same candidate lifecycle as any other terminal: it never emits a second, contradictory terminal chunk for a candidate that already finished, and it fails closed (rather than closing with the success-shaped terminal) when another candidate emitted client-visible output without reaching a terminal `finishReason`. A `role: "tool"` message whose `tool_call_id` does not match a preceding assistant `tool_calls` entry is rejected at claim admission with the precise `messages[i]` diagnostic — the same rule the translator enforces — instead of falling through to the generic "could not be translated safely" `400`. **Known fidelity gaps.** OpenAI `response_format` (including `{"type": "json_object"}`) is currently dropped rather than mapped to Gemini `generationConfig.responseMimeType`; send provider-native constraints in the prompt if you depend on them. The `MALFORMED_FUNCTION_CALL` and `UNEXPECTED_TOOL_CALL` finish reasons map to OpenAI `content_filter` because OpenAI has no closer equivalent — those two mean the model emitted a broken or unexpected tool call, not that a safety policy fired.

**Header rewriting.** Client `Authorization`, `Proxy-Authorization`, `Cookie`, `x-api-key`, `api-key`, `x-goog-api-key`, and `anthropic-version` headers are stripped before forwarding (unlike `ai_federation`, this path reuses the client's header map, so session-bearing headers must be dropped explicitly), and the provider's credential is injected (`Authorization: Bearer …` for OpenAI-compatible, `x-api-key` for Anthropic, `x-goog-api-key` for Google Gemini). `Host`, `Content-Type: application/json`, and `Accept: text/event-stream` are set for the provider. When provider SSE/stream framing will be normalized, every client `Accept-Encoding` variant is replaced with `Accept-Encoding: identity` so the provider does not return gzip/br octets that the line-delimited normalizer would misread. Because normalization rewrites the client-visible representation even when the provider sends identity bytes, representation headers (`Content-Encoding`, `Content-Length`, content-bound validators, and `Vary: Accept-Encoding`) are always repaired. If a provider still returns a non-identity `Content-Encoding`, Ferrum parses the header as an RFC 9110 `#content-coding` list (OWS-tolerant, case-insensitive; `x-gzip` normalized to `gzip`) and decodes supported chains (`gzip` / `br`, including stacked lists such as `gzip, br`) in reverse application order under the shared residual-decode limits — **8 MiB per layer**, **16 MiB aggregate decoded work**, **at most 4 layers**, and a **1024:1** raw-to-decoded amplification ratio (the same ratio the compression request path uses). `identity`-only lists are treated as identity; mixed `identity`, parameters, empty members, unsupported codings (`zstd`, `deflate`, …), more than four layers, ambiguous case-variant duplicate field-lines, truncated/malformed/concatenated members, and limit overruns fail closed with an OpenAI-shaped `502` before any opaque provider frame can be served as normalized AI output. Residual encoded streaming responses use a bounded fallback that holds encoded octets until EOF (never forwarding them mid-stream) and decodes only after the complete coding and checksum can be validated; ordinary identity SSE remains progressive. Gateway-asserted consumer-identity headers (`x-consumer-username` / `x-consumer-custom-id`) are also **suppressed** for claimed requests via the shared `suppress_backend_consumer_identity_headers` metadata marker, so an authenticated consumer's internal identifiers never reach the third-party provider (the resolved principal still drives rate limiting, logging, and policy plugins).
...
**Final provider boundary (`GHSA-xhp5-hqj8-3mwg`).** Claiming in `before_proxy` at 2984 only orders this plugin ahead of the plugins that run before it. `request_transformer` runs at 3000, and a `serverless_function` `pre_proxy` backend header overlay is merged later still — either could add, overwrite, or rename the provider credential this plugin just installed, leak a normal-backend static secret to the third party, restore a client credential, or rewrite the already-selected `model` in the translated body. Two shared lifecycle boundaries close that window, and neither depends on relative priority:

- **Header re-assertion.** The plugin declares `enforces_final_backend_header_policy()`, so the gateway re-applies its complete owned header set over the *finalized* backend-visible header map — after every `before_proxy` pass (including the deferred routing/remaining passes), after the reserved gateway-assertion refresh and egress baggage strip, and after a finalized-egress header overlay — on the H1/H2 and native HTTP/3 ladders alike. Every credential header listed above is re-stripped, gateway-asserted `x-consumer-username` / `x-consumer-custom-id` / `x-geo-country` are removed again, and only the selected provider's own credential (plus `Host`, `Content-Type`, `Accept`, `anthropic-version`, and the normalization `Accept-Encoding: identity`) is installed. The owned set is fixed and closed; headers outside it are untouched, so intended non-credential operator transforms still reach the provider. Ferrum does **not** attempt to classify an arbitrary unknown custom header as a credential — a bespoke normal-backend secret header configured on the same proxy will still be forwarded, so do not configure such a rule on a proxy that routes to a third-party provider.
- **Query commitment.** `request_transformer` query rules also run later (3000) and could otherwise append a normal-backend static secret to the provider URL. The claim therefore freezes the exact safe backend-visible query at claim time — after the authentication-owned strip markers and any earlier plugin's published outbound query — and the gateway replays that frozen value at the single point where the backend-visible query is captured, on H1/H2, on native HTTP/3, and for every retry attempt. When an endpoint query is folded into the absolute override path, the separately appended query is committed **empty**; otherwise the already-safe client query continues unchanged. The committed query lives in private request state, never in metadata or a transaction log.
- **Destination, TLS, and DNS commitment.** A visible host/port match is not destination identity. The claim commits a *direct* provider endpoint: it clears any earlier `route_override_upstream_id` (so no load balancer can pick the dial target), pins the exact resolved backend TLS configuration (verification flag, SNI, CA bundle, and mTLS client materials — with plaintext HTTP represented as a distinct "no TLS" state), and explicitly revokes any inherited proxy `dns_override` even when the configured backend host text already equals the provider host. Without that last step, a same-host DNS pin would send the provider credential to an operator-chosen address instead of provider DNS.
- **Body / model / route revalidation.** `on_final_request_body` re-parses the exact backend-visible body after every `transform_request_body` hook and fails closed unless the provider-visible `model` is a non-empty string that is still *exactly* the model which selected this provider **and** still matches that provider's `model_patterns`. A body that is no longer valid JSON, has no usable `model`, or carries duplicate JSON members (which would make the provider-visible generation parser-dependent) is rejected the same way, as is any drift in the committed destination / upstream identity / resolved TLS / DNS decision above. Model failures return an OpenAI-shaped `400` (`code: model_policy_violation`); a broken provider/route invariant returns a `500` (`code: provider_policy_violation`). No offending model, header, query, or body value is echoed into the error envelope or logged.

**Multiple instances.** Several `ai_stream_router` instances may be effective on one proxy, and two of them may legitimately share a provider `name` while differing in endpoint, API key, `provider_type`, `model_patterns`, and `normalize_response_stream`. Exactly one instance can claim a request: the first (in configured priority order) whose provider matches wins, and later instances leave the claim alone. Ownership is recorded as an opaque per-instance identity in private request state — never in metadata, logs, configuration, OpenAPI, or a response — and every claim-dependent hook verifies it: request-body transformation, final header re-assertion, final body/route revalidation, response-header handling, and response stream inspector / normalizer selection. A losing instance therefore cannot reapply its own credential, translate the body a second time, revalidate against the wrong model policy, or install a second SSE normalizer. `fail_on_missing_model` and `fail_on_no_matching_provider` still decide an *unclaimed* request in normal plugin order; ownership begins only once some instance has successfully claimed.

Because that header mutation happens after every `before_proxy` hook, no fingerprint taken during that phase can witness it: plugin-cache admission **rejects** `ai_stream_router` composed with `request_deduplication` on the same proxy and protocol. (`response_caching` was already refused alongside this plugin by its deferred-request-body-transformer rule.)

**Composition with other built-ins.** Because `ai_stream_router` runs first, a successful claim records a private typed claim on the request context and also publishes `ctx.metadata["ai_stream_router_claimed"] = "true"` for logs and third-party/custom plugins. Every BUILT-IN that must stand down on a claimed provider request — `ai_federation`, `request_mirror`, `serverless_function`, `mcp_gateway`, and `mesh_route_dispatch` — decides from that **private claim**, never from the metadata key (GHSA-xhp5-hqj8-3mwg): metadata is a mutable string map, so a later plugin deleting or rewriting the marker would otherwise let those plugins re-route, mirror, federate, or invoke a function over a request whose third-party provider credential, model, destination, and query are already committed. The plugins compose exactly as before: `stream: true` is served by `ai_stream_router`, `stream: false` by `ai_federation`. Intentional **pass-through** (`fail_on_missing_model` / `fail_on_no_matching_provider` disabled) is a genuinely unclaimed request and still coordinates through `ctx.metadata["ai_stream_router_pass_through"]`. Claimed requests also set the shared `ai_request_streaming` marker for non-policy streaming helpers such as token metrics. The marker is not authority to waive an outbound security policy: `ai_response_guard`, WAF response-body inspection, response validators, and strict response-size policy buffer conservatively and decide from the pristine backend representation. In particular, an enforcing/redacting `ai_response_guard` rejects a genuine provider SSE response before commit because it cannot promise complete bounded inspection; use a stream-aware policy such as `ai_semantic_firewall` progressive inspection instead of relying on the marker.

**Metadata keys written:** `ai_stream_router.enabled`, `ai_stream_router.claimed`, `ai_stream_router_claimed`, `ai_request_streaming`, `suppress_backend_consumer_identity_headers`, `ai_stream_router.provider`, `ai_stream_router.provider_type`, `ai_stream_router.model`, `ai_stream_router.request_translated`, `ai_stream_router.tool_choice_none` (when the translated Anthropic request forbids tools for this generation), `ai_stream_router.normalized_response_stream`, and `ai_stream_router.provider_content_encoding` (when a residual provider coding must be decoded). There is no `ai_stream_router.fallback_attempts` key — the plugin never attempts a second provider, so a permanently-zero counter would only advertise a capability that does not exist. Every one of these keys is **observability and cross-plugin coordination only**; a later plugin may write any of them and no enforcement point reads them back. The committed **model**, destination, resolved backend TLS, DNS decision, backend-visible query, Anthropic-translation witness, tool-use guard, and instance-ownership identity are deliberately **not** metadata: they are private typed request state, so they cannot be forged by another plugin, and neither the routed model nor a query string that may hold a relocated credential ever reaches a transaction log. In particular `ai_stream_router.model` mirrors the routed model for logs, while final model enforcement and the response normalizers use the private committed copy — otherwise a later plugin could rewrite both the final body's `model` and that key to the same new value and pass an equality check while bypassing provider selection. The one metadata value a claim-owned hook still consults is `ai_stream_router.provider_content_encoding`, which comes from the provider's own response headers, only selects how this instance decodes its own upstream bytes, and is bounded by that decoder's limits.
...
| `streaming.window` | string | `sentence` | `inspect` only. Window boundary: `sentence`, `paragraph`, `bytes`, or `tokens`. `tokens` requires an explicit `streaming.tokenizer` |
| `streaming.tokenizer` | string | required for `tokens` | `inspect` + `window: tokens` only. Bounded deterministic tokenizer: `chars4` (~4 Unicode scalars/token, same estimate as `ai_rate_limiter` / `ai_prompt_compressor`), `whitespace` (Unicode whitespace-delimited runs), or `unicode_words` (alphanumeric runs; each non-whitespace non-alphanumeric scalar is its own token, and CJK ideographs, kana, and Hangul syllables are one token per character rather than a joined run). Not a provider BPE model; rejected when `window` is not `tokens` |
...
| `streaming.overlap_bytes` | uint | `256` | `inspect` only. Already-cleared text re-inspected with the next window so a violation split across a boundary is still caught. Must be less than `streaming.max_window_bytes` and is included within that aggregate retained-state cap. For `window: tokens` this remains the hard memory ceiling on retained overlap while `overlap_tokens` selects the prose token budget |
| `streaming.max_window_tokens` | uint | `256` | `inspect` + `window: tokens` only. Soft window size in complete tokens under the selected tokenizer (`1..=65536`). The hard `max_window_bytes` cap still forces a window (preferring a complete-token cut) when retained state fills first. Rejected when `window` is not `tokens` |
| `streaming.overlap_tokens` | uint | `min(32, max_window_tokens-1)` | `inspect` + `window: tokens` only. Complete tokens of already-cleared prose re-inspected with the next window. Retained overlap tokens count against the next window's `max_window_tokens`, so a window closes once retained + new tokens reach the budget (at least one new token is always required). Must be strictly less than `max_window_tokens`. Rejected when `window` is not `tokens` |
...
Terminal usage signals understood on the streaming path:

| Family | Signal |
|---|---|
| OpenAI / Mistral / Azure | root `usage` on a chunk (`stream_options.include_usage: true`) |
...
The **entire** per-request reservation lifecycle is scoped to the limiter instance via a process-unique id: the AI-request classification, the reserved estimate, the local reservation id, the Redis reserved-window index (and therefore the inferred reservation backend), the recorded actual usage, the applied `on_unmetered_response` action, the release idempotency marker, the federation idempotency flag, and the `expose_headers` telemetry are all staged under instance-owned `ctx.metadata` keys of the form `<name>#<instance id>`. Two instances therefore reconcile and release independently even when they differ in `count_mode`, `limit_by`, estimate, or backend (local/local, Redis/Redis, or mixed), and even when their response passes complete out of order. Before this scoping, the second admission pass overwrote the first instance's reservation markers and the first release suppressed every sibling release, so one configured budget could be under-enforced while another was overcharged.

Because the `x-ai-ratelimit-*` header **names** are a fixed public contract, two instances that both set `expose_headers: true` still write the same four headers — the last instance in configured plugin order wins — but each instance's four values are internally consistent rather than an interleaving of two budgets. Prefer enabling `expose_headers` on a single instance.
...
> | A stream with no pre-request reservation whose policy would require a `502` (`reject`, or `charge_estimate` for a compressed candidate with no safe estimate) | Rejected before response headers are committed; clients must supply an output cap (or otherwise produce a non-zero estimate) to use streaming under that policy |
> | A usage-less committed stream with a non-zero reservation whose policy would have answered `502` | Cannot substitute a 502 after termination; degrades to fail-closed keep-charged accounting, logs that the rejection could not be delivered, and marks the release path consumed so a later reject-replay cannot free it |
...
>
> Window/TTL expiry remains the intentional backstop **only** for cancellation-only cases the proxy cannot await — for example a request-task abort, or a deferred-logger `Drop` outside a Tokio runtime that cannot spawn terminal hooks. A burst of those aborts can transiently over-count until the sliding window (local) or Redis key TTL (centralized, `2 × window_seconds + 1`) rolls; choose `window_seconds` with that recovery latency in mind. Do not read that backstop as covering ordinary non-2xx, reject-replay, client-disconnect, or stream-terminal paths — those reconcile in code.
>
...
**Aggregate SSE multiplexing.** In `aggregate_router` mode with `sessions.sse_multiplexing` (default `true`), a client may open **one** `GET` SSE listener per live downstream MCP session (`Accept: text/event-stream` plus the configured session header). The listener is served over HTTP/1.1, HTTP/2, and native HTTP/3 alike: the response is streamed with no `Content-Length` (`ClientResponseFraming::Streaming`), so no representation length is published for bytes that have not been written.

Every JSON-RPC event for that session is routed onto that one listener and identified by a bounded, **type-sensitive** stream identity derived from the JSON-RPC id — the string `"1"` and the number `1` are different streams and can never collide. Objects, arrays, booleans, null, empty, over-`sessions.sse_max_stream_id_bytes`, and control-bearing ids are not representable identities: those responses are answered inline on the POST instead (never multiplexed under a fabricated identity, never dropped).

*What is multiplexed, and when the identity opens.* While a listener is attached, **every** aggregate JSON-RPC **request** with a representable id opens its stream identity **before** the handler runs any catalog refresh, upstream `initialize`, or backend dispatch, and its terminal response is published on that listener while the POST answers `202` with no body. That covers the gateway-authored responses (`ping`, `notifications/initialized` in request form, `tools/list`, `prompts/list`, `resources/list`, `resources/templates/list`, and every gateway JSON-RPC error such as `-32601`/`-32006`) and the upstream-routed ones (`tools/call`, `prompts/get`, `resources/read`, and any method sent to the primary upstream by `capabilities.passthrough_unknown_methods`). **Both kinds use one two-phase publication boundary.** A gateway-authored response is *staged* in `before_proxy` — its stream identity stays open and the JSON-RPC payload stays on the response — and then travels Ferrum's synthetic-response lifecycle exactly as a routed response travels the backend lifecycle. After normalization, response-body guardrails, semantic transforms, and authoritative final body policy accept the exact JSON-RPC bytes, the final body hook reserves bounded event capacity and selects the empty POST-side `202`; it does **not** expose the event yet. The final response-header policy therefore decides the actual acknowledgement, and the observe-only committed hook publishes the reserved bytes only when that final response is still the empty `202`. A later policy replacement aborts the reservation, returns stream capacity exactly once, and remains inline on the POST. Direct publishers include outstanding reservations in their event/byte admission calculations, so a successful reservation cannot lose capacity before commit. The event bytes are therefore the same bytes an inline POST would have received, *after* every configured response-body WAF rule, `body_validator` response schema, `ai_response_guard` redaction, and response transform has accepted them; a policy that safely redacts a still-valid `200` JSON-RPC response with the same type-sensitive id publishes the redacted bytes. Opening before the slow work is what makes cancellation meaningful: a `notifications/cancelled` arriving on another connection while a call is still in flight — or while a staged gateway-authored response is still inside the response lifecycle — marks a genuinely **open** stream, and that response is then suppressed rather than published late. Three shapes never multiplex: a **notification** (no JSON-RPC response exists at all, so no identity is opened), a **batch member** (the batch restriction assembles one HTTP response array), and **`initialize`** (its reply carries the session header an event stream cannot convey, and there is no session to attach to yet). A response the final phase cannot read as a bounded, unambiguous JSON-RPC response naming the same type-sensitive id as the open request — including a non-200, event-stream, non-JSON, compressed, undeclared-length, over-`validation.max_upstream_response_bytes`, duplicate-key, mismatched-id, over-`sessions.sse_max_event_bytes`, or CR/LF-bearing body (a trailing newline included: SSE line folding cannot carry those octets unchanged, and it is reported as its own `mcp.sse.error = event_framing_invalid` rather than as an oversized payload) — answers inline on the POST instead of being re-encoded, misrouted, or stripped of its HTTP failure semantics (the same header-time buffering refinement `tools/call` result validation already uses). `mcp.sse.stream = open` records that an identity was opened; `mcp.sse.delivery` records the committed outcome as `multiplexed`, `inline`, or `suppressed`.

*Retention is one budget.* Staging (events published while nobody is attached) and `Last-Event-ID` replay share a single ring bounded by `sessions.sse_max_retained_events` and `sessions.sse_max_retained_bytes`. An event still owed to a consumer is **never** evicted: when the ring cannot admit a new event the publish fails closed, the gateway answers that POST with the ordinary inline JSON response (recording `mcp.sse.error = retention_overflow`), and the listener keeps running. Already-delivered history is evicted as needed while reserving the incoming event's count and bytes, so a ring exactly at either cap does not refuse new work when safe replay history can make room. A refusal still **terminalizes** the identity inside the same critical section, so the inline answer is that identity's one terminal outcome, its stream capacity is returned exactly once, and a later retry of an inline-completed id can never be published onto the stream. Repeated retention refusals therefore cannot exhaust `sessions.sse_max_streams_per_session`. `sessions.sse_max_replay_events` bounds how many already-consumed events stay behind for resumption and must not exceed the ring capacity; `0` evicts a consumed event immediately.

*Sizing the retention envelope.* The ring is per session, and broker session cardinality mirrors the MCP session store exactly, so the **worst-case aggregate** retained-event memory for one `mcp_gateway` instance is `sessions.max_sessions × sessions.sse_max_retained_bytes` — at the defaults (`16384 × 1 MiB`) that ceiling is large, so size both together for the deployment rather than raising either in isolation. Two properties keep the practical figure far below that ceiling and keep it finite regardless: a stream identity is opened **only while a listener is currently attached**, so a session accrues retained bytes only while it is holding a live downstream SSE connection (and is therefore also bounded by `FERRUM_MAX_CONNECTIONS`), and reaching `sessions.max_sessions` evicts the oldest sessions — which tears down their broker sessions through the same funnel — before a new `initialize` is admitted.

*Stream lifecycle.* `sessions.sse_max_streams_per_session` bounds concurrently **open** streams. Publishing a stream's terminal response completes it and returns its capacity, so a session is not exhausted after that many requests. The open identity is owned by a lease held privately on the request, so **every** ending releases it exactly once and never spawns a cleanup task: publication, cancellation, an inline fallback, a fail-closed policy replacement, a backend or body error, and a client transport disconnect all funnel into the same idempotent terminalization, and an already-known cancellation is never overwritten by a later completion. A completed or cancelled identity is retained as a bounded terminal record: a late or duplicate response for it is refused rather than misattributed onto a new stream, and a request that reuses a still-open or already-terminal id inside one session is not multiplexed (it answers inline, recording the fixed refusal token) so it can never join another request's stream. `notifications/cancelled` with `params.requestId` cancels the named stream as a **side effect only** — the notification keeps its ordinary routing (including passthrough to the primary upstream when `capabilities.passthrough_unknown_methods` is set), and because a JSON-RPC notification never receives an error response an unknown or unrepresentable id records a fixed `mcp.sse.cancel` token instead of changing the HTTP outcome. A cancelled stream's own response is suppressed and its POST acknowledged with `202`.

*Resumption.* With no `Last-Event-ID` the listener resumes at the delivery watermark: staged events are delivered and already-delivered ones are not replayed, so attach never duplicates. A cursor is otherwise explicit — ahead of anything issued is `400`, older than the retained history is `410 Gone` (continuity is never fabricated), non-numeric or over `sessions.sse_max_last_event_id_bytes` is `400`. The raw header value never reaches a log or metadata; only `mcp.sse.resume_requested = true` is recorded.

*Bounded lifetime and teardown.* An attached listener lives at most `sessions.sse_listener_max_lifetime_seconds` (default 300) and emits a keepalive comment every `sessions.sse_keepalive_seconds` (default 15), which is how a vanished peer is discovered on a quiet session. Be precise about what that deadline guarantees. The stream's own timer only advances while the transport polls the response body, and a peer that stops reading can park an HTTP/1.1 or HTTP/2 write indefinitely, so on those transports the timer alone is **not** a reclamation guarantee for a client that stalls without disconnecting. Native HTTP/3 does have a hard bound: its writer wraps the whole send loop in this deadline, because `send_data` awaits QUIC flow control and would otherwise park forever. What is guaranteed on **every** transport is that the session's single-listener slot stays usable: a `GET` arriving after the deadline **supersedes** the stalled listener instead of receiving `409`, which ends that listener on its next poll and makes its own release a no-op; while it is past the deadline the gateway also stops routing new responses to it and answers them inline. Unconditional teardown paths are unchanged and never use a best-effort lock: client disconnect (dropping the response body), session `DELETE`, TTL expiry, cap eviction, and dropping the plugin instance on reload/update/delete all end the stream. Broker session cardinality mirrors the MCP session store exactly — `initialize` admits both under one critical section and **fails closed** with JSON-RPC `-32013` rather than minting a session that could never attach its stream.

Duplicate listeners for the same session return `409` while the attached one is inside its lifetime; missing/unknown/stale sessions return the ordinary `404` that prompts re-initialization. Diagnostics are fixed low-cardinality tokens in `mcp.sse.error` / `mcp.sse.cancel` / `mcp.sse.delivery` — never a session id, JSON-RPC id, header value, or event payload. Transparent mode continues to forward `GET` upstream unchanged, and with `sse_multiplexing: false` aggregate `GET` keeps its `405`.
...
config:
  ...
  endpoint:
    ...
    grpc_services:
      # Canonical A2A 0.3 service (package `a2a.v1`) — card schema `a2a-0.3`.
      - a2a.v1.A2AService
      # A custom service fronting an A2A 0.3 backend must say so explicitly.
      - service: acme.agents.v1.AgentService
        card_schema: a2a-0.3
  detection:
...
**gRPC service identity and Agent Card schema.** A gRPC service *name* and an `AgentCard` wire *layout* are two different facts, and A2A has published two of each. A2A **0.3** — the protocol version this plugin implements, and the default `endpoint.protocol_versions` — declares `package a2a.v1` in `specification/grpc/a2a.proto` (`a2aproject/A2A` at tag `v0.3.0`), so its canonical service is **`a2a.v1.A2AService`**, the primary default `endpoint.grpc_services` entry. A2A **1.0** declares `package lf.a2a.v1` with a materially renumbered `AgentCard`, so **`lf.a2a.v1.A2AService`** is a different identity carrying a different card layout. It is also recognized by default to preserve method-policy enforcement for deployments that relied on the former default; its cards remain schema-gated and are never decoded as 0.3. The gateway never infers one from the other.

Each `endpoint.grpc_services` entry therefore has a declared card schema, in one of two spellings:

| Entry | Declared schema | Effect |
| --- | --- | --- |
| `a2a.v1.A2AService` (string) | `a2a-0.3` | Detected, policed, and Agent Card rewriting is implemented. |
| `lf.a2a.v1.A2AService` (string) | `a2a-1.0` | Detected and policed; card rewriting fails closed with `agent_card_grpc_schema_unsupported`. |
| any other name (string) | `none` | Detected and policed; card rewriting fails closed with `agent_card_grpc_schema_undeclared`. |
| `{service: <name>, card_schema: a2a-0.3 \| a2a-1.0 \| none}` | as declared | The explicit form a custom deployment uses to state which published layout its own service serves. |

The two published names have a schema fixed by the specification, so declaring a *contradicting* `card_schema` for one is rejected at config admission rather than honored as an override — the layout is a property of the protocol, not of the deployment. Restating the correct one is accepted and is a no-op. A custom service with no declared schema still gets full detection, method policy, and `a2a.*` metadata; only protobuf card rewriting is withheld. To front a custom or A2A 1.0 service whose cards must pass through untouched, set `discovery.rewrite_agent_card_urls: false` — with rewriting enabled, such a card is refused rather than served with un-rewritten internal URLs.

Agent Card rewriting applies to buffered HTTP JSON Agent Card responses and to unary gRPC Agent Card responses (`GetAgentCard` / `GetExtendedAgentCard`) for detected Agent Card requests. It rewrites JSON-RPC endpoint URLs to the public gateway base and `endpoint.path`, preserves advertised `GRPC`, `HTTP+JSON`, and `REST` interface URLs, and removes existing `signatures` because rewritten fields invalidate card signatures. gRPC rewriting decodes the A2A **0.3.x** protobuf Agent Card wire layout only, mutates only the supported endpoint fields, and re-frames an uncompressed unary response.

Only a **proven-successful** unary reply is treated as a candidate Agent Card: the response must be HTTP `200` *and* carry a terminal `grpc-status` of exactly `0`. On the buffered native-gRPC path plugins see a merged header+trailer view in which the trailing value wins for the reserved terminal keys, so an upstream that streamed `HEADERS(200) + DATA + TRAILERS(grpc-status: 13)` is recognized as the failure it is. An unproven reply — including one with no terminal status at all — is forwarded exactly as the upstream wrote it: it is neither rewritten nor failed closed, so a backend error is never relabelled as a gateway rewrite fault.

Before any byte of a candidate card is interpreted, the **responding service must be declared to carry the `a2a-0.3` layout**. A decoder is chosen by the protocol the endpoint speaks, never by whichever layout the bytes could be coerced into: an `lf.a2a.v1.A2AService` reply is refused with `agent_card_grpc_schema_unsupported` and an undeclared custom service with `agent_card_grpc_schema_undeclared`, neither of which is a complaint about the card's own `protocol_version`.

Once the service's layout is settled, the version gate is **positive and exact**, and has two independent requirements. First, the card must carry an explicit `protocol_version` (field 16) on the wire whose trimmed value equals an entry of `endpoint.protocol_versions`. That list is a set of exact version strings — there is no family, range, or wildcard syntax — so configuring `["0.3.0"]` admits `0.3.0` and refuses `0.3.99`; admitting another point release means listing it. Second, the matched version must belong to the A2A **0.3** family, which is the only wire layout this rewriter implements: a configured `1.0.0` that a backend echoes back is an exact match and is still refused. **A card with an absent or empty `protocol_version` fails closed** — it is not assumed to be 0.3.x even when `endpoint.protocol_versions` says so. A2A renumbered `AgentCard` for 1.0 (field 3 became `supported_interfaces`, `signatures` moved from 17 to 13, field 14 became `icon_url`, and `protocol_version` was removed), and proto3 cannot distinguish an unset field from an empty string, so an empty version is the exact case where the gateway cannot tell the two layouts apart. Rewriting it with 0.3 field numbers would flatten each interface into a bare URL and re-serve the card under its now-invalid original signature, so the plugin refuses instead.

An admitted card is then **schema-validated before any field is rewritten**, over the complete A2A 0.3 top-level `AgentCard` field table (`a2aproject/A2A` at tag `v0.3.0`). Three properties are enforced for every one of the seventeen known top-level fields:

- **Wire type.** Length-delimited for every string, submessage, map entry, and repeated string — `name` (1), `description` (2), `url` (3), `provider` (4), `version` (5), `documentation_url` (6), `capabilities` (7), `security_schemes` (8), `security` (9), `default_input_modes` (10), `default_output_modes` (11), `skills` (12), `preferred_transport` (14), `additional_interfaces` (15), `protocol_version` (16), and `signatures` (17) — and a varint for the one boolean, `supports_authenticated_extended_card` (13), whose value must additionally be a canonical `0` or `1`. A known field with an unexpected wire type would otherwise fall through the rewriter's catch-all and be preserved verbatim beside rewritten siblings after the signature block was dropped.
- **Multiplicity.** Every singular field may appear at most once; a duplicate fails closed, because proto3 last-wins semantics would let a backend hide the URL that actually gets served behind a decoy earlier in the message. The genuinely repeated fields (8, 9, 10, 11, 12, 15, 17) may of course repeat.
- **UTF-8 for every known string that is re-emitted** — `name`, `description`, `version`, `documentation_url`, `default_input_modes`, `default_output_modes`, `preferred_transport`, and `protocol_version`. These are copied into the rebuilt card, so a value the gateway cannot represent as text is not republished as though it had been inspected.

`AgentCard.url` (3) is held to a stronger proof: it must parse as an absolute `http`/`https` URL with a real host and no embedded credentials, bounded to 4096 bytes and free of ASCII whitespace and control characters. A prefix test would not do — `https://`, `https://:8080/a2a`, and `http://` followed by a control character all begin with those bytes without being a URL, and a serialized `AgentInterface` (which is what field 3 holds in the A2A 1.0 layout) is frequently valid UTF-8, so this check is what actually distinguishes a URL string from a submessage. A value that fails it is treated as a layout mismatch. The proof requires an explicit canonical authority spelling before any path, query, or fragment — forms such as `http:///a2a`, where WHATWG recovery would infer a host only after consuming an extra slash, are rejected because hostile wire input can be consumed by parsers with different recovery rules.

**Where the validation boundary is.** The opaque submessages this rewriter deliberately preserves — `provider`, `capabilities`, each `security_schemes` map entry, each `security`, each `skills`, and each `signatures` — are checked for wire type and multiplicity **only**. Their contents are never decoded, and the gateway makes no claim that they are well formed. The single nested exception is `additional_interfaces`, which the rewriter itself parses and can mutate: each `AgentInterface` is validated for wire type and multiplicity on `url` (1) and `transport` (2), and its `url` is **required** and held to the same absolute-URL proof. An interface with a missing URL is not a usable endpoint — it could never be the JSON-RPC endpoint the gateway fronts, so it would be preserved untouched inside a card published as rewritten with its signatures removed, while also being the only thing that made the card look like it advertised an endpoint at all — so it fails closed with `agent_card_protobuf_interface_url_missing`. Unknown fields with any valid wire type are preserved untouched, so a newer 0.3.x point release that adds a field still round-trips. The protobuf decoder is strict about hostile framing too: over-long, over-range, and non-canonical (redundantly padded) varints, field numbers of `0` or above the protobuf maximum `2^29 - 1`, out-of-range length prefixes, and the group wire types are all rejected rather than truncated or resynchronized against.

Unproven versions, layout mismatches, schema faults, malformed frames, and non-identity `grpc-encoding` all fail closed as a **trailers-only gRPC `INTERNAL` under HTTP `200`** — the same shape as the gateway's other gRPC terminals, so a policy refusal is not published as a synthetic backend 5xx — with an empty body and a fixed `a2a.error` diagnostic when metadata emission is enabled; response bytes are never reflected into `grpc-message` or metadata. The diagnostics are a closed set of fixed strings, and this is the complete list of the ones reachable by a client on the gRPC Agent Card path:

| Stage | Diagnostic | Meaning |
| --- | --- | --- |
| Service schema | `agent_card_grpc_schema_unsupported` | The responding gRPC service is declared to carry a known Agent Card layout this gateway does not implement (`a2a-1.0`). |
| Service schema | `agent_card_grpc_schema_undeclared` | The responding gRPC service declares no Agent Card layout (`none`), so there is nothing to decode against. |
| Framing | `agent_card_grpc_encoding_unsupported` | `grpc-encoding` or the frame's compressed-flag byte is anything but identity. |
| Framing | `agent_card_grpc_frame_malformed` | The reply is not exactly one length-prefixed unary message. |
| Framing | `agent_card_grpc_frame_too_large` | The rewritten card cannot be addressed by the 32-bit gRPC length prefix. |
| Version/layout | `unsupported_agent_card_protobuf_version` | `protocol_version` is absent, empty, not configured, or not in the 0.3 family. |
| Version/layout | `agent_card_protobuf_shape_unrecognized` | The message does not decode as any recognizable Agent Card shape. |
| Version/layout | `agent_card_protobuf_url_layout_mismatch` | A rewritable URL field does not parse as a bounded absolute `http`/`https` URL with a real host and no embedded credentials, so the layout is not 0.3. |
| Schema | `agent_card_protobuf_field_wire_mismatch` | A known 0.3 field carries a wire type the schema does not declare. |
| Schema | `agent_card_protobuf_field_duplicated` | A singular 0.3 field appears more than once. |
| Schema | `agent_card_protobuf_field_invalid` | A field number is `0` or above the protobuf maximum `2^29 - 1`. |
| Schema | `agent_card_protobuf_string_invalid` | A known 0.3 string field that would be re-emitted is not valid UTF-8. |
| Schema | `agent_card_protobuf_bool_invalid` | `supports_authenticated_extended_card` carries a varint that is not a canonical `0` or `1`. |
| Schema | `agent_card_protobuf_interface_url_missing` | An `AgentInterface` in `additional_interfaces` carries no `url`, so it is not a usable endpoint. |
| Schema | `agent_card_protobuf_url_invalid` | A rewritable URL field is not valid UTF-8. |
| Decoding | `agent_card_protobuf_truncated` | The message ends inside a tag, varint, or length-delimited value. |
| Decoding | `agent_card_protobuf_length_invalid` | A length prefix is out of range for the remaining bytes. |
| Decoding | `agent_card_protobuf_varint_overflow` | A varint is over-long or exceeds 64 bits. |
...
That proof is only available when the rewritten origin comes from configuration. With `discovery.public_base_url` **unset** and `discovery.trust_forwarded_headers: true`, the origin is instead derived from the request's `X-Forwarded-Proto` / `X-Forwarded-Host` / `Host` and — absent a forwarded scheme — from whether the connection carried a TLS SNI hostname. The deduplication fingerprint binds neither the complete forwarded header set nor the transport's SNI, so two requests sharing a replay key can legitimately deserve cards advertising different origins. Configuration admission therefore rejects that specific pairing with HTTP 400 (fatal in file mode, a rejected update in DP), naming both instances; setting `discovery.public_base_url` resolves it. A configured-public-base deployment composes with `request_deduplication` normally. At runtime the same case degrades the proxy's presentation provenance to "unprovable", so admission paths that only warn on pre-existing data still retain and replay nothing.
