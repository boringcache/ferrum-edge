...
In `terminate` mode, applicable end-to-end function response headers (including repeated `Set-Cookie`, `Location`, `Retry-After`, `ETag`, and `Content-Disposition`) are returned with the function status/body. Hop-by-hop, `Connection`-listed, transport-managed, provider-control, and credential-bearing fields are stripped. Ferrum recomputes framing; HEAD, 204, and 304 responses have no body. If request deduplication acquired the idempotency key earlier in the chain, the exact committed terminal response is stored after final rejection normalization and replayed so a retry does not repeat the function side effect; this includes empty, HEAD, and non-2xx responses that bypass synthetic body hooks. If the function may already have executed but `on_error: "continue"` falls through to a streamed backend response, the in-flight marker is retained until its TTL even after a clean stream completion because no replayable function response exists.
