...
| Parameter | Type | Default | Description |
|---|---|---|---|
| `db_path` | String | (required) | Path to a MaxMind `.mmdb` file no larger than 512 MiB. The declared sizes of all distinct MMDB paths in one validation generation/cache build must also total no more than 512 MiB. |
