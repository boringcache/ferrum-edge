...
**Memory protection:** The plugin tracks per-client state in a `DashMap` capped at 100,000 entries. When the cap is exceeded, only datagrams from already-tracked client IPs are forwarded — datagrams from new IPs are dropped without inserting state. This prevents spoofed-source-IP floods from causing unbounded memory growth. Stale entries (idle for `window_seconds × 2`, minimum 10 seconds) are pruned by a sampled periodic sweep (every 100,000 datagram hooks) even while the map is below the hard cap; when active state still exceeds the cap after that prune, remaining keys are force-evicted. The sweep is cooldown-gated to once per second via an atomic `compare_exchange` so concurrent sweeps cannot pile up under load, and Redis-fallback local maps use the same below-cap prune / over-cap force-eviction split.
