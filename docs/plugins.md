...
| `streaming_response` | string | `skip` | Behavior for `stream: true` requests when response-side inspection is active: `skip` (fail-open — allow the streamed response uninspected and record the skip), `reject` (fail-closed — reject streaming requests so clients retry with `stream: false`), `buffer` (force the SSE response onto the buffered path, reassemble its deltas, run the full engine before delivery — most accurate, highest time-to-first-byte; oversized streams fail closed via `FERRUM_MAX_RESPONSE_BODY_SIZE_BYTES`), or `inspect` (progressive windowed inspection with mid-stream cut — preserves streaming UX) |
