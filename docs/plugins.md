...
- Every retained response — local or Redis — is stamped with the response-side presentation policy it was produced under: a fixed-size content digest of the published `response_transformer` RTDS gate map **and** a content digest of the effective static `response_transformer` rules for that proxy, folded in configured execution order. A stored response replays only while both still match the live policy. Because a replay deliberately skips ordinary presentation transforms, this is what stops a response captured before a redaction/header/body rule was added or enabled from being served unredacted until TTL. Only digests are stored — never rule text, header values, or scope names
