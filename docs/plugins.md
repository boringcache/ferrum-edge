...
The `.mmdb` file is read into an owned immutable byte buffer at plugin startup, with metadata and bounded-read checks rejecting files larger than 512 MiB before parsing. Ferrum fully verifies the search tree and data section, checks for a supported country-capable product (`GeoIP2`/`GeoLite2` Country or City, or GeoIP2 Enterprise), and scans every country record against the same supported-code set used by policy admission before publishing the plugin. Generic plugin validation checks policy structure without opening node-local files; the mode-aware dependency stage deduplicates paths and owns each successful snapshot as part of that configuration generation. A successful generation hands its snapshots to a build-scoped load session, which shares one verified snapshot among every geo instance using the path. Rejected generations release their handoffs, and pending handoffs are aggregate-size bounded. Unix file identity metadata permits unchanged snapshots to be reused without reading; platforms without content-safe identity metadata, including Windows, re-read and digest once per path per build so a same-length, timestamp-preserving replacement cannot retain stale policy. Existing live plugin generations keep their immutable snapshot while a restart or config reload validates and publishes the replacement. A readable oversized, corrupt, incompatible, wrong-product, or unsupported-code database is rejected; only an absent or unreadable node-local file degrades to `on_lookup_failure` in modes that permit that fallback.
