...
- Successful synthetic short-circuits from a later `before_proxy` plugin (for example `response_mock`, `fault_injection`, or `request_termination`) release token-matched local and Redis in-flight ownership once the shared H1/H2/H3 reject finalizer commits a final HTTP 2xx. That release is body-independent: empty 200 and 204/205/304 responses skip the synthetic response-body hook pipeline, but the finalizer still records a finalized-synthetic lifecycle signal that `on_response_committed` consumes. Ownership release always matches the exact local owner token and Redis lock token, so a late finalizer cannot clear a successor request's marker. Synthetic bodies are never stored under the idempotency key
