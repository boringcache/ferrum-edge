...
- Response-header-side, decided as soon as the backend headers arrive: a status outside `cacheable_status_codes` (or one whose caching semantics this plugin does not implement) and a `Content-Range` partial answer are refused *before* any invalidating branch of the store path, so they are released unconditionally. A `Set-Cookie` response and an origin-selected `text/event-stream` are refused *after* those branches, so they are released only once none of them applies. An event stream therefore flows to the client incrementally even when the client never sent `Accept: text/event-stream` — unless that same response also has to evict an entry.
- Responses whose store-side refusal *also evicts an existing entry* — `Cache-Control: no-store`/`private`/`no-cache`, a zero freshness lifetime (including a configured `ttl_seconds: 0`, where this applies to every response), an unusable `Vary: *` — deliberately stay buffered so that eviction still happens, and they win over the `Set-Cookie` / event-stream releases above.
