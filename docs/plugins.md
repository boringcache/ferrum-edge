...
config:
  ...
  limits:
    ...
    max_redaction_scan_bytes: 1048576      # hard processing bound; larger bodies skip the excerpt
    buffer_max_bytes: 134217728            # aggregate retained-byte budget for this instance
    max_stream_reservation_secs: 900       # bound on one active stream's staging + queue reservation
    # max_entry_bytes: <derived>           # defaults to the per-record contract implied above
  sink:
    type: http
