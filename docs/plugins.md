...
- **Local-mode state survives plugin-cache reload generations.** Active idempotency leases, retained completed responses, execution barriers, and their accounting are owned by a stable policy identity (`namespace` + plugin-config id), not by the plugin instance. A routine configuration reload that rebuilds the affected proxy/global plugin list hands the replacement instance the *same* local state, so an in-flight request cannot have its duplicate admitted against an empty map, and a completed entry is not forgotten (GHSA-wmqm-6mxj-gm9p). A scope move (`global` ↔ `proxy` ↔ `proxy_group`) that keeps the resource id keeps the state; two tenants that reuse a bare plugin-config id never share it. Changing `header_name`, switching between `local` and `redis`, or changing `on_redis_unavailable` is a semantic change: the replacement isolates onto fresh state, and the retired generation's late completion lands on the state it took its lease from rather than corrupting the new policy. `ttl_seconds`, `inflight_ttl_seconds`, `max_entries`, `max_entry_size_bytes`, and `max_total_size_bytes` are compatible changes — a tightened bound is applied against the already-retained state instead of resetting it. Retention is bounded: an identity's state lives only while a configured instance or an in-flight request still holds it, and removing the policy releases it
