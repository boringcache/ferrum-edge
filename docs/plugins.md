...
- **Semantic index refresh**: The local HNSW snapshot is immutable and rebuilt in batches. The first semantic entry is indexed immediately; later semantic inserts/removals mark the snapshot dirty and rebuild at most once every 30 seconds. Very recent entries may therefore exact-hit before they become eligible for semantic hits.
