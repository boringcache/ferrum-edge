...
**JSON-RPC batches.** A top-level JSON array is admitted as a JSON-RPC 2.0 batch. Empty arrays are rejected with a single `-32600 Invalid Request` response (not an array). Before session, catalog, or upstream work, the gateway enforces `validation.max_batch_bytes` (default 1 MiB) on the aggregate body, `validation.max_batch_items` (default 32) on member count, `validation.max_batch_item_bytes` (default 256 KiB) on each member's serialized size, and rejects nested arrays / non-object members. Each member is validated independently under the same method, authorization, schema, tool-policy, and hostile-input rules as a singleton. Response-bearing results preserve JSON-RPC `id` values and input order; notifications produce no response entry; an all-notification batch returns HTTP 202 with an empty body. Invalid individual members yield protocol-correct per-item errors without skipping validation of siblings, and error messages never echo unbounded attacker input. Aggregate mode executes batch members sequentially inside the gateway (no per-item task fan-out) so items that route to different upstreams still assemble one ordered response array. Transparent mode forwards an admitted batch to its single upstream after the same admission checks. Changing these limits on reload/update reconstructs admission; deleting the plugin removes batch handling with the instance.
