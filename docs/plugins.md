...
**The declared length is only an admission screen.** `Content-Length` can be stale, wrong, or overtaken by a transform that grows the body. Both final body hooks therefore re-check the actual post-transform `body.len()` against the configured cap *before* any UTF-8 scan, parse, redaction, or allocation, and emit a content-free `over_capture_limit` omission instead of a sample when it does not fit. The header-time screen is unchanged, so oversized and unknown-length traffic still never leaves the streaming path.
