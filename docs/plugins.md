...
> **Local counters survive a compatible reload.** Local (and Redis-fallback) enforcement state is owned by the policy — `namespace` + plugin type + plugin-config id — not by the plugin instance a config generation happened to construct. A full reload (file-mode `SIGHUP`, a data plane receiving a full control-plane snapshot, the database-mode fallback after a rejected delta), a change or deletion of *any* global plugin config in the namespace, an edit to the owning proxy's plugin associations, or a rebuilt shared HTTP client all reconstruct the plugin instance; none of them resets a consumed budget any more. A **semantic** change to the policy does start fresh state, so a retired generation can never charge against the replacement: for `rate_limiting` that is `limit_by` or `limits`; for `graphql` `limit_by`, `type_rate_limits`, or `operation_rate_limits`; for `grpc_method_router` `limit_by` or `method_rate_limits`; for `ai_rate_limiter` `token_limit`, `window_seconds`, `count_mode`, `limit_by`, `provider`, or `on_unmetered_response`; for `ws_rate_limiting` `frames_per_second` or `burst_size`; for `udp_rate_limiting` `datagrams_per_second`, `bytes_per_second`, or `window_seconds`. `sync_mode`, `redis_failure_policy`, and the effective `redis_key_prefix` are part of that set for every one of them, because they decide whether the local map is an enforcement domain at all and which counter domain it belongs to. Presentation-only fields (`expose_headers`, `close_reason`) and stateless checks (GraphQL depth/complexity/alias limits, gRPC `allow_methods`/`deny_methods`) are deliberately *not* — changing them must not hand every caller a fresh budget. Retention is bounded: state is held weakly and released as soon as the last plugin instance using it is dropped, so a removed policy is reclaimed and a policy re-added later starts fresh. Credentials are never part of this decision and never logged. This applies to `rate_limiting`, `graphql`, `grpc_method_router`, `ai_rate_limiter`, `ws_rate_limiting`, and `udp_rate_limiting`.
