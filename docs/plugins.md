...
| `sensitive_header_patterns` | String[] | `[]` | Additional lowercased substrings matched against the header name that extend the built-in deny-by-default credential set. Covers HTTP headers and native gRPC metadata (gRPC metadata is carried as HTTP/2 headers) |
