...
- Because the phase, not the body hook, owns those effects, they also happen exactly once. When some other plugin keeps the body buffered for its own reasons, `on_final_response_body` consumes the phase's staged decision instead of repeating it — and refuses to store an event stream that another plugin's buffering happened to collect. The staged decision is namespaced per instance, so multiple `response_caching` instances on one proxy stay isolated.
- The same predicate is the plugin's retry-time release vote, so a proxy whose only buffering plugin is `response_caching` no longer buffers every response merely because retries are configured.
