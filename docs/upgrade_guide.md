...
An HTTPS origin that requires a client certificate previously logged `error_class=connection_reset` (or the `request_error` catch-all) and `X-Gateway-Error: backend_error` when the gateway presented none. On the observed reqwest HTTP/1 path the rustls error is not in the request chain (hyper `is_canceled`), so the class is `connection_pool_error`; that typed cancellation is pre-wire and `X-Gateway-Error` is `connection_failure`. Typed rustls failures are `tls_error` only in independently known setup contexts. A rustls alert exposed after reqwest's connect phase remains `connection_reset`, because a peer-controlled alert does not prove that no request bytes were processed. This prevents `retry_on_connect_failure` from bypassing method idempotency for an ambiguous failure.

**Operator action:** retarget alerts for the observed pool-cancellation form onto `connection_pool_error` / `connection_failure`, while retaining `connection_reset` coverage for ambiguous post-connect TLS failures. Configure `backend_tls_client_cert_path` / `backend_tls_client_key_path` (or stop requiring client certs on the origin).
