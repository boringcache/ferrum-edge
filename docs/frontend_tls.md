...
3. **Fallback certificate.** A ClientHello with no SNI, or an SNI no certificate covers, is answered with the namespace's deterministic default — the catch-all listener (one with no `hostname`) when there is one, otherwise the oldest Gateway's first listener. Selection never fails a handshake for lack of a match; it presents the fallback, exactly as a single-certificate listener would.
...
**Collision behavior.** Two listeners in one namespace that declare the **same** `hostname` with **different** certificates are a conflict. The winner is deterministic — the older Gateway (by `creationTimestamp`), then `(namespace, gateway, listener)` order — and the loser fails closed: it keeps its `Accepted`/`ResolvedRefs` status, reports `Conflicted=True` with reason `HostnameConflict`, and serves no route traffic, so a hostname is never answered by an ambiguous certificate. Several `certificateRefs` on one listener are not a conflict, and two catch-all listeners are not either — they claim no name, so both stay served and selection falls back to each certificate's own SANs.

**Fail-closed loading.** A listener whose `certificateRefs` contains any reference that is missing, malformed, not a `kubernetes.io/tls` Secret, or not authorized by a `ReferenceGrant` is left entirely unmaterialized — never partially. At the data plane, if any certificate in the delivered set fails to load, parse, pair, or is expired, the whole snapshot is rejected and the previous configuration keeps serving.
...
**Bounds.** At most 256 Gateway certificates are admitted per configuration snapshot and at most 4096 SNI names are indexed; excess listeners are withdrawn with a warning rather than growing resident state with the cluster. A stapled OCSP response (`FERRUM_FRONTEND_TLS_OCSP_RESPONSE_SOURCE`) is bound to one certificate, so it is stapled only when the data plane serves exactly one Gateway certificate; with several it is not stapled to any and a warning is logged.
