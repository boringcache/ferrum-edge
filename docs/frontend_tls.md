...
| Fenced store commits | Each account/order/certificate write runs *while the lease table's exclusive lock is held*, after re-verifying holder, fence, and liveness under that lock. Acquisition and takeover block on the same lock, so a superseded instance cannot land a stale write alongside the new owner's — a before/after ownership check would detect that race but could not undo it. Final renewal publication persists the already-CA-valid order as `Valid` first and the certificate store second inside that same lease fence, so published material can never coexist with a stale local `Processing` order (which would permanently block later renewals). The lease document itself is never rewritten by a commit, so a target-store error propagates without disturbing any holder's claim. The lock is held only across the synchronous write(s), never across a network call, hook, or sleep. If the order write fails, certificate material is not published and reload is not requested. If the order write succeeds but the certificate write then fails for persistence reasons, reload is not requested and the renewal is reported as failed — the prior certificate remains due and the now-`Valid` order does not block a later retry. |
