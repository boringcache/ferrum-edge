      ...
      - name: Generate release notes
        ...
        run: |
          ...
          **exits** (default \`FERRUM_NODE_AGENT_FALLBACK_MODE=fail\`) rather than
          degrading. The published \`-ebpf\` image is based on Debian slim and
          includes \`iproute2\` for the supported NodeWaypoint ingress policy-rule
          lifecycle, but it deliberately omits \`iptables\`/\`ip6tables\`; the
          explicit iptables fallback therefore **crash-loops**. To use that
          fallback, build a custom runtime image that adds \`iptables\`/\`ip6tables\`
