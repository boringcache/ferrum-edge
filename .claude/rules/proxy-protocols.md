...
- H3 WebSocket bridges to the same backend WebSocket transport and frame-plugin pipeline as H1/H2; the H3 frontend never speaks WebSocket directly to a backend. Mesh-tagged WebSocket upgrades ride the shared `websocket_mesh_egress` / `connect_mesh_websocket_backend` path (issue #3620).
- H3 cross-protocol request bodies use bounded channel bridges: Plain streams into reqwest and non-replayable native gRPC streams DATA/trailers into `GrpcBody::Channel`; retry/body-plugin/pre-buffered gRPC retains the buffered representation. Mesh-tagged plain attempts force-buffer so the shared HBONE / mesh-mTLS pools can replay. Responses stream with the H3 coalesce window when retry/plugin policy allows.
...
- The H3→gRPC bridge, the H3→HTTP plain bridge, and the H3 WebSocket bridge all resolve mesh-tagged targets through the shared HBONE / Sidecar mesh-mTLS pools (issues #3284 / #3620). Native H3 QUIC dispatch still cannot carry mesh tunnels — mesh-tagged targets are forced onto the bridges. Unix-socket targets remain refuse-closed on every H3 surface (no Unix dialer). There is deliberately NO “direct dial anyway” arm for mesh: an unsupported class, an ambiguous both-tags target, or unmaterializable identity/SNI/trust metadata FAILS CLOSED. Mixed-upstream H3 retry rotation skips Unix-ineligible candidates and fails closed when no eligible transport remains.
