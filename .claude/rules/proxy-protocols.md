...
- HTTP/3 0-RTT is disabled by default, method-gated by `FERRUM_TLS_EARLY_DATA_METHODS`, strips client `Early-Data`, and reinjects `Early-Data: 1` outbound. When enabled on a non-mTLS listener, QUIC rustls `max_early_data_size` is `u32::MAX` (quinn accepts only `0` or `2^32-1`; no finite QUIC TLS byte cap). A listener with a frontend client-cert verifier sets that TLS value to `0` and refuses the 0.5-RTT `into_0rtt()` accept path (`zero_rtt_admitted`), because 0.5-RTT materialization precedes client authentication and makes `peer_identity()` unknowable. H3 peer identity + early-data state are one per-connection `ArcSwap` snapshot (`http3::peer_identity::H3ConnectionIdentity`), read once per accepted request stream and published exactly once when the handshake completion future resolves; an identity-bearing snapshot is never early data, and a failed/cancelled handshake leaves the slot empty.
