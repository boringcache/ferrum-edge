...
- `src/proxy/sni.rs` is the ONE ClientHello parser (opaque SNI routing, mesh inbound, kTLS gate, DTLS passthrough). `extract_sni_from_tcp_stream` answers "what hostname"; `peek_client_hello_sni`/`classify_client_hello` answer "and why not" as `ClientHelloSni`. The hostname answer is identical between them — `classify_client_hello` runs the lenient extractor FIRST and only subdivides the former `None`, reusing `tls_clienthello_wire_span` for framing and `client_hello_ktls_facts` as the strict whole-hello parser. Do not add a second parser and do not loosen the DNS validator (kTLS depends on its strictness).
