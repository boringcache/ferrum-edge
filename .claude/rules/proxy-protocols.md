...
- HTTP/3 0-RTT is disabled by default, method-gated by `FERRUM_TLS_EARLY_DATA_METHODS`, strips client `Early-Data`, and reinjects `Early-Data: 1` outbound. When enabled, QUIC rustls `max_early_data_size` is `u32::MAX` (quinn accepts only `0` or `2^32-1`; no finite QUIC TLS byte cap). The 0.5-RTT `into_0rtt()` accept path is refused outright when the H3 listener has a frontend client-cert verifier (`zero_rtt_admitted`), because TLS 1.3 does not accept early data under client auth and 0.5-RTT materialization makes `peer_identity()` unknowable. H3 peer identity + early-data state are one per-connection `ArcSwap` snapshot (`http3::peer_identity::H3ConnectionIdentity`), read once per accepted request stream and published exactly once when the handshake completion future resolves; an identity-bearing snapshot is never early data, and a failed/cancelled handshake leaves the slot empty.
