...
- A rejected CP/DP trust-bundle reload retains the ENTIRE last accepted verifier, but only for `FERRUM_CP_DP_TRUST_MAX_STALE_SECONDS` (default 900, monotonic, reset only by an accepted reload including a semantically unchanged one). The first refusal marks trust reload degraded on authenticated `/health` (`cp_dp_trust`) and in the `ferrum_cp_dp_trust_*` families; at the bound readiness fails and `bind_to_store` refuses new ConfigSync/MeshSubscribe/SotW-ADS/Delta-ADS admissions while the shared lease ends established streams with `trust_stale`. An unexpectedly dead reload worker fails readiness immediately; a clean shutdown never counts as a failure. `0` is unbounded and requires `FERRUM_CP_DP_TRUST_ALLOW_UNBOUNDED_STALE=true`; a bound below three poll intervals fails startup. Published state is fixed-cardinality and carries no path, `kid`, namespace, token, or key material — and no generation identifier, fingerprint, or digest, however re-hashed or truncated, because any deterministic value derived from credential material is an offline verification oracle. The internal configuration fingerprint stays private to the reload worker as a change detector.
