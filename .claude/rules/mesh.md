...
- One node-waypoint listener serves many pods; source pod identity comes from the eBPF `connect4`/`connect6` cgroup-stamped `SO_COOKIE` record, resolved through `NodeWaypointIdentityResolver`. The hooks read the identity from `FERRUM_WORKLOAD_IDENTITY` keyed by `bpf_get_current_cgroup_id()` — the *container leaf* cgroup, a child of the pod cgroup on every K8s cgroup driver — so the node-agent enrolls the identity under the **whole pod cgroup subtree** (pod inode + descendant container-cgroup inodes via `cgroup::collect_cgroup_tree_inodes`) and re-walks on reconcile so late-starting/restarted containers are picked up. Keying only the pod inode would miss the leaf and fail closed in real pods. (`FERRUM_INCLUDE_PORTS` keys the pod inode only and shares the leaf limitation, but fails open — it only fails to narrow, never closed.)
- **Lazy-identity reclamation:** the production enrollment path (hash-join in `resolve_record`) binds no cgroup and nothing calls `remove_identity`, so the periodic sweep (`sweep_cgroup_stale_identities`, gated by `FERRUM_MESH_NODE_WAYPOINT_CGROUP_SWEEP_INTERVAL_SECS`) reclaims non-cgroup-bound identities whose `pod_uid` is no longer referenced by any live cookie record — a churned pod's accept-side cookies age out of `cookie_records`, so its orphaned identity and per-pod scope are reclaimed instead of accumulating. A still-live pod re-enrolls on its next connection; in-flight connections keep their already-resolved scope. cgroup-bound identities remain the cgroup-binding pass's domain — the idle pass never touches them.
