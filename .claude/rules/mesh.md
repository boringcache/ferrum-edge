...
- **GAP-2M accept-side bridge** (applies to both HBONE/HTTP and TCP paths equally; **IPv4 only**): `connect4`/`connect6` key the orig-dst record by the connecting-socket cookie, but accept paths look up the accept-side `SO_COOKIE`. The bridge is implemented in the kernel `sock_ops` program (`ebpf/ferrum-ebpf/src/sock_ops.rs`): at active-established it re-keys the IPv4 record by `(netns cookie, connection 4-tuple)` (`FERRUM_ORIG_DST_BY_TUPLE4`), and at passive-established it re-stamps it into `FERRUM_ORIG_DST4` under the accept-side cookie, so the resolver (fed by `orig_dst_bridge`) resolves scoped policies with no further proxy-side changes. Per-pod identity enrollment is wired with no separate channel: slice apply installs a `workload_spiffe_hash` → SPIFFE index alongside the policy scopes, and `resolve_record` lazily enrolls `pod_uid` → identity by hash-joining that index against the eBPF-stamped `(pod_uid, workload_spiffe_hash)` on first sight (an unmatched hash fails closed as `UnknownPod`). The current slice index is re-checked on **every** resolve, not only first-sight: a cached `pod_uid` whose workload a later slice removes (its hash leaves the index) stops resolving — the cached-identity branch fails closed (HBONE drops, TCP downgrades to mesh-wide) instead of serving the orphaned identity, and re-validates for free if the workload returns. The gate (`workload_spiffe_hash`→SPIFFE) and the scope indexes are **one `ArcSwap<NodeWaypointSlice>` generation**: scope is keyed by the exact **pod UID** the eBPF stamps (`scopes_by_pod_uid`, built from the slice's per-pod workloads' `metadata.uid`), with a SPIFFE-keyed fallback only for uid-less VM/WorkloadEntry workloads — so pods that share a service account (one SPIFFE) but carry different labels are scoped independently rather than collapsed to the label intersection. `resolve_*` returns the identity **and** its scope from a single load of that generation, so a slice reload can never pair a resolved identity with an old/missing scope (the "never partial" invariant for scoped authz). The TCP path captures both at accept and persists them for the stream; HBONE/HTTP re-queries the scope per request, each read from one coherent generation. The `netns_cookie` (`bpf_get_netns_cookie`) is load-bearing, not decoration: captured connections all target `127.0.0.1:15001`, so the 4-tuple collapses to loopback + an ephemeral port that can collide across pod netns in this one global map — the per-netns cookie keeps each pod's key distinct (preventing cross-pod record overwrite / identity misattribution) while staying identical for the two ends of a same-netns connection. Do not drop it from the key. **IPv6 is not bridged** — aya's SockOpsContext `local_ip6`/`remote_ip6` accessors copy the whole `[u32;4]` out of the ctx, which the verifier rejects, so IPv6 node-waypoint resolution stays fail-closed (pre-GAP-2M behavior) pending a verifier-safe per-element v6 read. **Verification:** CI compile-checks the program (`build-ebpf`) and load/attach-tests every program — incl. the bridge — on a real ≥5.7 kernel (`ebpf-live`, which already caught and forced fixes for `connect6`/`getpeername6`/`sock_ops` verifier rejections); the full connect→capture→accept datapath (tuple byte-order, end-to-end resolution) is **not** CI-verified and needs a live multi-pod node. Safe by construction: a tuple/byte-order mismatch writes no accept-side record → resolution stays fail-closed, never misattributed.
- **In-netns outbound capture listeners** (opt-in, `FERRUM_MESH_NODE_WAYPOINT_IN_NETNS_LISTENERS_ENABLED`, default `false`): `connect4` rewrites a captured pod's outbound to `127.0.0.1:15001` in the **pod's** loopback, but the default single outbound listener binds `127.0.0.1:15001` in the host/proxy netns and can never receive a pod loopback connection — so without this flag node-waypoint outbound capture is non-functional on a real multi-pod node (connections refused, scoped enforcement never engages); the GAP-2M `sock_ops` cookie bridge also independently requires the accept socket to share the connecting pod's netns. With the flag on, `NetnsCaptureManager` (`src/proxy/netns_capture.rs`) opens one `127.0.0.1:15001` listener **inside each enrolled pod's netns** (`setns(CLONE_NEWNET)` on a dedicated thread, deduped by netns inode), so captured pod-loopback connections are accepted there and the same-netns cookie bridge resolves source identity. Pod discovery reuses the pinned-path IPC pattern: the node-agent publishes a registry dir `FERRUM_MESH_NODE_WAYPOINT_POD_REGISTRY_DIR` (default `/run/ferrum/node-waypoint-pods`), one file per enrolled pod (name = pod UID, contents = pod cgroup path), written on enroll / removed on teardown; the manager polls and reconciles listeners (open on add, close on removal). The listener binds the configured `outbound_listen_addr` (not a hardcoded `15001`; skipped when that port is `0`), shares one `max_connections`-sized connection semaphore across all in-netns listeners, reopens when the pod's registered IP changes (so `source_ip_override` tracks the pod IP), and closes when a pod UID moves to a different netns (preserving the transient-unresolvable grace). **Deferred-redirect readiness gate:** to avoid redirecting a pod before its listener exists, the node-agent defers attaching `connect4`/`connect6` at enrollment and enables them only after the proxy writes `<registry_dir>/.ready/<pod_uid>` (on listener open; removed on close), polling on a 1s tick with a ~30s bounded fallback — so the registry dir must be mounted RW for the proxy. Linux-only (`setns`, `/proc/<pid>/ns/net`); non-Linux is an unsupported stub. Reconcile/registry logic is unit-tested; the `setns`/bind path and full pod-loopback datapath are **not** CI-testable (need a live multi-pod node) — implemented, live-datapath-unverified.
