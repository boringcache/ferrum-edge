...
- **GAP-2M accept-side bridge** (applies to both HBONE/HTTP and TCP paths equally): `connect4`/`connect6` key the orig-dst record by the connecting-socket cookie, but accept paths look up the accept-side `SO_COOKIE`. The bridge is now implemented in the kernel `sock_ops` program (`ebpf/ferrum-ebpf/src/sock_ops.rs`): at active-established it re-keys the record by the connection 4-tuple (`FERRUM_ORIG_DST_BY_TUPLE4/6`), and at passive-established it re-stamps it into `FERRUM_ORIG_DST4/6` under the accept-side cookie, so the resolver (fed by `orig_dst_bridge`) resolves scoped policies with no further proxy-side changes. **Verification:** CI compile-checks the program (`build-ebpf`) and load/attach-tests it on a real ≥5.7 kernel (`ebpf-live`); the full connect→capture→accept datapath (tuple byte-order, end-to-end resolution) is **not** CI-verified and needs a live multi-pod node. Safe by construction: a tuple/byte-order mismatch writes no accept-side record → resolution stays fail-closed, never misattributed.
