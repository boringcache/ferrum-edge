...
- Branch protection must directly require `Tests`, `Chart Runtime Lint`, `Merge Coverage`, `Gateway API Conformance`, `Mesh E2E Sidecar Live`, `Trusted Cross Build Policy`, and `Multicluster Federation Live`. `Chart Runtime Lint` is the always-on `chart-runtime-lint` job in `ci.yml`; it is required directly rather than through the `Tests` aggregate because `Trusted Cross Build Policy` compares that aggregate byte for byte. The dedicated workflows trigger on every PR and on `merge_group`, and path-filter internally; do not add polling mirror jobs back to `ci.yml`. See `docs/ci_cd.md` for merge-queue SHA semantics and staged enablement.
