...
- Smart/default path discovery in `src/cli.rs` must yield to a suffixed external source (`cli::externally_sourced` -> `secrets::external_source_configured`). Discovery runs in `main()` *before* resolution, so an auto-set `FERRUM_CONF_PATH`/`FERRUM_FILE_CONFIG_PATH` would otherwise look like a competing user-provided source and fail the command with a multiple-sources error in any directory containing `./ferrum.conf` or `./resources.yaml`. File-mode inference (`cli::infer_file_mode`) counts a suffixed spec source as "configured" for the same reason. An **explicit** `-s`/`-c` path is a real conflict and still errors. `EXTERNAL_SECRET_SUFFIXES` is deliberately not feature-gated: an unsupported suffix is a fail-closed error, so every build must agree a key is externally sourced.
