...
- `tests/integration/vendor_integrity_tests.rs` hashes every governed `vendor/`
  file (LF-normalized SHA-256) against `vendor/VENDOR_INTEGRITY.sha256` and runs
  in the `protocols-data-plane` integration shard. Incidental vendor
  `Cargo.lock` files are ignored unless listed in `GOVERNED_VENDOR_LOCKFILES`
  (currently the committed dimpl standalone-regression lockfile). Drift beyond
