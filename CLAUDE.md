...
**Multi-namespace control plane** (T2-A): `FERRUM_CP_NAMESPACES` lifts the historic one-CP-per-namespace restriction. Empty/unset preserves the back-compat `Single(FERRUM_NAMESPACE)` scope; `*` makes the CP cluster-wide; CSV `prod,staging` defines an explicit `Set`. Per-namespace `tokio::broadcast` partitions in `src/grpc/cp_server.rs::NamespaceBroadcasts` guarantee a DP only receives its own namespace's slice, with the DP-side `dp_client::filter_config_to_namespace` as defense in depth. The optional `FERRUM_CP_REQUIRE_NAMESPACE_CLAIM=true` requires every DP `ConfigSync.Subscribe` JWT to carry an `ns` claim (string or array) authorising the subscribe namespace. Full operator guide: [docs/cp_namespace_tenancy.md](docs/cp_namespace_tenancy.md). xDS ADS, `MeshConfigSync`, and the K8s controller's broadcast hook still use the back-compat single-namespace sender — multi-namespace expansion and `ns`-claim enforcement for those surfaces are tracked separately.

**K8s controller default-on inside pods** (T2-B): `FERRUM_K8S_CONTROLLER_ENABLED` and `FERRUM_K8S_POD_DISCOVERY_ENABLED` default to `true` when the gateway process is inside a Kubernetes pod (detected via `KUBERNETES_SERVICE_HOST`, the same heuristic `kube-rs::Config::incluster()` uses). Outside a pod the historic `false` default applies so tests and CLI runs are unaffected. Explicit `=false` from the operator always wins (env var or `ferrum.conf`). The controller's watch scope falls back to the T2-A CP scope when `FERRUM_K8S_WATCH_NAMESPACES` is unset: `CpScope::Single`/`Set` → namespaced watches; `CpScope::All` → cluster-wide. T2-B also patches `status.conditions[]` on Istio `AuthorizationPolicy`, `PeerAuthentication`, and `DestinationRule` CRDs via `IstioStatusWriter` in `src/k8s_controller/istio_status.rs` so `kubectl describe` surfaces translation outcome. The writer is built only when `watch_istio == true`; failures (e.g. CRD missing `subresources.status`) log a warn and otherwise no-op. The remaining Istio CRDs (`VirtualService`, `ServiceEntry`, `RequestAuthentication`, `Sidecar`, `Telemetry`, `WorkloadEntry`) are deferred to a follow-on.
...
- `FERRUM_CP_NAMESPACES` (unset) — multi-namespace CP scope: empty = back-compat single namespace, `*` = cluster-wide, CSV = explicit set. See [docs/cp_namespace_tenancy.md](docs/cp_namespace_tenancy.md)
- `FERRUM_CP_REQUIRE_NAMESPACE_CLAIM` (`false`) — when `true`, DP `ConfigSync.Subscribe` JWTs must carry an `ns` claim (string or array) authorising the subscribe namespace. Multi-tenant deployments should enable
