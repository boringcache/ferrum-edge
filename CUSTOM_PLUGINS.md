...
| `fn correlation_id_header_name(&self) -> Option<&str>` | `None` | Return the normalized correlation header owned by this instance. Candidate admission and runtime cache construction reject duplicate effective headers or priorities on one plugin chain, including custom-only chains. |
