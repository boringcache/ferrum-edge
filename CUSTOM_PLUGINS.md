...
| `fn correlation_id_header_name(&self) -> Option<&str>` | `None` | Return the correlation header owned by this instance. Core candidate admission and runtime cache construction defensively trim and compare claims ASCII-case-insensitively, rejecting duplicate effective headers or priorities on one plugin chain, including custom-only chains. Custom plugins must still trim, validate, and normalize the header names used by their own runtime writes. |
