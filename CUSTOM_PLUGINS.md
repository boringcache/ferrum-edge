...
| `fn egresses_request_body_before_finalization(&self) -> bool` | `false` | Set to `true` if `before_proxy` sends the buffered request body to an external service before request transforms/final hooks. Candidate admission and runtime cache construction then reject same-protocol body-transform compositions and same-protocol final request-body policy plugins (`enforces_finalized_request_policy()`). |
...
- [ ] `egresses_request_body_before_finalization()` returns `true` if `before_proxy` sends body bytes to an external service before finalization (candidate admission then refuses same-protocol body transformers and final request-body policy plugins)
