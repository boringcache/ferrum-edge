...
| `fn allows_hbone_inner_reuse(&self) -> bool` | `!is_authorize_plugin()` | Whether one admitted mesh HBONE CONNECT may carry LATER application operations without running this plugin again. `is_authorize_plugin()` itself defaults to `true`, so a custom plugin refuses by default and the destination simply stops advertising `x-ferrum-mesh-tunnel-reuse: fenced` while it is attached — costing one CONNECT per operation, never correctness. Override to `true` only when the plugin takes no per-operation decision and has no per-operation side effect, or when every decision it takes is re-issued for the tunnel's whole life by the HBONE admission fence. Anything that charges an operation (a token, a permit, a mirror, an external check) or authenticates the CONNECT with a bearer credential must leave it `false`: the fence bounds the peer's mTLS leaf and nothing else. See [docs/mesh.md](docs/mesh.md) → "HBONE Inner Application Connection Reuse". |
