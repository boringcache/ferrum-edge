...
| `fn correlation_id_header_name(&self) -> Option<&str>` | `None` | Return the correlation header owned by this instance. Custom plugins should normalize it for their own runtime use; candidate admission and runtime cache construction defensively compare it case-insensitively and reject duplicate effective headers or priorities on one plugin chain, including custom-only chains. |
