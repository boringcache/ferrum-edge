...
/// silently approximated. Credentialed effectively universal prefix/regex
/// matchers are deferred the same way (issue #4269).
#[test]
...
/// Credentialed effectively universal prefix/regex origin matchers cannot be
/// projected: they would reflect an arbitrary origin with credentials. Exact
/// `*` keeps the documented deferral; these matchers are refused the same way
/// rather than silently dropping credentials (issue #4269).
#[test]
fn vs_cors_policy_credentialed_universal_matcher_not_projected() {
    register_feature!(
        category = CATEGORY,
        feature = "http[].corsPolicy credentialed universal prefix/regex",
        status = Status::Deferred,
        notes = "allowCredentials plus an effectively universal prefix or regex is fail-closed: left unprojected (deferred) rather than reflecting arbitrary origins with credentials or silently dropping credentials.",
    );
    for origins in [
        json!([{"prefix": "https://"}]),
        json!([{"prefix": "h"}]),
