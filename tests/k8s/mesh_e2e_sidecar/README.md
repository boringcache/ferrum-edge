...
| `sidecar.destination_rule.export_to_namespace_visibility` | the cross-namespace target service's public 2000ms rule is changed to namespace-local `export_to: ["."]`; it must NOT apply to the client in `ferrum`, so the black-holed dial falls back to the built-in 5000ms default and rises well above the 2000ms baseline (issue #2465, fail-closed direction) |
| `sidecar.destination_rule.lookup_tier_client_wins` | for the service in `ferrum-other-tenant`, a client-namespace rule at 2000ms and a root-namespace (`istio-system`) rule at 8000ms both target the host; the client rule must win even though `istio-system` sorts LAST lexically, so the dial stays in the ~2s window (issue #2469) |
