...
| `sidecar.destination_rule.export_to_namespace_visibility` | the SAME 2000ms rule re-exported only to `ferrum-other-tenant` (a namespace this client is not in) must NOT apply: the black-holed dial falls back to the built-in 5000ms default and the observed time must rise well above the 2000ms baseline (issue #2465, fail-closed direction) |
| `sidecar.destination_rule.lookup_tier_client_wins` | a client-namespace rule at 2000ms and a root-namespace (`istio-system`) rule at 8000ms both target the host; the client-namespace rule must win even though `istio-system` sorts LAST lexically, so the dial stays in the ~2s window (issue #2469) |
