...
//!
//! Absent → default. Out-of-range numbers clamp. Malformed values fail closed.
//! `0` is never unlimited. Diagnostics name the setting and never echo the
//! configured value (every `FERRUM_*` key is externally sourceable).
...
#[test]
fn zero_and_oversized_values_clamp_rather_than_becoming_unlimited() {
    assert_eq!(
        parse_tls_max_material_size_bytes(Some("0")).expect("zero is numeric"),
