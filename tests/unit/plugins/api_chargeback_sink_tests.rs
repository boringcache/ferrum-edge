...
async fn openapi_schema_matches_runtime_admission_boundaries() {
    ...
    let retry_err = validate_plugin_config("api_chargeback_sink", &inverted_retry)
        .err().unwrap_or_else(|| panic!("inverted retry bounds must fail runtime admission"));
    assert!(
    ...
    let budget_err = validate_plugin_config("api_chargeback_sink", &over_budget)
        .err().unwrap_or_else(|| panic!("over-budget retry must fail runtime admission"));
    assert!(
...
fn spool_rejects_empty_spool_oversized_batch() {
    ...
    let err = spool
        ...
        .err().unwrap_or_else(|| panic!("one-byte-over batch must be rejected on an empty spool"));
    assert!(
...
fn spool_quota_uses_compressed_encoded_size() {
    ...
    let err = over_spool
        .write_events(std::slice::from_ref(&event))
        .err().unwrap_or_else(|| panic!("compressed one-byte-over must reject"));
...
fn replaced_namespace_coordination_inode_refuses_spool_mutation() {
    ...
    let error = spool
        ...
        .err().unwrap_or_else(|| panic!("a replaced coordination inode must fail closed"));
...
fn hard_linked_namespace_coordination_inode_refuses_spool_mutation() {
    ...
    let error = spool
        ...
        .err().unwrap_or_else(|| panic!("a hard-linked coordination inode must fail closed"));
...
fn independent_managers_cannot_over_admit_streamed_dead_letter_appends() {
    ...
    let error = second_result.err().unwrap_or_else(|| panic!("second protected append must fail closed"));
    assert!(
...
fn spool_walk_refuses_a_probe_that_fails_for_a_reason_other_than_not_found() {
    ...
    let error = manager
        .scan_stats_for_tests()
        .err().unwrap_or_else(|| panic!("an EACCES probe must refuse the walk, not be skipped"));
...
fn quota_eviction_fails_closed_when_the_inventory_never_stabilizes() {
    ...
    let error = spool
        .evict_until_can_admit_for_tests(file_len)
        .err().unwrap_or_else(|| panic!("a perpetually mutating namespace must refuse admission"));
    set_spool_write_hook_for_tests(None);
...
fn failed_atomic_spool_write_removes_tmp_and_does_not_publish() {
    ...
    .err().unwrap_or_else(|| panic!("rename onto a directory must fail the atomic publish"));
    assert!(
...
fn durable_config_rejects_wait_for_async_insert_zero_without_lossy_opt_in() {
    ...
    let err = validate_plugin_config("api_chargeback_sink", &config)
        .err().unwrap_or_else(|| panic!("wait_for_async_insert=0 must be rejected in durable mode"));
    assert!(
    ...
    let unknown = validate_plugin_config("api_chargeback_sink", &config)
        .err().unwrap_or_else(|| panic!("unknown clickhouse keys must stay rejected"));
    assert!(
...
async fn replay_keeps_original_when_dead_letter_metadata_cannot_be_written() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("dead-letter metadata failure must stop the replay tick"));
...
async fn dead_letter_recovery_reuses_quota_held_by_a_partially_published_payload() {
    ...
    replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("blocked metadata publication must retain the source"));
...
async fn dead_letter_payload_publish_refuses_symlink_and_restores_the_source_claim() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a planted dead-letter symlink must fail closed"));
...
async fn dead_letter_payload_admits_each_append_before_exceeding_spool_quota() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("the uncompressed dead-letter copy must not exceed spool.max_bytes"));
    set_spool_write_hook_for_tests(None);
...
fn tampered_dead_letter_source_credit_record_grants_no_capacity() {
    ...
    let error = publish_dead_letter_payload_for_claim_for_tests(&spool, &claim, &row)
        .err().unwrap_or_else(|| panic!("a credit no durable record witnesses must not be spent"));
...
fn replaced_dead_letter_source_credit_path_grants_no_capacity() {
    ...
    let error = publish_dead_letter_payload_for_claim_for_tests(&spool, &claim, &row)
        .err().unwrap_or_else(|| panic!("a replaced credit pathname must invalidate the old locked descriptor"));
    assert!(
        error.contains("cannot fit within spool.max_bytes"),
        "unexpected quota diagnostic: {error}"
    );
...
fn resized_pinned_source_invalidates_dead_letter_credit() {
    ...
    let error =
        publish_dead_letter_payload_for_claim_for_tests(&spool, &claim, &expanded_rejection)
            .err().unwrap_or_else(|| panic!("a stale source length must not raise the admission ceiling"));
...
fn independent_managers_cannot_double_spend_the_dead_letter_source_credit() {
    ...
    let error = publish_dead_letter_payload_for_claim_for_tests(&second, &claim_b, &row)
        .err().unwrap_or_else(|| panic!("an uncredited concurrent handoff must fail closed"));
...
fn renamed_aside_dead_letter_source_credit_path_grants_no_capacity() {
    ...
    let error = publish_dead_letter_payload_for_claim_for_tests(&spool, &claim, &row)
        .err().unwrap_or_else(|| panic!("capacity without a live coordination inode must fail closed"));
    assert!(
        error.contains("cannot fit within spool.max_bytes"),
        "unexpected quota diagnostic: {error}"
    );
...
fn oversized_dead_letter_source_credit_record_grants_no_capacity() {
    ...
    let error = publish_dead_letter_payload_for_claim_for_tests(&spool, &claim, &row)
        .err().unwrap_or_else(|| panic!("oversized credit body must not admit the rejected copy"));
    assert!(
        error.contains("cannot fit within spool.max_bytes"),
        "unexpected quota diagnostic: {error}"
    );
...
async fn replay_stops_on_retryable_redirect_auth_408_429_and_5xx_without_removing_file() {
    for status in [302u16, 401u16, 403u16, 408u16, 429u16, 500u16, 503u16] {
        ...
        let err = replay_spool_once_for_tests(&spool, &server.uri())
            .await
            .err().unwrap_or_else(|| panic!("retryable status must fail the replay tick"));
        assert!(
...
async fn replay_stops_on_network_failure_without_removing_file() {
    ...
    let err = replay_spool_once_for_tests(&spool, "http://127.0.0.1:1/")
        .await
        .err().unwrap_or_else(|| panic!("unreachable ClickHouse must be retryable"));
    assert!(
...
async fn replay_keeps_spool_on_ambiguous_non_empty_200_acknowledgement() {
    ...
    let err = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("ambiguous acknowledgement must be retryable"));
    assert!(
...
async fn replay_retries_after_ambiguous_acknowledgement_without_double_billing_identity_change() {
    ...
    let first = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("first ambiguous ACK must keep the file"));
...
fn spool_before_write_gate_timeout_names_unreachable_gate_state() {
    ...
    let error = gate
        ...
        .err().unwrap_or_else(|| panic!("an unreachable BeforeWrite gate must fail closed"));
    assert!(
...
fn spool_path_component_rejects_or_encodes_escape_forms() {
    ...
    let nul_err = SpoolManager::encode_spool_path_component_for_tests("x\0y")
        .err().unwrap_or_else(|| panic!("NUL must be rejected"));
    assert!(nul_err.contains("NUL"));
    let empty_err = SpoolManager::encode_spool_path_component_for_tests("   ")
        .err().unwrap_or_else(|| panic!("a whitespace-only component must not become a path segment"));
...
fn spool_metadata_owner_mismatch_fails_closed_without_mutating_records() {
    ...
    let err = spool
        ...
        .err().unwrap_or_else(|| panic!("mismatched ownership metadata must fail closed"));
    assert!(
...
fn oversized_namespace_metadata_fails_closed_without_mutating_records() {
    ...
    let err = spool
        .list_replayable_spool_files_for_tests()
        .err().unwrap_or_else(|| panic!("an oversized ownership manifest must fail closed"));
...
fn foreign_owner_tagged_records_are_never_replayed_or_evicted() {
    ...
    let err = spool
        .write_events(std::slice::from_ref(&event))
        .err().unwrap_or_else(|| panic!("eviction must not be able to reclaim another owner's bytes"));
    assert!(
...
fn spool_walk_bounds_empty_directory_entries() {
    ...
    let error = spool
        ...
        .err().unwrap_or_else(|| panic!("empty directories must count toward the traversal bound"));
    assert!(
...
fn namespace_root_symlink_swap_fails_closed() {
    ...
    let error = spool
        .list_replayable_spool_files_for_tests()
        .err().unwrap_or_else(|| panic!("a swapped namespace-root symlink must fail closed"));
    assert!(
...
fn spool_scan_survives_directory_cycles_and_bounds_depth() {
    ...
    let err = spool
        ...
        .err().unwrap_or_else(|| panic!("traversal beyond the depth bound must fail closed"));
...
fn admission_eviction_never_unlinks_a_live_peer_generation_temp() {
    ...
    let err = gen2
        .write_events(std::slice::from_ref(&event))
        .err().unwrap_or_else(|| panic!("admission must fail closed rather than evict a live peer temp"));
    assert!(
        err.contains("cannot fit within spool.max_bytes"),
        "unexpected error: {err}"
    );
...
fn admission_eviction_never_unlinks_a_fresh_peer_process_temp() {
    ...
    let err = spool
        .write_events(std::slice::from_ref(&event))
        .err().unwrap_or_else(|| panic!("a peer process's fresh temp must not be evicted to admit a write"));
    assert!(
        err.contains("cannot fit within spool.max_bytes"),
        "unexpected error: {err}"
    );
...
fn compressed_spool_record_expanding_past_its_bound_fails_closed() {
    ...
    let err = decode_spool_file_for_tests(&bomb_path)
        .err().unwrap_or_else(|| panic!("a high-ratio archive must not expand without bound"));
    assert!(
...
fn spool_replay_caps_large_encoded_and_decoded_artifacts_absolutely() {
    ...
    let err = decode_spool_file_for_tests(&oversized_path)
        .err().unwrap_or_else(|| panic!("an oversized raw artifact must fail before allocation"));
    assert!(
...
fn atomic_spool_write_fault_injection_surfaces_before_success() {
    ...
    for fault in faults {
        ...
        .err().unwrap_or_else(|| panic!("an injected durable-write fault must fail the write"));
        assert!(
...
fn rollback_before_rename_preserves_a_peer_published_shared_final() {
    ...
    for fault in [SpoolFsFault::FileSync, SpoolFsFault::Rename] {
        ...
        .err().unwrap_or_else(|| panic!("an injected pre-rename fault must fail the write"));
        assert!(
            err.contains("injected fault"),
            "unexpected error for {fault:?}: {err}"
        );
...
fn rollback_after_rename_never_unlinks_a_shared_final() {
    ...
    .err().unwrap_or_else(|| panic!("an injected directory-sync fault must fail the write"));
...
fn shared_manifest_rollback_leaves_live_storage_unprepared() {
    ...
    let err = faulted
        ...
        .err().unwrap_or_else(|| panic!("an unsynced manifest publish must fail the write"));
    assert!(err.contains("injected fault"), "unexpected error: {err}");
    ...
    let err = faulted
        ...
        .err().unwrap_or_else(|| panic!("a retry under the same fault must not silently succeed"));
    assert!(err.contains("injected fault"), "unexpected error: {err}");
    assert_eq!(
        disk_owned_bytes(&default_test_namespace_root(temp.path())),
        0,
...
fn injected_file_sync_failure_keeps_the_spool_batch_uncommitted() {
    ...
    let err = faulted
        ...
        .err().unwrap_or_else(|| panic!("a failed durable handoff must be reported to the caller"));
    assert!(err.contains("injected fault"), "unexpected error: {err}");
...
fn injected_rename_failure_keeps_the_spool_batch_uncommitted() {
    ...
    let err = faulted
        ...
        .err().unwrap_or_else(|| panic!("a failed rename must be reported to the caller"));
    assert!(err.contains("injected fault"), "unexpected error: {err}");
    assert_eq!(
        disk_owned_bytes(&default_test_namespace_root(temp.path())),
        0,
...
fn injected_directory_open_failure_rolls_back_the_publish() {
    ...
    let err = faulted
        ...
        .err().unwrap_or_else(|| panic!("a directory-open failure must not be reported as a durable commit"));
    assert!(err.contains("injected fault"), "unexpected error: {err}");
    assert_eq!(
        disk_owned_bytes(&default_test_namespace_root(temp.path())),
        0,
...
fn injected_directory_sync_failure_rolls_back_the_publish() {
    ...
    let err = faulted
        ...
        .err().unwrap_or_else(|| panic!("a directory-sync failure must not be reported as a durable commit"));
    assert!(err.contains("injected fault"), "unexpected error: {err}");
    assert_eq!(
        disk_owned_bytes(&default_test_namespace_root(temp.path())),
        0,
...
async fn replay_claim_is_excluded_from_eviction_and_released_on_retryable() {
    ...
    let err = spool
        ...
        .err().unwrap_or_else(|| panic!("an in-flight claim is never an eviction candidate"));
    ...
    let err = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("503 must remain retryable"));
    assert!(!err.is_empty());
    assert!(
        oldest.exists(),
...
fn managed_path_containment_rejects_escape_and_absolute_replacement() {
    ...
    // filesystem call.
    let popped = within_root("spool/../../etc/passwd").err().unwrap_or_else(|| panic!("escape refused"));
    assert!(popped.contains("escapes root"), "{popped}");

    // A sibling tree that merely shares a parent is outside the root.
    let sibling = within_root("other/a.ndjson").err().unwrap_or_else(|| panic!("sibling refused"));
    assert!(sibling.contains("outside root"), "{sibling}");

    // An absolute component replaces the join instead of extending it.
    let absolute = within_root("/etc/shadow").err().unwrap_or_else(|| panic!("absolute refused"));
...
fn claiming_a_foreign_or_non_replayable_spool_file_is_refused() {
    ...
    let non_replayable = spool
        ...
        .err().unwrap_or_else(|| panic!("an in-flight claim must not be re-claimed"));
    assert!(
    ...
    let refused = spool
        ...
        .err().unwrap_or_else(|| panic!("a foreign-tagged record must not be claimable"));
    ...
    let not_a_claim = SpoolManager::claim_restore_path_for_tests(&foreign)
        .err().unwrap_or_else(|| panic!("only claim markers can be restored"));
    assert!(
...
fn chargeback_insert_body_is_refused_rather_than_serialized_under_a_saturated_ceiling() {
    ...
    let error = probe_charge_body_materialization_for_tests(ceiling, &events)
        .err().unwrap_or_else(|| panic!("a 1 KiB ceiling cannot admit an eight-row insert body"));
    assert!(
...
async fn spool_replay_refuses_before_decoding_when_the_ceiling_cannot_hold_the_artifact() {
    ...
    let error = replay_spool_once_with_ceiling_for_tests(&spool, "http://127.0.0.1:1/", 4, ceiling)
        .await
        .err().unwrap_or_else(|| panic!("a ceiling below the artifact size must defer replay"));
...
fn spool_artifact_materialization_fails_closed_on_row_count_row_bytes_and_replay_ceiling() {
    ...
    let error = spool
        ...
        .err().unwrap_or_else(|| panic!("row-count overflow must refuse before serialization"));
    assert!(
    ...
    let error = spool
        ...
        .err().unwrap_or_else(|| panic!("an oversize conservative row bound must refuse"));
    assert!(
    ...
    let error = tight_spool
        ...
        .err().unwrap_or_else(|| panic!("artifacts that cannot stream under the ceiling must be refused"));
    assert!(
...
fn streaming_replay_batch_range_probe_releases_on_second_index_refusal() {
    ...
    let error = probe_streaming_replay_batch_range_errors_for_tests(ceiling)
        .err().unwrap_or_else(|| panic!("the second line index must be refused after both payloads are charged"));
...
fn streaming_reader_rejects_a_directory_descriptor_as_non_regular() {
    ...
    let error = probe_streaming_replay_path_swap_for_tests(&directory, &replacement, ceiling)
        .err().unwrap_or_else(|| panic!("a directory descriptor must fail before streaming preflight"));
...
async fn streaming_replay_defers_when_the_second_batch_index_is_starved() {
    ...
    let error = replay_spool_once_with_ceiling_for_tests(&spool, &server.uri(), 4, ceiling)
        .await
        .err().unwrap_or_else(|| panic!("the second batch body may fit while its fixed index is refused"));
...
async fn dead_letter_publish_refuses_a_final_planted_after_writer_open() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a post-open non-file final must fail dead-letter publication"));
    set_spool_write_hook_for_tests(None);
...
async fn dead_letter_publish_refuses_a_completed_temp_path_replacement() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a replaced completed dead-letter temp must fail publication"));
    set_spool_write_hook_for_tests(None);
...
async fn dead_letter_handoff_publishes_nothing_when_the_source_path_becomes_a_directory() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("the dead-letter handoff must refuse a replaced claim pathname"));
...
async fn claim_substituted_after_the_validated_replay_open_is_never_mutated() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a substituted claim pathname must fail the tick"));
    drop(clear);
...
async fn claim_substituted_before_finalization_publishes_nothing_and_never_mutates() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("the terminal claim operation must refuse a substituted claim pathname"));
    drop(clear);
...
async fn wrong_type_claim_before_finalization_publishes_nothing_and_never_mutates() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("finalization must refuse a wrong-type claim pathname"));
    drop(clear);
...
async fn wrong_type_claim_during_dead_letter_append_is_unauthorized_not_retryable() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a wrong-type claim during dead-letter append must fail the tick"));
    drop(clear);
...
async fn substituted_claim_during_dead_letter_open_is_unauthorized_not_retryable() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a substituted claim during dead-letter open must fail the tick"));
    drop(clear);
...
async fn finalize_authorization_precedes_generic_publication_failure() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a blocked metadata store must still fail the tick"));
    drop(clear);
...
async fn substituted_claim_before_metadata_publish_is_unauthorized_not_retryable() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a substituted claim at metadata publication must fail the tick"));
    drop(clear);
...
async fn wrong_type_claim_before_terminal_removal_is_unauthorized_not_retryable() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a wrong-type claim at terminal removal must fail the tick"));
    drop(clear);
...
fn claim_renewal_refuses_a_substituted_pathname_as_unauthorized() {
    ...
    match spool
        ...
        .err().unwrap_or_else(|| panic!("a substituted claim pathname must not be renewed"))
    {
...
async fn initial_ceiling_release_never_promotes_a_substituted_claim() {
    ...
    let error = replay_spool_once_with_ceiling_for_tests(&spool, "http://127.0.0.1:1/", 4, ceiling)
        .await
        .err().unwrap_or_else(|| panic!("the bound initial release must refuse a substituted claim"));
    drop(peer_hold);
    drop(clear);
...
async fn spool_replay_defers_when_ceiling_cannot_hold_stream_and_batch() {
    ...
    let error = replay_spool_once_with_ceiling_for_tests(&spool, "http://127.0.0.1:1/", 4, ceiling)
        .await
        .err().unwrap_or_else(|| panic!("a ceiling that cannot hold one row and batch must defer"));
    assert!(
...
async fn spool_replay_defers_when_a_peer_reservation_starves_batch_admission() {
    ...
    let error = replay_spool_once_with_ceiling_for_tests(&spool, &server.uri(), 4, ceiling)
        .await
        .err().unwrap_or_else(|| panic!("a peer-saturated ceiling must defer before delivery"));
    assert!(
        error.contains("ceiling") || error.contains("deferred"),
...
async fn spool_replay_defers_when_a_peer_reservation_starves_dead_letter_tally() {
    ...
    let error = replay_spool_once_with_ceiling_for_tests(&spool, "http://127.0.0.1:1/", 4, ceiling)
        .await
        .err().unwrap_or_else(|| panic!("a peer-saturated ceiling must defer before isolation state exists"));
    assert!(
        error.contains("ceiling") || error.contains("deferred"),
...
async fn dead_letter_publish_faults_restore_the_authoritative_source() {
    ...
    let error = replay_spool_once_for_tests(&blocked, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a blocked dead-letter temp must fail closed"));
    assert!(
    ...
    let error = replay_spool_once_for_tests(&occupied, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a directory planted at the dead-letter payload path must fail closed"));
    assert!(
        error.contains("failed to replace prior dead-letter payload")
            || error.contains("dead-letter")
            || error.contains("Is a directory")
    ...
    let empty_error = probe_empty_dead_letter_publish_for_tests(&empty_probe, &source)
        .err().unwrap_or_else(|| panic!("empty dead-letter payloads must not publish"));
    assert!(
...
fn namespace_coordination_lock_rejects_nonempty_and_repairs_insecure_mode() {
    ...
    let error = spool
        ...
        .err().unwrap_or_else(|| panic!("a non-empty coordination lock must fail closed"));
    assert!(
...
fn namespace_coordination_lock_open_refuses_a_directory_replacement() {
    ...
    let error = spool
        ...
        .err().unwrap_or_else(|| panic!("a directory planted as the coordination lock must fail closed"));
    assert!(
...
fn probe_helpers_fail_closed_on_ceiling_starvation_and_invalid_rows() {
    ...
    let capacity_error = probe_preallocated_payload_capacity_guard_for_tests(exhausted)
        .err().unwrap_or_else(|| panic!("a one-byte ceiling must refuse the preallocated probe"));
    assert!(
    ...
    let range_error = probe_streaming_replay_batch_range_errors_for_tests(range_ceiling)
        .err().unwrap_or_else(|| panic!("a 64-byte ceiling must refuse the batch index reservation"));
    assert!(
    ...
    let empty_error = publish_dead_letter_payload_for_tests(&spool, &source, b"")
        .err().unwrap_or_else(|| panic!("empty dead-letter probe rows must fail closed"));
    assert!(
    ...
    let oversized_error = publish_dead_letter_payload_for_tests(&spool, &source, &oversized)
        .err().unwrap_or_else(|| panic!("oversized dead-letter probe rows must fail closed"));
    assert!(
    ...
    let materialize_error = publish_dead_letter_payload_for_tests(&spool, &source, b"{\"a\":1}")
        .err().unwrap_or_else(|| panic!("a saturated ceiling must refuse dead-letter materialization"));
    assert!(
    ...
    let index_error = publish_dead_letter_payload_for_tests(&spool, &source, row)
        .err().unwrap_or_else(|| panic!("a ceiling that only fits the row must refuse the index reservation"));
    assert!(
...
fn stale_dead_letter_claim_cannot_delete_completed_handoff() {
    ...
    let error = publish_dead_letter_payload_for_tests(&spool, &stale_claim_path, row)
        .err().unwrap_or_else(|| panic!("stale claim must not reopen dead-letter writer"));
    assert!(
        error.contains("no longer resolves to the authoritative claimed artifact"),
...
fn substituted_regular_file_at_claim_cannot_clear_completed_dead_letter_siblings() {
    ...
    let error =
        probe_empty_dead_letter_publish_with_identity_for_tests(&spool, &claim_path, &pinned)
            .err().unwrap_or_else(|| panic!("a substituted claim must be refused before publication"));
    assert!(
        error.contains("no longer resolves to the authoritative claimed artifact"),
...
fn exact_live_claim_identity_still_clears_prior_dead_letter_siblings() {
    ...
    let error =
        probe_empty_dead_letter_publish_with_identity_for_tests(&spool, &claim_path, &pinned)
            .err().unwrap_or_else(|| panic!("empty publish must still be refused"));
    assert!(
...
fn substituted_regular_file_at_claim_cannot_replace_dead_letter_artifacts() {
    ...
    .err().unwrap_or_else(|| panic!("a substituted claim must not authorize payload replacement"));
    assert!(
    ...
    .err().unwrap_or_else(|| panic!("a substituted claim must not authorize metadata replacement"));
    assert!(
...
fn directory_at_claim_cannot_publish_or_clear_dead_letter_siblings() {
    ...
    let empty_error = probe_empty_dead_letter_publish_for_tests(&spool, &claim_path)
        .err().unwrap_or_else(|| panic!("a directory claim must be refused before publication"));
    assert!(
    ...
    let payload_error = publish_dead_letter_payload_for_tests(&spool, &claim_path, row)
        .err().unwrap_or_else(|| panic!("a directory claim must not authorize payload publication"));
    assert!(
        payload_error.contains("no longer resolves to the authoritative claimed artifact"),
        "unexpected payload authorization diagnostic: {payload_error}"
    );
    ...
    let meta_error = write_dead_letter_meta_for_tests(&spool, &claim_path, &payload_path, row, 1)
        .err().unwrap_or_else(|| panic!("a directory claim must not authorize metadata publication"));
    assert!(
        meta_error.contains("no longer resolves to the authoritative claimed artifact"),
        "unexpected metadata authorization diagnostic: {meta_error}"
    );
...
fn dead_letter_open_fails_closed_when_prior_rejected_payload_is_a_directory() {
    ...
    let error = publish_dead_letter_payload_for_tests(&spool, &claim_path, row)
        .err().unwrap_or_else(|| panic!("directory at prior rejected payload must fail open cleanup"));
    assert!(
...
fn dead_letter_open_fails_closed_when_prior_rejected_meta_is_a_directory() {
    ...
    let error = publish_dead_letter_payload_for_tests(&spool, &claim_path, row)
        .err().unwrap_or_else(|| panic!("directory at prior rejected meta must fail open cleanup"));
    assert!(
...
fn dead_letter_meta_publish_fails_closed_when_prior_meta_is_a_directory() {
    ...
    let error = write_dead_letter_meta_for_tests(&spool, &claim_path, &payload_path, row, 1)
        .err().unwrap_or_else(|| panic!("directory at prior meta must fail metadata replace"));
    assert!(
...
fn directory_at_claim_publishes_no_dead_letter_evidence_at_all() {
    ...
    let before = spool.cached_stats_for_tests();
    let payload_error = publish_dead_letter_payload_for_tests(&spool, &claim_path, row)
        .err().unwrap_or_else(|| panic!("a wrong-type claim pathname must authorize no payload publication"));
    assert!(
        payload_error.contains("no longer resolves to the authoritative claimed artifact"),
        "unexpected payload authorization diagnostic: {payload_error}"
    );
    ...
    let meta_error = write_dead_letter_meta_for_tests(&spool, &claim_path, &payload_path, row, 1)
        .err().unwrap_or_else(|| panic!("a wrong-type claim pathname must authorize no metadata publication"));
    assert!(
        meta_error.contains("no longer resolves to the authoritative claimed artifact"),
        "unexpected metadata authorization diagnostic: {meta_error}"
    );
...
fn wrong_type_claim_between_open_and_append_admits_no_bytes_and_evicts_nothing() {
    ...
    .err().unwrap_or_else(|| panic!("a claim replaced between open and append must admit no rows"));
    drop(claim);
    assert!(
        error.contains("no longer resolves to the authoritative claimed artifact"),
...
fn wrong_type_claim_between_append_and_publish_never_clobbers_the_final_name() {
    ...
    .err().unwrap_or_else(|| panic!("a claim replaced between append and publish must publish nothing"));
    drop(claim);
    assert!(
        error.contains("no longer resolves to the authoritative claimed artifact"),
...
fn decode_spool_helper_and_path_swap_probe_fail_closed_on_missing_inputs() {
    ...
    let decode_error = decode_spool_file_for_tests(&missing)
        .err().unwrap_or_else(|| panic!("a missing spool path must fail closed before allocation"));
    assert!(
    ...
    #[cfg(unix)]
    {
        ...
        let ceiling = leaked_chargeback_test_ceiling(8 * 1024 * 1024);
        let error = probe_streaming_replay_path_swap_for_tests(&source, &replacement, ceiling)
            .err().unwrap_or_else(|| panic!("a missing replacement path must fail the identity probe setup"));
        assert!(
...
async fn streaming_replay_refuses_a_post_preflight_path_swap_without_mutating_it() {
    ...
    let error = replay_spool_once_for_tests(&spool, &server.uri())
        .await
        .err().unwrap_or_else(|| panic!("a post-preflight substitute must not be quarantined or delivered"));
    drop(clear);
...
async fn streaming_replay_defers_when_ceiling_starves_between_preflight_and_reopen() {
    ...
    let error = replay_spool_once_with_ceiling_for_tests(&spool, "http://127.0.0.1:1/", 4, ceiling)
        .await
        .err().unwrap_or_else(|| panic!("ceiling starvation between passes must defer without quarantine"));
    assert!(
        error.contains("ceiling") || error.contains("deferred"),
...
async fn streaming_replay_never_uses_a_quarantine_target_for_a_substituted_claim() {
    ...
    let error = replay_spool_once_for_tests(&spool, "http://127.0.0.1:1/")
        .await
        .err().unwrap_or_else(|| panic!("a substituted artifact must fail before quarantine is authorized"));
    drop(clear);
    assert!(
        error.contains("no longer resolves to the authoritative claimed artifact"),
        "unexpected substituted-claim diagnostic: {error}"
...
fn insert_query_params_reject_credential_bearing_names() {
    ...
    for name in [
        ...
        let err = validate_plugin_config("api_chargeback_sink", &config)
            .err().unwrap_or_else(|| panic!("credential-bearing parameter names must be rejected"));
        assert!(
...
fn spool_write_is_refused_rather_than_materialized_under_a_saturated_ceiling() {
    ...
    let error = spool
        ...
        .err().unwrap_or_else(|| panic!("a saturated ceiling must refuse the spool artifact"));
...
fn compact_snapshot_recovery_is_refused_rather_than_built_under_a_saturated_ceiling() {
    ...
    let error = probe_compact_recovery_retry_for_tests(ceiling, events)
        .err().unwrap_or_else(|| panic!("a saturated ceiling must refuse the recovery payload"));
...
fn a_zstd_artifact_without_a_frame_content_size_still_decodes_via_the_ratio_clamp() {
    ...
    fs::write(&bomb_path, &bomb).unwrap();
    let err = decode_spool_file_for_tests(&bomb_path)
        .err().unwrap_or_else(|| panic!("the ratio clamp must still bound a header-less high-ratio archive"));
    assert!(
        err.contains("decompression bound"),
        "unexpected error: {err}"
    );
...
fn a_declared_zstd_size_cannot_bypass_the_decompression_ratio_limit() {
    ...
    let err = decode_spool_file_for_tests(&path)
        .err().unwrap_or_else(|| panic!("a declared size must not bypass the decompression-ratio limit"));
    assert!(
        err.contains("decompression bound"),
        "unexpected error: {err}"
    );
...
async fn oversized_billing_labels_are_refused_at_admission() {
    ...
    for field in ["currency", "pricing_version"] {
        ...
        let error = ApiChargebackSink::new(&rejected, PluginHttpClient::default(), "ferrum")
            .err().unwrap_or_else(|| panic!("an unexportable label must be refused"));
        assert!(
...
async fn cold_admission_rejects_an_incomplete_clickhouse_client_certificate_pair() {
    ...
    for field in ["client_cert_file", "client_key_file"] {
        ...
        let error = ApiChargebackSink::new(&config, PluginHttpClient::default(), "ferrum")
            .err().unwrap_or_else(|| panic!("an unpaired client identity must be refused"));
        assert!(
