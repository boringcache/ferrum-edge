...
async fn kafka_rejects_topic_names_kafka_can_never_create() {
    ...
    for topic in [
        ...
        .expect_err("an invalid Kafka topic name must be refused at admission");
        assert!(
...
async fn kafka_rejects_transactional_producer_configuration() {
    ...
    for config in [
        ...
        let error = KafkaLogging::new(&config, &default_http_client())
            .expect_err("transactional.id must be refused before a generation is published");
        assert!(
...
async fn kafka_rejects_unsupported_sasl_credential_shapes_without_rewriting_them() {
    for (username, password) in [
        ...
        let error = KafkaLogging::new(&config, &default_http_client())
            .expect_err("an unsupported credential shape must be rejected, not rewritten");
        assert!(
...
async fn kafka_message_timeout_ms_is_bounded_by_the_documented_range() {
    ...
    .expect_err("a message_timeout_ms above librdkafka's range must be rejected");
    assert!(
