...
fn test_unknown_keys_rejected_with_path_qualified_suggestions() {
    ...
    assert_eq!(
        err,
        "grpc_web: `config`: unknown configuration key(s): \"config.expose_header\" (did you mean `expose_headers`?)"
    );
    ...
    assert_eq!(
        err,
        "grpc_web: `config`: unknown configuration key(s): \"config.a_unknown\", \"config.expose_headerz\" (did you mean `expose_headers`?), \"config.z_unknown\""
    );
