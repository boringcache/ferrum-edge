...
async fn accepted_generation_hands_every_distinct_mmdb_path_to_cache_build() {
    ...
    let plugins = cache.request_view("ferrum", "http", ProxyProtocol::Http).plugins();
...
async fn delta_budget_counts_retained_and_rebuilt_mmdb_snapshots() {
    ...
    cache
        .apply_delta(&config, &HashSet::from([NamespacedResourceId::new("ferrum", "p2")]), &[], false)
        .unwrap();
    ...
    let p1_plugins = cache.request_view("ferrum", "p1", ProxyProtocol::Http).plugins();
    ...
    let p2_plugins = cache.request_view("ferrum", "p2", ProxyProtocol::Http).plugins();
