...
// path.
//
// These never took a pre-`after_proxy` snapshot, so the gate reads live headers.
// That makes the response *status* the load-bearing signal for whether range or
// delta headers describe these bytes at all — some of these bytes come from a
// provider that may echo representation metadata which never described this
// response.
// ---------------------------------------------------------------------------
