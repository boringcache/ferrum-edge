...
pub(crate) async fn validate_plugin_security_composition_candidates(
    db: &dyn DatabaseBackend,
    state: &AdminState,
    namespace: &str,
    proxies: &[Proxy],
    ...
    crate::plugin_cache::validate_plugin_security_composition_candidate(&candidate, &http_client)
...
/// associations.
pub(crate) async fn validate_plugin_security_composition_api_spec_replacement_candidate(
    db: &dyn DatabaseBackend,
    state: &AdminState,
    namespace: &str,
...
pub(crate) fn validate_plugin_security_composition_restore_candidate(
    state: &AdminState,
    replacement: &GatewayConfig,
) -> Result<(), AfterValidateError> {
    let http_client = super::plugin_validation_http_client(state);
    crate::plugin_cache::validate_plugin_security_composition_candidate(replacement, &http_client)
        .map_err(|error| AfterValidateError::BadRequest(vec![error]))
}
...
impl AdminResource for PluginConfig {
    ...
    async fn after_validate(
        ...
        validate_plugin_security_composition_candidates(
            db,
            state,
            namespace,
            &[],
