...
//!   ([`super::charged_decode::SupportedCoding`]), reserved before that
//!   decoder is constructed and
...
use crate::proxy::response_buffer_budget::{BudgetRef, ResponseBufferReservation, charged_bytes};
...
/// Map the shared charged decoder's failure onto this gate's vocabulary.
///
/// The decoder deliberately does not know about response terminals: a byte-level
/// failure is a statement about the REPRESENTATION, while a refused reservation
/// is a statement about the GATEWAY and must not be blamed on a correct backend.
fn decode_failure_from(error: ChargedDecodeError) -> DecodeFailure {
    match error {
        ChargedDecodeError::Unsupported => {
            DecodeFailure::Rejected(RepresentationRejection::UnsupportedCoding)
        }
        ChargedDecodeError::Malformed => {
            DecodeFailure::Rejected(RepresentationRejection::MalformedCoding)
        }
        ChargedDecodeError::TooLarge => {
            DecodeFailure::Rejected(RepresentationRejection::DecodedBodyTooLarge)
        }
        ChargedDecodeError::CapacityRefused => DecodeFailure::CapacityRefused,
    }
}
