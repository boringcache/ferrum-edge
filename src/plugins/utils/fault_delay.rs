...
//!    A delayed unit may be an HTTP request, a TCP/UDP/DTLS session admission,
//!    or a UDP/DTLS client→backend datagram parked on the isolated session
//!    worker (never the shared listener recv loop).
