...
///
/// Provides atomic counter and key-value operations using native Redis
/// commands, plus the single HTTP-window admission script
/// ([`HTTP_WINDOW_ADMISSION`]) that rate-limit quotas evaluate with `EVALSHA`.
/// It does NOT fall back on its own: an unreachable endpoint
...
/// Largest window list one admission script call may evaluate.
///
/// Configuration cannot reach this: `rate_limiting` produces at most three
/// windows (per second/minute/hour) and the GraphQL / gRPC-method limiters
/// exactly one. The bound keeps the script's argument vector, state value, and
/// per-call work statically small, and a caller that somehow exceeds it fails
/// closed into the configured `redis_failure_policy` rather than growing the
/// key.
pub(crate) const MAX_REDIS_ADMISSION_WINDOWS: usize = 16;
...
impl RedisRateLimitClient {
    ...
    ///   outcome.
    ///
    /// `WATCH`-based rather than scripted so RESP-compatible servers without
    /// scripting still fence correctly (same rationale as
    /// [`Self::delete_if_value_matches`]); only the rate-limit admission path
    /// requires `@scripting`, and this fence backs consumers that do not.
    /// Only one key is touched, so the transaction is also slot-safe on
    /// sharded deployments.
    ///
