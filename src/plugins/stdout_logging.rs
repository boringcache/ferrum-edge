...
impl Plugin for StdoutLogging {
    ...
    /// Reusable (issue #5583): pure observability. It is a SINK — it acts only
    /// in the post-transaction logging hooks, serializing summaries the proxy
    /// already produced. It takes no admission decision, holds no per-request
    /// budget, makes no external
