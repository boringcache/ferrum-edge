...
pub fn error_response_for_content_type(
    ...
    // native HTTP/2/H3 response headers. Keep a separate trailer-only map for
    // frame construction so early gateway refusals have the same wire shape as
    // transformed backend responses.
