            ...
            .enumerate()
        {
            ...
            for (listen_port_index, (listen_port, hosts)) in listen_bindings.iter().enumerate() {
                ...
                proxies.push(proxy_for_route(RouteProxySpec {
                    ...
                    namespace: config_namespace.clone(),
                    // Listener-intersected SNI names from the binding, not the
                    // route's raw `spec.hostnames`, so a TLSRoute cannot claim
                    // names outside the listener's delegated set.
                    hosts: hosts.clone(),
                    listen_path: None,
                    strip_listen_path: false,
