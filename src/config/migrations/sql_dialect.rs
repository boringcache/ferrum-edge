...
mod tests {
    ...
    // `COLLATE utf8mb4_bin` so uniqueness on `(namespace, name)` and similar
    ...
    // sensitive but still folds NFC/NFD equivalents. MySQL 8.0+ floor; the
    // project test infra already runs MySQL 8 (tests/scripts/setup_db_tls.sh).
    // ------------------------------------------------------------------
    ...
    fn test_mysql_identity_columns_reject_uca_collation() {
        ...
        for (label, sql) in [
            ...
            assert!(
                ...
                "{label} must use binary collation for byte-exact identity"
            );
            assert!(
