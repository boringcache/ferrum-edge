...
mod tests {
    ...
    #[test]
    fn parses_request_host_and_string_port() {
        assert_eq!(
            parse_metric_tag_cel_expression("request.host").unwrap(),
            MetricTagCelExpr::Attribute {
                name: MetricTagCelAttr::RequestHost
            }
        );
        assert_eq!(
            parse_metric_tag_cel_expression("string(destination.port)").unwrap(),
    ...
    #[test]
    fn parses_has_ternary_without_echoing_input_on_error() {
        let expr =
            parse_metric_tag_cel_expression(r#"has(request.host) ? request.host : "unknown""#)
                .unwrap();
        assert!(matches!(expr, MetricTagCelExpr::HasThenElse { .. }));
        let err = parse_metric_tag_cel_expression("request.headers[\"authorization\"]")
    ...
    #[test]
    fn rejects_http_only_attrs_for_tcp_families() {
        let expr = parse_metric_tag_cel_expression("request.method").unwrap();
        assert!(validate_metric_tag_cel_for_families(&expr, true).is_err());
        assert!(validate_metric_tag_cel_for_families(&expr, false).is_ok());
    }

    #[test]
    fn evaluates_missing_attrs_as_empty_and_sanitizes() {
        ...
        let host = parse_metric_tag_cel_expression("request.host").unwrap();
        assert_eq!(evaluate_metric_tag_cel(&host, ctx), "");
        let port = parse_metric_tag_cel_expression("string(destination.port)").unwrap();
