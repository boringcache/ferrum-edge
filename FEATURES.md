...
- Five subcommands: `run` (foreground gateway), `validate` (config check), `reload` (SIGHUP — a config reload in file mode and in mesh mode with a local file/xDS config source; a logged no-op in every other mode), `health`, `version`
