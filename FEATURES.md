...
- **AI Prompt Compressor** — shorten admitted OpenAI Chat/Text Completions prompts with a strictly bounded model-free statistical (extractive) filter. Rewrites configured-role `messages[].content` and legacy `prompt`, preserves matching-backtick code, URLs, Unicode numbers, common identifiers, negations, and nested `preserve_tag` text, and records final-representation per-instance plus aggregate savings metadata. Successful rewrites intentionally reserialize the whole JSON body
