...
- **Compression** — on-the-fly response compression (gzip, brotli) with Accept-Encoding negotiation, content-type filtering, minimum body size threshold, bounded `spawn_blocking` codec offload, and optional request decompression with hard-capped zip-bomb / amplification protection and fail-closed Content-Encoding list parsing
