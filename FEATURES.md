...
- **SPIFFE identity** — extracted from mTLS peer certificates and HBONE W3C Baggage headers with trust-domain aliasing for federated multi-cluster. Workload API serves X.509-SVIDs, plus JWT-SVID mint/validate/bundle streaming (ES256, attested-subject-only, bounded lifetimes, rotation overlap) on the `internal` CA backend; the `spire` backend answers the JWT RPCs `UNIMPLEMENTED` ([#3617](https://github.com/ferrum-edge/ferrum-edge/issues/3617))
