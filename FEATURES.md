...
- **MCP Gateway** — Agent Tool Gateway for HTTP-based MCP JSON-RPC traffic. Supports transparent proxying, aggregate-router initialize/catalog/session mediation, namespaced `tools/list` and `tools/call` routing, basic resource/prompt routing, bounded JSON-RPC batch admission/response handling, multiplexed aggregate SSE sessions (GET listener with per-stream identity routing, bounded queues/replay, disconnect and reload cleanup), tool argument and optional tool-result JSON Schema validation (`validation.validate_tool_results`), deny/hide tool policy, and `mcp.*` metadata for existing Ferrum policy, logging, tracing, chargeback, and alerting plugins
