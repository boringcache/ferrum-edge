...
- **MCP Gateway** — Agent Tool Gateway for HTTP-based MCP JSON-RPC traffic. Supports transparent proxying, aggregate-router initialize/catalog/session mediation, namespaced `tools/list` and `tools/call` routing, basic resource/prompt routing, bounded JSON-RPC batch admission/response handling, MCP Streamable HTTP event streams over HTTP/1.1, HTTP/2, and native HTTP/3 (a POST carrying a JSON-RPC request is always answered on that POST — `application/json` when the gateway already holds the result, its own `text/event-stream` carrying the governed bytes when the result had to be fetched upstream — while `202` stays reserved for notification-only POSTs; one GET listener per session carries server-to-client traffic and never a POST request's response, with an explicit `Last-Event-ID` resuming the stream an earlier POST began from the same retained-event ring; identities opened before catalog/upstream work so `notifications/cancelled` can suppress an in-flight result and its POST stream closes carrying no message, responses routed by a bounded type-sensitive JSON-RPC request/stream identity, one retained-event ring with a single byte budget, bounded listener lifetime with keepalive plus supersession of a stalled listener, and unconditional teardown on disconnect, session delete, idle eviction, and reload), tool argument and optional tool-result JSON Schema validation (`validation.validate_tool_results`), deny/hide tool policy, and `mcp.*` metadata for existing Ferrum policy, logging, tracing, chargeback, and alerting plugins
