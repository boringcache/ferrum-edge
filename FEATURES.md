...
- **A2A Gateway** — Agent-to-Agent gateway for HTTP/HTTPS JSON-RPC, HTTP+JSON/REST, and gRPC/grpcs A2A traffic. Supports transparent proxying, standard method detection, lightweight method allow/deny policy, HTTP Agent Card URL rewriting, streaming-safe SSE/gRPC pass-through, and `a2a.*` metadata for existing Ferrum policy, logging, tracing, chargeback, and alerting plugins
