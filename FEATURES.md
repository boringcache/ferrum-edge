...
- **HMAC** — versioned request signature verification with body integrity protection via Digest/Content-Digest (RFC 9530 / RFC 3230). Exactly one of those headers is required; sending both, mixing RFC 9530 and legacy spellings, or presenting an unsupported algorithm fails closed. The default `ferrum-hmac-v2` profile binds a mandatory client nonce alongside namespace, consumer identity, authority, method, raw path/query, `Date`, and the literal digest field value, and makes each accepted request **single-use** by claiming that nonce against the shared replay authority before backend dispatch (`replay_scope` must be declared as `process` or, for multi-replica deployments, `shared` over Redis with an enforced no-eviction memory policy). The legacy freshness-only `ferrum-hmac-v1` profile has no replay protection and requires an explicit `allow_unsafe_replayable_v1` acknowledgement
- **JWKS Auth** — multi-provider JWKS JWT validation with claim-based authorization, optional mTLS certificate binding, single-use RFC 9449 DPoP proof checks (each accepted proof is claimed exactly once against the shared replay authority bound to a required exact `issuer` realm; `dpop_replay_scope` must be declared as `process` or, for multi-replica deployments, `shared` over Redis with an enforced no-eviction memory policy), and claim header fan-out
- **LDAP Auth** — LDAP directory authentication via direct bind or search-then-bind with optional AD group filtering
- **SOAP WS-Security** — WS-Security header validation with UsernameToken (PasswordText/PasswordDigest), X.509 signature verification, SAML 2.0 assertion validation (XMLDSIG signature verification against trusted IdP signing certs with enveloped-signature transform, Issuer/NotBefore/NotOnOrAfter/Audience checks, Subject NameID exported as `soap_ws_saml_subject`), timestamp freshness, and nonce replay protection (PasswordDigest binds the UsernameToken `Created` to the outer Timestamp inside a bounded freshness window, each nonce claim is retained for a fixed 93,601-second horizon — the widest acceptance window the schema admits anywhere, so no later reload can outlive a claim in either scope — and `nonce.replay_scope` must be declared as `process` or, for multi-replica deployments, `shared` over Redis); SOAP bodies are decoded from UTF-8 / UTF-16LE / UTF-16BE (BOM + charset) before validation while preserving original wire bytes
...
- **Rate Limiting** — per-IP or per-consumer with configurable windows and optional header exposure; supports centralized Redis-backed mode (`sync_mode: "redis"`) for coordinated rate limiting across multiple data plane instances, with a `redis_failure_policy` that defaults to failing closed rather than silently degrading to per-process budgets (Redis Cluster is screened and rejected, not supported). Compatible with any RESP-protocol server (Redis, Valkey, DragonflyDB, KeyDB, Garnet). TLS uses gateway-level `FERRUM_TLS_CA_BUNDLE_PATH` and `FERRUM_TLS_NO_VERIFY`
...
- **AI Token Metrics** — extract token usage (prompt, completion, total) from LLM responses (OpenAI, Anthropic, Google, Cohere, Mistral, Bedrock, TGI) into transaction metadata for downstream observability. SSE accounting is off by default; explicit opt-in buffers the full response, delays accounting until completion, and disables live delivery
...
- **AI Rate Limiter** — HTTP-only token-aware rate limiting per consumer or IP with sliding window, auto-detecting provider format from responses; native gRPC is unsupported (protobuf frames have no usable usage schema, so it is not attachable to the gRPC protocol view); supports centralized Redis-backed mode for cross-instance token budget coordination. Compatible with any RESP-protocol server (Redis, Valkey, DragonflyDB, KeyDB, Garnet). TLS uses gateway-level settings
...
- **AI Prompt Compressor** — shorten admitted OpenAI Chat/Text Completions prompts with a strictly bounded model-free statistical (extractive) filter. Rewrites configured-role `messages[].content` and legacy `prompt`, preserves matching-backtick code, URLs, Unicode numbers, common identifiers, negations, and nested `preserve_tag` text, removes configured markers through a bounded fallback even when compression is skipped, and records final-representation per-instance plus aggregate savings metadata. Successful compression rewrites intentionally reserialize the whole JSON body
...
- **AI Tool Governor** — deterministic allow/deny/redact/approval policy for AI tool/function calls, enforced on concrete tool names and arguments (max size, required args, JSON Schema, blocked-arg regexes) rather than intent. Screens request tool definitions (`tools[]`/`functions[]`), buffered response tool calls (`choices[].message.tool_calls[]` and legacy `function_call`), and streaming SSE tool-call deltas (held until the call is complete and cleared by policy/approval, then released — or the stream is cut with a terminal SSE error event, never leaking the held call). Optional MCP `tools/call` and A2A JSON-RPC method policy, an optional approval webhook with decision caching, dry-run rollout, and hashed audit metadata (raw arguments never logged or placed in metadata by default). Complements `ai_semantic_firewall`: the firewall catches intent, the governor enforces deterministic tool/action policy
- **AI Transcript Audit** — controlled AI payload capture for compliance/audit: redacted request/response excerpts (shared PII patterns with prompt shield / response guard), keyed HMAC-SHA256 body hashes over the exact observed bytes (keyed with `redaction.hash_secret` for fleet-stable digests, else a per-process random key; never an unkeyed or canonicalized JSON digest, so a collector recomputes them from the raw bytes and semantically equal JSON with different whitespace or key order hashes differently), model/provider, token/guardrail/tool/cache metadata, sampling with always-capture-on-guardrail/error, per-minute caps, and asynchronous batched HTTP export to a collector. Capture modes `metadata_only`/`redacted_body`/`full_body`/`hash_only`; never blocks the proxy hot path unless configured fail-closed on buffer overflow or sink failure
- **MCP Gateway** — Agent Tool Gateway for HTTP-based MCP JSON-RPC traffic. Supports transparent proxying, aggregate-router initialize/catalog/session mediation, namespaced `tools/list` and `tools/call` routing, basic resource/prompt routing, bounded JSON-RPC batch admission/response handling, multiplexed aggregate SSE sessions over HTTP/1.1, HTTP/2, and native HTTP/3 (one GET listener per session carrying every gateway-authored and upstream-routed JSON-RPC response, with final governed event bytes capacity-reserved at the body phase and made listener-visible only after the empty POST-side `202` survives final response policy, identities opened before catalog/upstream work so `notifications/cancelled` can suppress an in-flight result, events routed by a bounded type-sensitive JSON-RPC request/stream identity, one retained-event ring with a single byte budget whose refusals answer inline without leaking stream capacity, explicit `Last-Event-ID` semantics, bounded listener lifetime with keepalive plus supersession of a stalled listener, and unconditional teardown on disconnect, session delete, idle eviction, and reload), tool argument and optional tool-result JSON Schema validation (`validation.validate_tool_results`), deny/hide tool policy, and `mcp.*` metadata for existing Ferrum policy, logging, tracing, chargeback, and alerting plugins
- **A2A Gateway** — Agent-to-Agent gateway for HTTP/HTTPS JSON-RPC, HTTP+JSON/REST, and gRPC/grpcs A2A traffic. Supports transparent proxying, standard method detection, lightweight method allow/deny policy, HTTP and unary gRPC Agent Card URL rewriting, streaming-safe SSE/gRPC pass-through, and `a2a.*` metadata for existing Ferrum policy, logging, tracing, chargeback, and alerting plugins
...
- **Response Mock** — returns configurable mock responses without proxying to the backend. Mock rule paths are relative only for prefix `listen_path` values (trailing slashes trimmed before stripping); exact (`=`), regex (`~`), root (`/`), and host-only proxies match the full request path (see the listen-path scope table in `docs/plugins.md`). Supports matching by HTTP method and path pattern (exact or regex with `~` prefix), configurable status codes, headers, body, and optional latency simulation via `delay_ms`. When `passthrough_on_no_match` is true, unmatched requests continue to the real backend. Supports HTTP and WebSocket upgrade handshakes only — native gRPC is excluded because plugin rejects normalize to trailers-only errors that discard configured bodies. Useful for early API testing, contract testing, and local development

### Spec Expose Plugin

- **Spec Expose** — exposes API specification documents (OpenAPI, Swagger, WSDL, WADL) through `GET` and bodyless `HEAD` on a canonical `/specz` sub-path of each prefix-based listen path, including trailing-slash prefixes. Origin `Content-Encoding` (`gzip` / `x-gzip`, `br`, `identity`) is decoded to identity before cache and `/specz`; unsupported coding is HTTP 502. Without an explicit trusted override, only supported spec media types are preserved from upstream and every other value falls back to `application/octet-stream`; responses include `nosniff`. The unauthenticated endpoint uses bounded single-flight fetch admission, short negative-cache backoff, response-size limits, egress screening, credential-safe diagnostics, and fail-closed custom-CA handling. It short-circuits before authentication but after IP and bot controls.
...
- **Compression** — on-the-fly response compression (gzip, brotli) with Accept-Encoding negotiation, content-type filtering, minimum body size threshold, bounded `spawn_blocking` codec offload with Prometheus saturation/failure counters, identity restore on encode abort, and optional request decompression with hard-capped zip-bomb / amplification protection and fail-closed Content-Encoding list parsing
- **Response Caching** — cache backend responses with TTL, cache key rules, and conditional caching
- **Request Mirror** — duplicate live proxy traffic to a secondary destination for shadow testing, validation, or migration checks. Fire-and-forget — mirror latency never blocks the primary client response. Configurable deterministic evenly spaced percentage sampling (0.1% granularity; no contiguous 1,000-request prefix), optional body forwarding (binary-safe; the finalized body is copied once into an owned `Bytes` per selected mirror instance — the primary buffer is untouched — and charged to that instance's retained-byte + `max_in_flight` budgets), finite absolute mirror deadlines covering every shared-client retry even when primary `backend_read_timeout_ms` is zero, keep-alive-safe bounded response drains, protocol-valid mesh `-shadow` Host values (IPv4/IPv6 literals unchanged), deny-by-default stripping of origin-bound credentials (headers and query) to distinct mirror origins (fail-closed allowlist opt-in; primary request-target unchanged), and authenticated Prometheus `ferrum_request_mirror_*_total` lifecycle counters. Mirror response metadata flows through transaction logs
