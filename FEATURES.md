...
- **HMAC** — versioned request signature verification with body integrity protection via Digest/Content-Digest (RFC 9530 / RFC 3230). The default `ferrum-hmac-v2` profile binds a mandatory client nonce alongside namespace, consumer identity, authority, method, raw path/query, `Date`, and body digest, and makes each accepted request **single-use** by claiming that nonce against the shared replay authority before backend dispatch (`replay_scope` must be declared as `process` or, for multi-replica deployments, `shared` over Redis with an enforced no-eviction memory policy). The legacy freshness-only `ferrum-hmac-v1` profile has no replay protection and requires an explicit `allow_unsafe_replayable_v1` acknowledgement
