...
- **SPIFFE identity** — extracted from mTLS peer certificates and HBONE W3C Baggage headers with trust-domain aliasing for federated multi-cluster. Workload API serves X.509-SVIDs, plus JWT-SVID mint/validate/bundle streaming (ES256, attested-subject-only, bounded lifetimes, operator-configured signing material for restart/HA continuity, provable rotation overlap) on the `internal` CA backend; the `spire` backend serves JWT bundles/validate from the agent's own stream, and delegated JWT mint stays fail-closed `UNIMPLEMENTED` there ([#3617](https://github.com/ferrum-edge/ferrum-edge/issues/3617))
