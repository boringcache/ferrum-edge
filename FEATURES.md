...
- **SOAP WS-Security** — WS-Security header validation with UsernameToken (PasswordText/PasswordDigest), X.509 signature verification, SAML 2.0 assertion validation (XMLDSIG signature verification against trusted IdP signing certs with enveloped-signature transform, Issuer/NotBefore/NotOnOrAfter/Audience checks, Subject NameID exported as `soap_ws_saml_subject`), timestamp freshness, and nonce replay protection (PasswordDigest binds the UsernameToken `Created` to the outer Timestamp inside a bounded freshness window, and `nonce.replay_scope` must be declared as `process` or, for multi-replica deployments, `shared` over Redis); SOAP bodies are decoded from UTF-8 / UTF-16LE / UTF-16BE (BOM + charset) before validation while preserving original wire bytes
