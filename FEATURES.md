...
- **AI Prompt Compressor** — shorten prompt text to cut LLM token usage, cost, and latency using a model-free statistical (extractive) filter with no external models, services, or new dependencies. Rewrites `messages[].content` (per configurable role) and the legacy top-level `prompt`; targets a keep ratio, skips content below a token floor, preserves code blocks, inline code, URLs, numbers, identifiers, and negations verbatim, supports an optional `preserve_tag` for must-keep spans, and records `ai_prompt_compressor.*` token-savings metadata
