...
wait_for_tcp_echo() {
  ...
  local body=""
  for _ in $(seq 1 60); do
    if body="$(tcp_exchange "$GATEWAY_API_STATUS_ADDRESS" "$port" "$payload" 2>/dev/null)" \
...
assert_tcp_exchange_fails() {
  ...
  local label="$2"
  local body=""
  if body="$(tcp_exchange "$GATEWAY_API_STATUS_ADDRESS" "$port" "should-fail" 2>/dev/null)"; then
...
YAML
  ...
  local delete_ok=0
  for _ in $(seq 1 30); do
    if ! tcp_exchange "$GATEWAY_API_STATUS_ADDRESS" "$TCP_BLACKBOX_PORT_DELETE" "post-delete" >/dev/null 2>&1; then
      delete_ok=1
      break
    fi
    sleep 2
...
TCPRoute black-box ports: ${TCP_BLACKBOX_PORT_MAIN},${TCP_BLACKBOX_PORT_CROSS},${TCP_BLACKBOX_PORT_FAIL},${TCP_BLACKBOX_PORT_DELETE}
